...
async function createPackage() {
  const directory = await mkdtemp(path.join(tmpdir(), 'yiru-skill-manifest-'))
...
describe('skill bundle manifest generator', () => {
  ...
  it('joins renamed release history to current branded skill packages', () => {
    const currentSkillNames = new Set(['linear-tickets', 'yiru-cli', 'yiru-emulator-android'])

    expect(normalizeReleasedSkillName('legacy-cli', currentSkillNames)).toBe('yiru-cli')
    expect(normalizeReleasedSkillName('legacy-emulator-android', currentSkillNames)).toBe(
      'yiru-emulator-android'
    )
    expect(normalizeReleasedSkillName('linear-tickets', currentSkillNames)).toBe('linear-tickets')
    expect(normalizeReleasedSkillName('unrelated-package', currentSkillNames)).toBe(
      'unrelated-package'
    )
  })
  ...
  it('rejects rewrites of released snapshots and allows floating-tail replacement', () => {
    ...
    const artifacts = {
      releasedSnapshotCounts: { 'yiru-cli': 2 },
      snapshotRegistry: {
        schemaVersion: 1,
        skills: { 'yiru-cli': [snapshot(1, 'aaa'), snapshot(2, 'bbb'), snapshot(3, 'ccc')] }
      }
    }

    expect(() =>
      assertReleasedHistoryPreserved(
        { schemaVersion: 1, skills: { 'yiru-cli': [snapshot(1, 'aaa'), snapshot(2, 'bbb')] } },
        artifacts
      )
    ).not.toThrow()
    expect(() =>
      assertReleasedHistoryPreserved(
        {
          schemaVersion: 1,
          skills: { 'yiru-cli': [snapshot(1, 'aaa'), snapshot(2, 'bbb'), snapshot(3, 'stale')] }
        },
        artifacts
      )
    ).not.toThrow()
    expect(() =>
      assertReleasedHistoryPreserved(
        {
          schemaVersion: 1,
          skills: { 'yiru-cli': [snapshot(1, 'aaa'), snapshot(2, 'rewritten')] }
        },
        artifacts
      )
    ).toThrow('Released snapshot history changed for yiru-cli at revision 2')
    expect(() =>
      assertReleasedHistoryPreserved(
        {
          schemaVersion: 1,
          skills: {
            'yiru-cli': [snapshot(1, 'aaa'), { ...snapshot(2, 'bbb'), gitTreeSha: 'rewritten' }]
          }
        },
        artifacts
      )
      ...
      assertReleasedHistoryPreserved(
        {
          schemaVersion: 1,
          skills: {
            'yiru-cli': [snapshot(1, 'aaa'), snapshot(2, 'bbb'), snapshot(3, 'stale')]
          }
        },
        { ...artifacts, releasedSnapshotCounts: { 'yiru-cli': 1 } }
      )
    ).toThrow('Released snapshot history is incomplete for yiru-cli')
  ...
  it('tolerates only redundant trailing release-mapping rows', () => {
    ...
    const rows = [
      { appVersion: '1.0.0', skills: { 'yiru-cli': 1 } },
      { appVersion: '1.1.0', skills: { 'yiru-cli': 2 } }
    ]
    const artifacts = {
      currentManifest: { skills: [{ name: 'yiru-cli', releaseRevision: 2 }] },
    ...
    expect(
      isToleratedReleaseMappingPrefix(committedPrefix, {
        ...artifacts,
        currentManifest: { skills: [{ name: 'yiru-cli', releaseRevision: 3 }] }
      })
    ).toBe(false)
    expect(
      isToleratedReleaseMappingPrefix(committedPrefix, {
        ...
        currentManifest: {
          skills: [
            { name: 'yiru-cli', releaseRevision: 2 },
            { name: 'yiru-linear', releaseRevision: 1 }
          ]
        }
      })
    ).toBe(false)
    ...
    expect(
      isToleratedReleaseMappingPrefix(
        serialized({
          schemaVersion: 1,
          releases: [{ appVersion: '0.9.0', skills: { 'yiru-cli': 1 } }]
        }),
        artifacts
      )
    ).toBe(false)
  ...
  it('computes the same Git tree identity as Git', async () => {
    const packageRoot = path.resolve('skills', 'yiru-cli')
    const files = await collectPackageFiles(packageRoot)
    const expected = execFileSync('git', ['ls-tree', 'HEAD:skills', 'yiru-cli'], {
      encoding: 'utf8'
    })
      .trim()
      .split(/\s+/)[2]
