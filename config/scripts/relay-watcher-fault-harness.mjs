...
async function main() {
  ...
  let relay
  try {
    tempRoot = await mkdtemp(join(tmpdir(), 'yiru-relay-watcher-fault-'))
    ...
    const afterPtyMarker = `YIRU_PTY_AFTER_${Date.now()}`
    startIndex = relay.messageCount()
