...
function tempReportPath() {
  const dir = mkdtempSync(join(tmpdir(), 'yiru-terminal-perf-gate-'))
  tempDirs.push(dir)
...
describe('run-terminal-scale-perf-report-gate', () => {
  ...
  it('runs the scale suite with JSON output, then summarizes and budget-checks the report', () => {
    ...
    const status = runTerminalScalePerfReportGate({
      ...
      env: { ...process.env, YIRU_TEST_MARKER: '1' },
      spawnSyncImpl
    })
    ...
    expect(calls[0].options.env.YIRU_TEST_MARKER).toBe('1')
  ...
  it('uses the report path from env when no flag is provided', () => {
    ...
    const status = runTerminalScalePerfReportGate({
      env: { ...process.env, YIRU_E2E_TERMINAL_PERF_REPORT_PATH: reportPath },
      spawnSyncImpl
    })
  ...
  it('uses the HTML report path from env when provided', () => {
    ...
    const status = runTerminalScalePerfReportGate({
      env: {
        ...process.env,
        YIRU_E2E_TERMINAL_PERF_HTML_REPORT_PATH: 'tmp/terminal-report.html',
        YIRU_E2E_TERMINAL_PERF_REPORT_PATH: reportPath
      },
      spawnSyncImpl
    })
