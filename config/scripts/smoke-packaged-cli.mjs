...
function readAppDirArg(argv) {
  ...
  if (process.platform === 'darwin') {
    return 'dist/mac-arm64/Yiru.app'
  }
...
function getPackagedCliPath(appDir) {
  if (process.platform === 'darwin' || appDir.endsWith('.app')) {
    return join(appDir, 'Contents', 'Resources', 'bin', 'yiru')
  }
  if (process.platform === 'win32') {
    return join(appDir, 'resources', 'bin', 'yiru.exe')
  }
  return join(appDir, 'resources', 'bin', 'yiru')
}

const appDir = resolve(readAppDirArg(process.argv.slice(2)))
const tempRoot = await mkdtemp(join(tmpdir(), 'yiru-packaged-cli-smoke-'))
