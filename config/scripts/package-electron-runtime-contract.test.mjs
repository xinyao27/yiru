...
describe('Electron runtime package contract', () => {
  ...
  it('keeps Electron e2e commands local instead of invoking them from GitHub Actions', () => {
    const packageScripts = packageJson.scripts
    const workflowDirectory = join(projectDir, '.github/workflows')
    ...
    expect(workflowSources).not.toContain('pnpm run test:e2e')
    expect(workflowSources).not.toContain('pnpm test:e2e')
    expect(workflowSources).not.toContain('npx playwright')
    expect(workflowSources).not.toContain('tools/win-update-e2e/run.mjs')
  })
})
