...
export class TerminalKittyKeyboardModeTracker {
  ...
  scan(data: string, scanFlags?: TerminalChunkScanFlags): void {
    this.scanInternal(data, false, scanFlags)
  }
  ...
  private scanInternal(data: string, replay: boolean, scanFlags?: TerminalChunkScanFlags): void {
    if (this.scanTail.length === 0 && scanFlags && !scanFlags.hasEsc && !data.includes('\x9b')) {
      return
    }
...
import type { TerminalChunkScanFlags } from './chunk-scan-flags'
