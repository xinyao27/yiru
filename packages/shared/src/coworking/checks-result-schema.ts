...
import type { CoworkingChecksReadResult } from './operation-contract'
...
const safeHttpUrl = z
  ...
  .transform((value, context) => {
    const normalized = normalizeCoworkingChecksHttpUrl(value)
    if (!normalized) {
...
export const CoworkingChecksReadResultSchema: z.ZodType<CoworkingChecksReadResult> = z
  .object({
    review: z
      .object({
...
export function normalizeCoworkingChecksHttpUrl(value: string): string | null {
