...
`@yiru/shared` owns pure cross-process logic, shared types, and the preload contract. Its source
must not import desktop, client, or Electron modules, and internal imports are always relative so
the package never depends on a host alias resolver.
