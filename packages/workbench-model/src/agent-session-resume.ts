...
import type { AgentHookSource, TuiAgent } from './agent-types'
...
export type SleepingAgentLaunchConfig = {
  ...
  /** Exact OMP transcript locator when the session was discovered outside its
   *  default store; OMP otherwise resumes by provider session id. */
...
export function normalizeAgentProviderSession(raw: unknown): AgentProviderSessionMetadata | null {
  ...
  // Why: persisted and relayed metadata crosses a trust boundary too, so apply
  // the same control-character rejection as hook-reported transcript paths.
  const transcriptPath = readTranscriptPathFromKeys(record, ['transcriptPath'])
  return transcriptPath ? { key, id, transcriptPath } : { key, id }
}

/** Compare the values that identify the provider's actual CLI resume target. */
