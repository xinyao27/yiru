...
} from '~renderer/terminal-pane/pane-manager/pane-terminal-output-scheduler'
import { resetAndRefreshAllTerminalWebglAtlases } from '~renderer/terminal-pane/pane-manager/registry'
import { resetTerminalLinkifierHoverState } from '~renderer/terminal-pane/pane-manager/terminal-linkifier-hover-reset'
import {
  enforceTerminalCurrentScrollIntent,
  syncTerminalScrollIntentFromViewport
} from '~renderer/terminal-pane/pane-manager/terminal-scroll-intent'
import type { ScrollState } from '~renderer/terminal-pane/pane-manager/types'
...
export function recoverVisibleTerminalWindowWake({
  ...
  // stale renderer/input surface; Yiru's own hidden-state resume never runs.
  for (const pane of manager.getPanes()) {
