...
} from '~renderer/agent/picker-search'
...
export default function AgentCombobox({
  ...
  const filteredAgents = (() => searchAgentPickerEntries(agents, query))()
  const blankMatchesQuery = (() => allowBlankTerminal && agentPickerBlankTerminalMatches(query))()
  ...
  const setCommandValue = (nextCommandValue: string): void => {
  ...
  const handleSelect = (nextValue: TuiAgent | null) => {
