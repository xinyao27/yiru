...
export function resolveZoomTarget(args: {
  ...
  const editorFocused =
    ...
    Boolean(
      (
        ...
        'diffs-container, .diff-editor, .markdown-preview, .rich-markdown-editor, .rich-markdown-editor-shell'
      )
    )
  ...
  // Why: keyboard/menu zoom in an active browser tab belongs to Yiru chrome.
