...
import { scanWorkspacePortsForTarget, workspacePortScanKeyForTarget } from '~renderer/ports/actions'
import { getExternalWorkspacePorts, getWorkspacePortGroups } from '~renderer/ports/groups'
...
export function PortsStatusSegment({ iconOnly }: PortsStatusSegmentProps): React.JSX.Element {
  ...
  const workspaceGroups = (() => getWorkspacePortGroups(scan))()
  const externalPorts = (() => getExternalWorkspacePorts(scan))()
        ...
        <SelectedTextCopyMenu>
          <div className="border-border flex items-center justify-between gap-2 border-b px-3 py-1.5">
            <div className="text-foreground flex min-w-0 items-center gap-1.5 text-[11px] font-medium">
              <Plug className="text-muted-foreground size-3 shrink-0" />
