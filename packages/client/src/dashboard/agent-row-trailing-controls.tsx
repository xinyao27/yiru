...
import { PaperPlaneRight as Send, CaretDown as ChevronDown, X } from '~renderer/icons/hugeicons'
...
export function DashboardAgentRowTrailingControls({
  ...
  const stopKeyDown = (event: React.KeyboardEvent) => {
  ...
  const handleInlineSendTargetClick = (event: React.MouseEvent<HTMLButtonElement>) => {
    event.preventDefault()
    event.stopPropagation()
