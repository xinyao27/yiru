...
const getElectronCompatibilitySearchEntries = createLocalizedCatalog((): SettingsSearchEntry[] => [
  {
    title: translate(
...
export function getAdvancedPaneSearchEntries(options: {
  includeElectronCompatibility: boolean
}): SettingsSearchEntry[] {
  return [
    ...getAdvancedNetworkSearchEntries(),
    ...(options.includeElectronCompatibility ? getElectronCompatibilitySearchEntries() : [])
  ]
}

function findEntry(title: string): SettingsSearchEntry {
  const entry = getElectronCompatibilitySearchEntries().find(
