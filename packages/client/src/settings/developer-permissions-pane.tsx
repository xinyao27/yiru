...
export function DeveloperPermissionsPane(): React.JSX.Element {
  ...
  const stateById = (() => new Map(states.map((state) => [state.id, state.status] as const)))()
  ...
  const refresh = useEventCallback(async (): Promise<void> => {
    const refreshId = refreshSequenceRef.current + 1
    refreshSequenceRef.current = refreshId
    setLoading(true)
    try {
      const nextStates = await shellClient.developerPermissions.getStatus()
  ...
  const request = async (id: DeveloperPermissionId): Promise<void> => {
    setPendingId(id)
    try {
      const result = await shellClient.developerPermissions.request({ id })
          ...
          return (
            <div key={permission.id} className="flex items-center justify-between gap-4 px-4 py-3">
              <div className="flex min-w-0 items-start gap-3">
                <div className="text-muted-foreground mt-0.5">{permission.icon}</div>
