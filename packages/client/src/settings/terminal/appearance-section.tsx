...
import { TerminalCursorAppearanceSection } from './cursor-appearance-section'
import { TerminalFontSizeSetting } from './font-size-setting'
import { TerminalPaneAppearanceSection } from './pane-appearance-section'
import {
...
import { TerminalThemeCatalogSection } from './theme-sections'
import { TerminalWindowSection } from './window-section'
  ...
  const previewAdvancedContent = showAdvancedDisclosure ? (
    ...
    >
      {advancedGroups.map((group, index) => (
        <div
          key={group.key}
          className={index > 0 ? 'border-border/60 mt-2 border-t pt-4' : undefined}
        >
          {group.node}
        </div>
      ))}
    ...
    <div className="space-y-5">
      ...
      {showPrimaryTypography ? (
        <section className="space-y-3 pt-2">
          ...
          <div className="divide-border/40 border-border/40 ml-4 divide-y border-y">
