import type { GitLabRateLimitSnapshot } from '@yiru/runtime-protocol/workbench/types'
...
export function useGitLabRateLimitSnapshot(options?: { autoRefresh?: boolean }): {
  ...
  const refresh = useEventCallback(async (force = false): Promise<void> => {
    const token = ++latestToken.current
    setIsFetching(true)
    try {
      const target = getActiveRuntimeTarget(settings)
      const params = force ? { force: true } : undefined
      const res = await callRuntimeOrpc(target, (client) => client.gitlab.rateLimit, params ?? {}, {
        timeoutMs: 30_000
      })
