...
import type { AgentActivityDisplayMode, Tab, TerminalTab, TuiAgent } from '~shared/types'
...
function GenericTabRow(props: {
  ...
  const handleClick = useCallback(() => activateSidebarTab(row.tab), [row.tab])
...
const AgentTabRows = React.memo(function AgentTabRows(props: AgentTabRowsProps): React.JSX.Element {
  const {
    agentByUnifiedTabId,
    displayMode,
  ...
  const openTerminalTabByEntityId = useMemo(
    () =>
      new Map(
        rows.flatMap((row) =>
  ...
  const handleDismissAgent = useCallback(
    ...
    [dismissRetainedAgent, dropAgentStatus]
  )
...
export const WorktreeCardTabRows = React.memo(function WorktreeCardTabRows(
  ...
  const launchAgentByTabId = useMemo(
    () => new Map(terminalTabs.map((tab) => [tab.id, tab.launchAgent])),
    [terminalTabs]
  )
  const agentByUnifiedTabId = useMemo(
    () =>
      new Map(
        rows.flatMap((row) => {
