...
export function useRemotePathPreview({
  ...
  const resolveBasePath = useCallback(
    async (base: 'root' | 'home' | 'cwd', generation: number): Promise<string | null> => {
      if (base === 'root') {
        return '/'
      }
      if (base === 'cwd') {
    ...
    [fetchListing, homePathRef, resolvedPath]
  )
  const resolvePathInput = useCallback(
    ...
    [fetchListing, resolveBasePath, resolvedPath]
  )
  const handleInputChange = useCallback(
    ...
    [cancelTimers, clearFileHint, preview, resolvePathInput]
  )
