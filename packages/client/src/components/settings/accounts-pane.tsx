...
export function AccountsPane({
  ...
  const refreshMiniMaxCredentialStatus = async (): Promise<void> => {
    try {
      const status = await shellClient.minimaxCredentials.getStatus()
  ...
  const saveMiniMaxCookie = async (): Promise<void> => {
    ...
    try {
      const status = await shellClient.minimaxCredentials.saveCookie(miniMaxCookieDraft.trim())
  ...
  const clearMiniMaxCookie = async (): Promise<void> => {
    setMiniMaxCredentialBusy(true)
    try {
      const status = await shellClient.minimaxCredentials.clearCookie()
      setMiniMaxConfigured(status.configured)
      setMiniMaxCookieDraft('')
      recordFeatureInteraction('usage-tracking')
    } catch (error) {
  ...
  const visibleSections = [
    ...
    matchesSettingsSearch(searchQuery, getAccountsLocationSearchEntries()) ? (
      <section key="account-runtime" id="accounts-runtime" className="scroll-mt-6 space-y-3">
        {accountRuntimeControls}
      </section>
    ) : null,
    matchesSettingsSearch(searchQuery, getAccountsClaudeSearchEntries()) ? (
      <section key="claude-accounts" id="accounts-claude" className="scroll-mt-6 space-y-4">
        <div className="space-y-1">
    ...
    matchesSettingsSearch(searchQuery, getAccountsCodexSearchEntries()) ? (
      <section key="codex-accounts" id="accounts-codex" className="scroll-mt-6 space-y-4">
        <div className="space-y-1">
          <h3 className="flex items-center gap-2 text-sm font-semibold">
            <OpenAIIcon size={16} />
                        ...
                        <Button
                          ...
                          onClick={(event) => {
                            ...
                            void runCodexAccountAction(
                              `reauth:${account.id}`,
                              () =>
                                shellClient.accounts.codex.reauthenticate({
                                  accountId: account.id
                                }),
                              getProviderAccountRuntime(account)
                            )
    ...
    matchesSettingsSearch(searchQuery, getAccountsGeminiSearchEntries()) ? (
      <section key="gemini" id="accounts-gemini" className="scroll-mt-6 space-y-4">
        <div className="space-y-1">
          <h3 className="flex items-center gap-2 text-sm font-semibold">
            <GeminiIcon size={16} />
    ...
    matchesSettingsSearch(searchQuery, getAccountsOpencodeSearchEntries()) ? (
      <section key="opencode-go" id="accounts-opencode-go" className="scroll-mt-6 space-y-4">
        <div className="space-y-1">
          <h3 className="flex items-center gap-2 text-sm font-semibold">
    ...
    matchesSettingsSearch(searchQuery, getAccountsMiniMaxSearchEntries()) ? (
      <section key="minimax" id="accounts-minimax" className="scroll-mt-6 space-y-4">
