...
import type { PtyBufferSnapshot, PtyConnectResult } from './transport-types'
...
export function connectPanePty(
  ...
  const registerEffectiveLaunchConfig = (
    ...
    if (!effectiveLaunchConfig) {
      if (metadata?.launchAgent) {
        // Why: daemon launch identity can outlive the process while Yiru is
  ...
  const scheduleReattachIdleAgentCursorReset = (): void => {
    ...
    reattachIdleAgentCursorResetTimer = setTimeout(() => {
      ...
      // fire because the agent was already idle before Yiru restarted.
      queueAgentIdleTerminalModeReset()
  ...
  const isForegroundTrackingAllowed = (): boolean => true
  ...
  const onTerminalKeyDown = (event: KeyboardEvent): void => {
    ...
    // Why: user shell frameworks (bash-preexec/iTerm2) can replace Yiru's
  ...
  const registerSideEffectFactConsumerForPty = (ptyId: string): void => {
    ...
    unregisterSideEffectFactConsumer = registerTerminalSideEffectFactConsumer({
      ...
      callbacks: {
        ...
        // Why: hidden panes do not receive terminal bytes; the multiplex fact
        // keeps color-scheme query ownership in the renderer for every host.
  ...
  const onBell = (): void => {
    ...
    // standalone BEL some time after daemon snapshot reattach, even when Yiru
  ...
  // Why: inject YIRU_PANE_KEY so global Claude/Codex hooks can attribute their
  // callbacks to the correct Yiru pane without resolving worktrees from cwd.
  // The key matches the `${tabId}:${leafId}` composite used for cacheTimerByKey
  // and agentStatusByPaneKey. Treat it as opaque outside Yiru.
  const state = useAppStore.getState()
  ...
  const folderWorkspace =
    parsedWorkspaceKey?.type === 'folder'
      ...
      : null
  const workspaceEnv: Record<string, string> = { YIRU_WORKSPACE_ID: deps.worktreeId }
  if (folderWorkspace) {
  ...
  const paneIdentityEnv = {
    ...workspaceEnv,
    YIRU_PANE_KEY: cacheKey,
    YIRU_TAB_ID: deps.tabId,
    YIRU_WORKTREE_ID: deps.worktreeId,
  ...
  const remoteRuntimeOwnerForTransport =
    (restoredPtyIdForTransport
      ? getRuntimeTerminalEnvironmentId(restoredPtyIdForTransport)
      : null) ?? (tab?.ptyId ? getRuntimeTerminalEnvironmentId(tab.ptyId) : null)
  ...
  // Why: multiplex carries ordered side-effect facts for every runtime host;
  // renderer byte parsing is no longer a transport-dependent fallback.
  const mainSideEffectAuthority = true
  ...
  const transportOptions = {
    ...
    // Why: local IPC terminals are now model-owned in main: YiruRuntimeService
  ...
  // Why: multiplex side-effect facts are the sole DECSET 2031 responder so a
  // visible xterm and a hidden reveal path cannot both answer one query.
  const handleHiddenMode2031SubscribeFact = (): void => {
    if (disposed || !transport.getPtyId()) {
      return
    }
  ...
  const ptySizeReassertion = createPtySizeReassertion({
    ...
    getAppliedSize: async () => ({ cols: pane.terminal.cols, rows: pane.terminal.rows }),
  ...
  const runDeferredConnect = (): void => {
    ...
    const buildColdRestoreAgentResumeStartup = (): ColdRestoreAgentResumeStartup | null => {
      ...
      return {
        ...
        env: {
          ...startupPlan.env,
          YIRU_AGENT_LAUNCH_TOKEN: coldRestoreLaunchToken
        },
    ...
    const mergeStartupEnvWithPaneIdentity = (
      ...
      env
        ? {
            ...env,
            ...paneIdentityEnv,
            ...(env.YIRU_AGENT_LAUNCH_TOKEN
              ? { YIRU_AGENT_LAUNCH_TOKEN: env.YIRU_AGENT_LAUNCH_TOKEN }
              : {})
          }
        : undefined
    ...
    const schedulePendingStartupCommandDelivery = (): void => {
      ...
      if (!sshStartupShellReady) {
        if (sshShellReadyFallbackTimer === null) {
          // Why: some SSH shells cannot emit Yiru's ready marker. Prefer the
    ...
    function canUseHiddenOutputSnapshot(ptyId: string | null): ptyId is string {
      return (
        Boolean(ptyId) &&
        transport.getPtyId() === ptyId &&
        typeof transport.serializeBuffer === 'function'
      )
    }
    ...
    function writePtyOutputToXterm(
      ...
      writeTerminalOutput(pane.terminal, data, {
        ...
        // Why: claims the in-progress output delivery's parse-deferred ACK
    ...
    function observeRendererOrderedSeqRegression(meta: PtyDataMeta | undefined): void {
      ...
      if (rendererChannelSeq !== null && meta.seq < rendererChannelSeq) {
        // Why: output delivery is FIFO per PTY, so seq only moves backwards
    ...
    function pulseVisibleLocalPtySizeForTuiRepaint(): void {
      if (!isRendererPtyResizeAuthoritative() || shouldSuppressDesktopPtyResize()) {
        return
      }
      const cols = pane.terminal.cols
      const rows = pane.terminal.rows
    ...
    function skipBackgroundAlternateScreenOutput(data: string): void {
      ...
      pulseVisibleLocalPtySizeForTuiRepaint()
    ...
    const sleptRemoteRuntimeSessionId =
      restoredSessionId && hasSleepingAgentSession ? restoredSessionId : null
    ...
    const candidateHasEagerBuffer = Boolean(candidateReattachSessionId)
    ...
    const eagerLivePtyId =
      ...
      currentTabLivePtyIds.includes(candidateReattachSessionId)
        ? candidateReattachSessionId
        : null
    if (detachedRemoteLeafPtyId || detachedLivePtyId || eagerLivePtyId) {
      ...
      // Why: surface synchronous attach failures through
