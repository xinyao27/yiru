...
export function usePierreFileTreeNativeDrop({
  ...
  const clearTarget = useEventCallback((host?: HTMLElement) => {
  ...
  const onDragOverCapture = (event: React.DragEvent<HTMLElement>) => {
  ...
  const onDragLeaveCapture = (event: React.DragEvent<HTMLElement>) => {
