...
} from '~renderer/source-control/macos-data-access'
import {
  useAllWorktrees,
  useRepoById,
  useRepoMap,
...
export function useGitStatusPolling(options: { enabled?: boolean } = {}): void {
  ...
  const isConnectionReady = (connectionId: string | null | undefined): boolean =>
  ...
  const runFetchStatus = async (request: {
  ...
  const fetchStatusOnChangeSignal = () => {
