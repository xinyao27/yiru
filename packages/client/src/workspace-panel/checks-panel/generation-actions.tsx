...
import type { useChecksPanelGenerationDefaultsState } from './generation-defaults'
...
export function useChecksPanelGenerationActions(context: useChecksPanelGenerationDefaultsState) {
  ...
  const handleGeneratePullRequestFieldsForActive = async (
    ...
    if (useAppStore.getState().pullRequestGenerationRecords[generationKey]?.status === 'running') {
      return
    }
    ...
    try {
      ...
      updatePullRequestGenerationRecord(generationKey, (record) =>
        resolvePullRequestGenerationFailure({
          record,
          requestId,
          error: error instanceof Error ? error.message : 'Failed to generate pull request details'
        })
      )
    }
  }
  const handleCancelGeneratePullRequestFieldsForActive = (): void => {
  ...
  const handlePullRequestGenerationSeedRestored = (): void => {
