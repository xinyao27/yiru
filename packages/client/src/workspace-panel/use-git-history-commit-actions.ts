...
export function useGitHistoryCommitActions({
  ...
  const loadCommitFiles = async (item: GitHistoryItem): Promise<GitBranchChangeEntry[]> => {
  ...
  const openHistoryCommitDiff = async (item: GitHistoryItem): Promise<void> => {
    if (!activeWorktreeId || !worktreePath) {
      return
    }
    try {
  ...
  const copyCommitText = async (text: string, label: string): Promise<void> => {
    try {
