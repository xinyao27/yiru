...
export function useRichMarkdownSearch({
  ...
  const matches = (() => {
    // Why: ProseMirror edits are external to React; this revision schedules the
    // render in which the live document is searched again.
    void searchRevision
  ...
  const openReplace = useEventCallback(() => {
  ...
  const toggleMatchCase = () => setMatchCase((value) => !value)
  const toggleWholeWord = () => setWholeWord((value) => !value)
  const toggleReplaceMode = () => setIsReplaceMode((value) => !value)
  ...
  const handleEditorUpdate = useEventCallback(() => {
