...
export function ConflictReviewPanel({
  ...
  const snapshotTime = new Date(
    file.conflictReview?.snapshotTimestamp ?? mountedAt
  ).toLocaleTimeString()
  const setFileTreeCollapsed = (collapsed: boolean) => {
    ...
    <div className="bg-background flex h-full min-h-0">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="flex min-h-0 flex-1 flex-col">
          {selectedContent ?? (
            <div className="text-muted-foreground flex h-full min-h-0 items-center justify-center px-6 text-center text-sm">
              {translate(
