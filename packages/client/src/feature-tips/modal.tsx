...
import { CommandPaletteTipDialog } from './command-palette-tip-dialog'
...
function FeatureTipVisual({ tip }: { tip: FeatureTip }): JSX.Element {
  ...
  // Why: the command-palette tip has a dedicated wide dialog and never reaches this branch.
  return <CommandPaletteFeatureTipVisual />
}
...
export default function FeatureTipsModal(): JSX.Element | null {
  ...
  if (currentTip.action === 'setup-cli') {
    return (
      <Dialog open={isOpen} onOpenChange={handleOpenChange}>
        ...
        <DialogContent
          className="!flex max-h-[calc(100vh-2rem)] flex-col gap-0 overflow-hidden bg-[color-mix(in_srgb,var(--foreground)_8%,var(--background))] p-0 sm:max-w-4xl md:!h-[min(31rem,calc(100vh-2rem))] md:!flex-row dark:bg-[color-mix(in_srgb,var(--foreground)_16%,var(--background))]"
  ...
  if (currentTip.action === 'learn-command-palette') {
    return (
      <CommandPaletteTipDialog
        open={isOpen}
        tip={currentTip}
        primaryBusy={primaryBusy}
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogContent className="gap-4 p-7 sm:max-w-md" showCloseButton>
