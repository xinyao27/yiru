...
export function useRemoteDirectory(
  ...
  const invalidateRequests = useEventCallback(() => {
    generationRef.current++
  })
  const fetchListing = async (path: string): Promise<RemoteDirectoryBrowseResult> => {
  ...
  const loadDirectory = useEventCallback(async (path: string): Promise<void> => {
