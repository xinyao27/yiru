...
import { searchRepos } from '~renderer/repo/search'
...
const SidebarRepositoryFilterSection = function SidebarRepositoryFilterSection() {
  ...
  const selectedRepos = (() => repos.filter((repo) => selectedRepoIdSet.has(repo.id)))()
  const availableRepos = (() => repos.filter((repo) => !selectedRepoIdSet.has(repo.id)))()
  const matchingAvailableRepos = (() => searchRepos(availableRepos, query))()
  ...
  const clearRepos = () => setFilterRepoIds([])
    ...
    <div className="flex items-center justify-between px-2 py-1">
      <span className="text-muted-foreground inline-flex items-center gap-1 text-[11px] font-semibold">
        ...
        {hasRepoFilter && (
          <Badge
            variant="outline"
            className="text-foreground h-4 min-w-4 px-1 py-0 text-[10px] leading-none font-semibold"
          >
            {selectedCount}
          </Badge>
        )}
