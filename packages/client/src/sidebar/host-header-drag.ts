...
export function useHostHeaderDrag({
  ...
  const clearDeferredComputeFrame = useEventCallback(() => {
  ...
  const computeDrop = useEventCallback(
  ...
  const applyDrop = useEventCallback(
  ...
  const scheduleDeferredDropCompute = useEventCallback((pointerY: number) => {
  ...
  const endDrag = useEventCallback((commit: boolean, pointerY?: number) => {
    const session = dragSessionRef.current
    if (!session) {
      clearDeferredComputeFrame()
  ...
  const onHandlePointerDown = (event: ReactPointerEvent<HTMLElement>, hostId: ExecutionHostId) => {
