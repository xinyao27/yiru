...
type CommentMarkdownProps = React.ComponentPropsWithRef<'div'> & {
  content: string
...
// Why: HoverCardTrigger merges its ref and pointer handlers onto this root.
function CommentMarkdown({
  ...
  ref,
  ...rest
}: CommentMarkdownProps): React.JSX.Element {
  const components = (() => {
    if (!onLinkClick) {
