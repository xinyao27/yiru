...
import { DeleteWorktreeDialogDescription } from './dialog-description'
import { DeleteWorktreeDialogFooter } from './dialog-footer'
import { getDeleteWorktreeDirtyChangeCounts } from './dirty-change-counts'
import { runWorktreeDeletesInParallel } from './flow'
import { DeleteWorktreeLineageNotice } from './lineage-notice'
import { persistDeleteWorktreeConfirmSkipPreference } from './preference-toast'
import { DeleteWorktreeSkipConfirmOption } from './skip-confirm-option'
import { DeleteWorktreeTargetPreview } from './target-preview'
import { DeleteWorktreeWarningPanels } from './warning-panels'

const DeleteWorktreeDialog = function DeleteWorktreeDialog() {
  ...
  const { allWorktrees, repos, worktreeLineageById } = catalog
  ...
  const folderWorkspaceDeleteCount = (() => countFolderWorkspaceDeletes(repoMap, worktrees))()
  ...
  const lineageFolderWorkspaceDeleteCount = (() =>
  ...
  const deleteTargets = (() => (canDeleteAllLineage ? lineageDelete.deleteAllTargets : worktrees))()
  const dirtyChangeCountsByWorktreeId = (() => {
  ...
  useEffect(() => {
    ...
    for (const item of statusTargets) {
      void getRuntimeGitStatus({
        ...
        settings: getSettingsForWorktreeRuntimeOwner({ repos, settings, worktreesByRepo }, item.id),
        worktreeId: item.id,
  ...
  const persistDontAskAgainPreference = (): void => {
  ...
  const handleForceDeletedFromToast = (deletedId: string): void => {
    onDeleted?.([deletedId])
  }
