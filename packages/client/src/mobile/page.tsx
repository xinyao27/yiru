...
export default function MobilePage(): React.JSX.Element {
  ...
  const showStage = useEventCallback((nextStage: FlowStage | null): void => {
  ...
  const showPairedDevices = useEventCallback((deviceCount: number): void => {
  ...
  const loadDevices = useEventCallback(async (): Promise<PairedDevice[]> => {
    try {
  ...
  const generatePairing = useEventCallback(async (rotate: boolean, addressOverride?: string) => {
  ...
  const loadNetworkInterfaces = useEventCallback(async () => {
    if (mountedRef.current) {
