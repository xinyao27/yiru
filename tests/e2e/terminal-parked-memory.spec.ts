...
// scoped to this spec's app launches via yiruAppExtraEnv (same pattern as
...
test.use({
  ...
  yiruAppExtraArgs: ['--enable-precise-memory-info']
})

// Why: 8 hidden tabs is below the 12-tab hot-retain limit, but that limit
// never retains anything here — the YIRU_E2E_TERMINAL_PARKING_DELAY_MS
...
test.describe('Terminal parked memory', () => {
  test('releases renderer terminal memory when hidden tabs park', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.yiru-parked-memory-${runId}.mjs`)
    writeScrollbackFillScript(scriptPath, runId)
    try {
      const { worktreeId, scrollbackTabs } = await setUpScrollbackTabs(yiruPage, scriptPath, runId)
      ...
      // the other 7 park.
      const visibleTab = await createActiveTerminalTab(yiruPage, worktreeId)
      ...
      const metrics = await sampleParkedMemoryMetrics(yiruPage)
      ...
      for (const tab of parkableTabs) {
        expect((await readTerminalTabViewState(yiruPage, tab.tabId)).hasManager).toBe(false)
      }
      expect((await readTerminalTabViewState(yiruPage, lastActiveTab.tabId)).hasManager).toBe(true)
      const visibleState = await readTerminalTabViewState(yiruPage, visibleTab.tabId)
  ...
  test('retains terminal views when parking is disabled', async ({
    ...
    await expect
      .poll(
        () =>
          yiruPage.evaluate(() => window.__store?.getState().settings?.terminalHiddenViewParking),
    ...
    try {
      ...
      expect(await countMountedPaneManagers(yiruPage, scrollbackTabIds)).toBe(SCROLLBACK_TAB_COUNT)
      ...
      for (const tab of scrollbackTabs) {
        const state = await readTerminalTabViewState(yiruPage, tab.tabId)
        expect(state.hasManager).toBe(true)
        expect(state.paneCount).toBeGreaterThan(0)
      }
      expect((await readTerminalTabViewState(yiruPage, visibleTab.tabId)).hasManager).toBe(true)
