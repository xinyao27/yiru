...
test.describe('Terminal typing latency vs scrollback history size', () => {
  test('typing stays responsive as terminal history grows', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.yiru-history-benchmark-${runId}.mjs`)
    writeFileSync(scriptPath, historyEchoScript(runId))
    let commandSent = false
    try {
      await sendToTerminal(yiruPage, ptyId, `node ${JSON.stringify(scriptPath)}\r`)
      commandSent = true
      await waitForRecentTerminalMarker(yiruPage, `HIST_READY_${runId}`, 10_000)
      await focusActiveTerminalInput(yiruPage)

      const phases: PhaseLatency[] = []
      let seq = 0

      const baseline = await measureTypingLatency(yiruPage, runId, 'empty history', seq)
