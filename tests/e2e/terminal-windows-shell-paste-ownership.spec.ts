...
test.describe('Windows terminal shell paste ownership', () => {
  ...
  test('PowerShell default terminal keyboard paste preserves exact content with one PTY owner', async ({
    ...
    await createWindowsDefaultShellTerminalTab(yiruPage, 'powershell.exe')
    await waitForActiveTerminalManager(yiruPage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(yiruPage)
    const runId = randomUUID()
    const sentinel = `YIRU_E2E_POWERSHELL_DONE_${runId}`
    const powershellEscape = '`'
    const payload = [
      `YIRU_E2E_POWERSHELL_PASTE_${runId}`,
      `PowerShell metacharacters: ${powershellEscape} $ " ' ; | & < > @ { } ( )`,
      'quoted Windows path: C:\\Program Files\\Yiru Test\\file name.txt',
      'cmd metacharacters preserved as text: %PATH% !PROMPT! ^ & | < >',
      'Unicode: café 你好 مرحبا 😀',
      `mixed-newline-before\r\nlf-line\ncrlf-line\r\n${sentinel}`
    ].join('\n')
    const scriptPath = path.join(testRepoPath, `.yiru-paste-powershell-shell-${runId}.mjs`)
  ...
  test('cmd.exe default terminal keyboard paste preserves exact content with one PTY owner', async ({
    ...
    await waitForActiveWorktree(yiruPage)
    await ensureTerminalVisible(yiruPage)
    await createWindowsDefaultShellTerminalTab(yiruPage, 'cmd.exe')
    await waitForActiveTerminalManager(yiruPage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(yiruPage)
    const runId = randomUUID()
    const sentinel = `YIRU_E2E_CMD_DONE_${runId}`
    const payload = [
      `YIRU_E2E_CMD_PASTE_${runId}`,
      'cmd metacharacters: %PATH% !PROMPT! ^ & | < >',
      'quoted Windows path: C:\\Program Files\\Yiru Test\\file name.txt',
      'PowerShell metacharacters: ` $ " \' ; @ { } ( )',
      `mixed-newline-before\r\nlf-line\ncrlf-line\r\n${sentinel}`
    ].join('\n')
    const scriptPath = path.join(testRepoPath, `.yiru-paste-cmd-shell-${runId}.mjs`)
    writeFileSync(scriptPath, pasteCollectScript(runId, sentinel, payload))
    let scriptStarted = false
  ...
  test('Git Bash default terminal keyboard paste preserves POSIX-shaped content with one PTY owner', async ({
    ...
    await skipWhenGitBashUnavailable(yiruPage)

    await waitForSessionReady(yiruPage)
    await waitForActiveWorktree(yiruPage)
    await ensureTerminalVisible(yiruPage)
    await createWindowsDefaultShellTerminalTab(yiruPage, WINDOWS_GIT_BASH_SHELL)
    await waitForActiveTerminalManager(yiruPage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(yiruPage)
    const runId = randomUUID()
    const sentinel = `YIRU_E2E_GIT_BASH_DONE_${runId}`
    const payload = [
      `YIRU_E2E_GIT_BASH_PASTE_${runId}`,
    ...
    const scriptPath = path.join(testRepoPath, `.yiru-paste-git-bash-shell-${runId}.mjs`)
    writeFileSync(scriptPath, pasteCollectScript(runId, sentinel, payload))
    let scriptStarted = false
  ...
  test('WSL terminal keyboard paste preserves Linux shell content with one PTY owner', async ({
    ...
    await waitForActiveWorktree(yiruPage)
    await ensureTerminalVisible(yiruPage)
    const wslDistro = await configureActiveProjectWslRuntime(yiruPage)
    test.skip(!wslDistro, 'No WSL distro is available on this Windows host')
    await createWindowsProjectRuntimeTerminalTab(yiruPage, 'wsl.exe')
    await waitForActiveTerminalManager(yiruPage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(yiruPage)
    const runId = randomUUID()
    const sentinel = `YIRU_E2E_WSL_DONE_${runId}`
    const payload = [
      `YIRU_E2E_WSL_PASTE_${runId}`,
      'POSIX shell metacharacters: $ ` " \' ; | & < > * ? [ ] ( )',
    ...
    const scriptPath = path.join(testRepoPath, `.yiru-paste-wsl-shell-${runId}.mjs`)
    writeFileSync(scriptPath, pasteCollectScript(runId, sentinel, payload))
    let scriptStarted = false
  ...
  test('existing WSL terminal keeps paste runtime after default shell changes', async ({
    ...
    const wslDistro = await configureActiveProjectWslRuntime(yiruPage)
    test.skip(!wslDistro, 'No WSL distro is available on this Windows host')
    const tabId = await createWindowsProjectRuntimeTerminalTab(yiruPage, 'wsl.exe')
    await updateWindowsDefaultShellSetting(yiruPage, 'cmd.exe')
    await expect(
      yiruPage.locator(`[data-testid="sortable-tab"][data-tab-id="${tabId}"] [data-shell-icon]`)
    ...
    const ptyId = await waitForActivePanePtyId(yiruPage)
    const runId = randomUUID()
    const sentinel = `YIRU_E2E_WSL_RETENTION_DONE_${runId}`
    const payload = [
      `YIRU_E2E_WSL_RETENTION_PASTE_${runId}`,
    ...
    const scriptPath = path.join(testRepoPath, `.yiru-paste-wsl-retention-${runId}.mjs`)
    writeFileSync(scriptPath, pasteCollectScript(runId, sentinel, payload))
    let scriptStarted = false
