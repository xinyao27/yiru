...
test.describe('Terminal long table scroll restore repro', () => {
  test('reproduces long markdown table artifacts after workspace switch and scroll', async ({
    ...
    await yiruPage.evaluate(() => {
      window.__store
        ?.getState()
        .markFeatureTipsSeen(['yiru-cli', 'cmd-j-palette', 'voice-dictation'])
    })
    ...
    const scriptPath = path.join(testRepoPath, `.yiru-long-table-${runId}.mjs`)
    ...
    try {
      ...
      await scrollActiveTerminalLikeUser(yiruPage)
      await closeFeatureTips(yiruPage)
      const diagnostics = await readTerminalRenderDiagnostics(yiruPage)
  ...
  test('keeps narrow wrapped signer markdown table coherent after restore and scroll', async ({
    ...
    await setRenderedTableViewport(yiruPage)
    await forceDarkTerminalRendererPath(yiruPage)
    await ensureTerminalVisible(yiruPage)
    await waitForActiveTerminalManager(yiruPage, 30_000)
    const ptyId = await waitForActivePanePtyId(yiruPage)
    await waitForPtyShellEcho(yiruPage, ptyId, 15_000)
    ...
    const scriptPath = path.join(testRepoPath, `.yiru-narrow-signer-table-${runId}.mjs`)
    ...
    try {
      ...
      const content = await getTerminalContent(yiruPage, 30_000)
  ...
  test('keeps real emoji markdown table right edge clean after restore and scroll', async ({
    ...
    await ensureTerminalVisible(yiruPage)
    await waitForActiveTerminalManager(yiruPage, 30_000)
    await setNarrowTerminalViewport(yiruPage)
    const renderedTableTerminalCols = await waitForRenderedTerminalColumnsAtMost(
      yiruPage,
      NARROW_TERMINAL_MAX_COLS
    )
    const ptyId = await waitForActivePanePtyId(yiruPage)
    await waitForPtyColumnsAtMost(yiruPage, ptyId, renderedTableTerminalCols)
    const runId = randomUUID()
    const marker = `EMOJI_FIXTURE_TABLE_RESTORE_${runId}`
    const scriptPath = path.join(testRepoPath, `.yiru-emoji-fixture-table-${runId}.mjs`)
    ...
    try {
      ...
      await waitForActiveTerminalManager(yiruPage, 30_000)
      await setNarrowTerminalViewport(yiruPage)
      await waitForRenderedTerminalColumnsAtMost(yiruPage, NARROW_TERMINAL_MAX_COLS)
      await expect
        .poll(() => getTerminalContent(yiruPage, 30_000), {
          timeout: 10_000,
        ...
        .toContain(marker)
      const generatedWidthContent = await getTerminalContent(yiruPage, 30_000)
      ...
      await scrollActiveTerminalToText(yiruPage, retainedEmojiCell)
      await closeFeatureTips(yiruPage)
      await expect
        .poll(() => readActiveTerminalVisibleText(yiruPage), {
          timeout: 5_000,
          message: `${retainedEmojiCell} row fragment should be visible before screenshot`
        })
        .toContain(retainedEmojiCell)
      const diagnostics = await readTerminalRenderDiagnostics(yiruPage)
      const overpaint = await readTerminalRightEdgeOverpaint(yiruPage)
      const wrapDiagnostics = await readTerminalBoxTableWrapDiagnostics(yiruPage)
