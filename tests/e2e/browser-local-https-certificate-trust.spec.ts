...
test.describe('local HTTPS certificate trust', () => {
  ...
  test('approves one exact local certificate endpoint without trusting sibling tabs or ports', async ({
    ...
    try {
      ...
      const firstTab = await createBrowserTab(yiruPage, worktreeId, firstServer.schemeLessUrl)
      const firstSlot = browserSlot(yiruPage, firstTab.id)
      ...
      await expect
        .poll(() => readBrowserHeading(yiruPage, firstTab.id), { timeout: 10_000 })
        .toBe('Local HTTPS request 1')
      await expect
        .poll(() => readBrowserState(yiruPage, firstTab.id, '__localTlsState'))
      ...
      const secondTab = await createBrowserTab(yiruPage, worktreeId, firstServer.secureUrl)
      const secondSlot = browserSlot(yiruPage, secondTab.id)
      await expect(
      ...
      const probeTab = await createBrowserTab(yiruPage, worktreeId, siblingProbeServer.url)
      await expect
        .poll(() => readBrowserState(yiruPage, probeTab.id, '__siblingTlsProbe'))
        .toEqual({ asset: 'blocked', webSocket: 'blocked' })
      expect(firstServer.assetRequestCount()).toBe(1)
      expect(firstServer.webSocketConnectionCount()).toBe(1)

      await switchToBrowserTab(yiruPage, worktreeId, firstTab.id)
      await reloadBrowserGuest(yiruPage, firstTab.id)
      ...
      const firstAddressBar = firstSlot.locator('[data-yiru-browser-address-bar="true"]')
