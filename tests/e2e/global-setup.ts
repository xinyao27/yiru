...
export const TEST_REPO_PATH_FILE = path.join(os.tmpdir(), 'yiru-e2e-test-repo-path.txt')
...
export default function globalSetup(): void {
  ...
  if (process.env.YIRU_E2E_SSH_LOCALHOST === '1' || process.env.YIRU_E2E_SSH_DOCKER === '1') {
    // Why: the SSH specs deploy Yiru's relay from out/relay. The
  ...
  const testRepoDir = realpathSync(mkdtempSync(path.join(os.tmpdir(), 'yiru-e2e-repo-')))
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ name: 'yiru-e2e-test', version: '0.0.0', private: true }, null, 2)}\n`
  )
  ...
  const worktreeDir = path.join(testRepoDir, '..', `yiru-e2e-worktree-${randomUUID()}`)
