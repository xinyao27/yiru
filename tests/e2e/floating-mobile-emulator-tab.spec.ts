...
test('floating Mobile Emulator tab renders content and closes from the tab strip', async ({
  ...
  const tab = await seedFloatingSimulatorTab(yiruPage)
  await openFloatingPanelIfNeeded(yiruPage)

  const openPanel = yiruPage.locator(OPEN_PANEL_SELECTOR).first()
  ...
  await attachPanelScreenshot(yiruPage, testInfo)
