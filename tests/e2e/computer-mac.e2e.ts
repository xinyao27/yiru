...
describe.skipIf(!isMac || !e2eOptIn)('computer-use macOS e2e (TextEdit)', () => {
  ...
  test('list-apps includes TextEdit', async () => {
    const result = await runYiruCli(['computer', 'list-apps', '--json'])
  ...
  test('get-app-state returns TextEdit state', async () => {
    const result = await runYiruCli(['computer', 'get-app-state', '--app', 'TextEdit', '--json'])
  ...
  test('click, type-text, and re-observe show inserted text', async () => {
    const before = parseJsonOutput<{ result: ComputerSnapshotResult }>(
      (await runYiruCli(['computer', 'get-app-state', '--app', 'TextEdit', '--json'])).stdout
    )
    ...
    const marker = `yiru computer e2e ${Date.now()}`
    await runYiruCli(['computer', 'type-text', '--app', 'TextEdit', '--text', marker])
  ...
  test('paste-text and hotkey verify TextEdit text replacement', async () => {
    ...
    const marker = `yiru paste final ${Date.now()}`
    ...
    expect(after.result.snapshot.treeText).toContain(marker)
    expect(after.result.snapshot.treeText).not.toContain('yiru paste first')
  })
  ...
  test('accessibility text actions work when TextEdit is not frontmost', async () => {
    ...
    const marker = `yiru unfocused final ${Date.now()}`
    ...
    expect(after.result.snapshot.treeText).toContain(marker)
    expect(after.result.snapshot.treeText).not.toContain('yiru unfocused first')
  })
  ...
  test('screenshot capture returns image metadata', async () => {
    ...
    expect(envelope.result.screenshot?.path).toContain('yiru-computer-use')
  })
})
