...
test.describe('Browser Tab', () => {
   ...
   * - Browser works and also retains state when switching tabs etc.
   */
  test('creating a browser tab adds it and activates browser view', async ({ yiruPage }) => {
    const worktreeId = (await getActiveWorktreeId(yiruPage))!
    const browserTabsBefore = await getBrowserTabs(yiruPage, worktreeId)

    await createBrowserTab(yiruPage, worktreeId)

    // Wait for the browser tab to appear in the store
    await expect
      .poll(async () => (await getBrowserTabs(yiruPage, worktreeId)).length, { timeout: 5_000 })
      .toBe(browserTabsBefore.length + 1)

    // The active tab type should switch to 'browser'
    await expect.poll(async () => getActiveTabType(yiruPage), { timeout: 3_000 }).toBe('browser')
  })
   ...
   * - Browser works and also retains state when switching tabs etc.
   */
  test('browser tab is created and active in the store', async ({ yiruPage }) => {
    const worktreeId = (await getActiveWorktreeId(yiruPage))!

    await createBrowserTab(yiruPage, worktreeId)
    await expect.poll(async () => getActiveTabType(yiruPage), { timeout: 5_000 }).toBe('browser')

    // Verify the browser tab exists in the store
    const browserTabs = await getBrowserTabs(yiruPage, worktreeId)
    ...
    const activeBrowserTabId = await yiruPage.evaluate(() => {
      const store = window.__store
   ...
   * - Browser works and also retains state when switching tabs etc.
   */
  test('browser tab retains state when switching to terminal and back', async ({ yiruPage }) => {
    const worktreeId = (await getActiveWorktreeId(yiruPage))!
    ...
    await switchToTerminalTab(yiruPage, worktreeId)
    await expect.poll(async () => getActiveTabType(yiruPage), { timeout: 3_000 }).toBe('terminal')

    // Switch back to browser tab
    await switchToBrowserTab(yiruPage, worktreeId, browserTabId!)
    await expect.poll(async () => getActiveTabType(yiruPage), { timeout: 3_000 }).toBe('browser')

    // The browser tab should still exist with the same ID
    const browserTabsAfter = await getBrowserTabs(yiruPage, worktreeId)
  ...
  test('browser webview form state survives switching between browser tabs', async ({
    ...
    try {
      ...
      await writeBrowserInputValue(yiruPage, firstTab!.id, 'first typed value')
      ...
      await writeBrowserInputValue(yiruPage, secondTab!.id, 'second typed value')
      ...
      await switchToBrowserTab(yiruPage, worktreeId, firstTab!.id)
      await expect
        .poll(async () => readBrowserInputValue(yiruPage, firstTab!.id), { timeout: 5_000 })
        .toBe('first typed value')

      await switchToBrowserTab(yiruPage, worktreeId, secondTab!.id)
      await expect
        .poll(async () => readBrowserInputValue(yiruPage, secondTab!.id), { timeout: 5_000 })
  ...
  test('plain links stay current while explicit new-tab gestures activate Yiru tabs', async ({
    electronApp,
    yiruPage
  }) => {
    ...
    try {
      ...
      const baseTabCount = await yiruPage.locator('[data-tab-id]').count()
      await clickBrowserLink(yiruPage, sourceTab!.id, '#external-link')

      const sourceTabLocator = yiruPage.locator(`[data-tab-id="${sourceTab!.id}"]`)
      await expect(sourceTabLocator).toContainText('Linked destination', { timeout: 10_000 })
      await expect(yiruPage.locator('[data-tab-id]')).toHaveCount(baseTabCount)

      await clickBrowserLink(yiruPage, sourceTab!.id, '#return-link')
      await expect(sourceTabLocator).toContainText('Source page', { timeout: 10_000 })
      await clickBrowserLink(yiruPage, sourceTab!.id, '#frame-link', {
      ...
      await clickBrowserLink(yiruPage, sourceTab!.id, '#frame-modifier-link', {
        frameSelector: '#link-frame',
        modifiers: process.platform === 'darwin' ? ['meta'] : ['control']
      })
      await expectBrowserTabActive(yiruPage, 'Frame modifier destination')
      await switchToBrowserTab(yiruPage, worktreeId, sourceTab!.id)
      await clickBrowserLink(yiruPage, sourceTab!.id, '#frame-middle-link', {
        button: 'middle',
        frameSelector: '#link-frame'
      })
      await expectBrowserTabActive(yiruPage, 'Frame middle destination')
      await switchToBrowserTab(yiruPage, worktreeId, sourceTab!.id)

      await clickBrowserLink(yiruPage, sourceTab!.id, '#modifier-link', {
        modifiers: process.platform === 'darwin' ? ['meta'] : ['control']
      })
      await expectBrowserTabActive(yiruPage, 'Modifier destination')
      await switchToBrowserTab(yiruPage, worktreeId, sourceTab!.id)

      const tabCountBeforeCancelledClick = await yiruPage.locator('[data-tab-id]').count()
      await clickBrowserLink(yiruPage, sourceTab!.id, '#cancelled-link')
      await expect(
        yiruPage.locator('[data-tab-id]').filter({ hasText: 'Click handled in page' })
      ).toBeVisible({ timeout: 10_000 })
      await expect(yiruPage.locator('[data-tab-id]')).toHaveCount(tabCountBeforeCancelledClick)

      await clickBrowserLink(yiruPage, sourceTab!.id, '#middle-link', { button: 'middle' })
      await expectBrowserTabActive(yiruPage, 'Middle-click destination')
      await expect
   ...
   * - Browser works and also retains state when switching tabs etc.
   */
  test('browser tab retains state when switching worktrees and back', async ({ yiruPage }) => {
