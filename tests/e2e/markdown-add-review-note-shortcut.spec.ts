...
test.describe('Markdown add-review-note shortcut', () => {
  ...
  test('opens the review-note composer for the current selection in the rich editor', async ({
    ...
    try {
      ...
      const editor = await waitForRichMarkdownEditor(yiruPage)
      await editor.click()
      await yiruPage.keyboard.press('ControlOrMeta+A')

      await yiruPage.keyboard.press('ControlOrMeta+Alt+N')

      await expect(yiruPage.getByPlaceholder('Add note for the AI')).toBeVisible({
        timeout: 5_000
      })
    } finally {
  ...
  test('opens the composer for the current selection in the Monaco source editor', async ({
    ...
    try {
      ...
      const monaco = yiruPage.locator('.monaco-editor').first()
  ...
  test('opens the inline composer for the selected block in the markdown preview', async ({
    ...
    try {
      ...
      await expect(yiruPage.locator('[data-annotation-block-key]').first()).toBeVisible({
        timeout: 25_000
      })
  ...
  test('does not open the composer without a text selection', async ({ yiruPage }, testInfo) => {
    const context = await getActiveWorktreeContext(yiruPage)
    let filePath: string | null = null
    ...
    try {
      ...
      await expect(yiruPage.getByPlaceholder('Add note for the AI')).toHaveCount(0)
    } finally {
      await cleanupMarkdownFixture(filePath)
    }
  })
