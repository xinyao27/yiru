...
test.describe('Settings sidebar search on the Shortcuts pane', () => {
  test('pane-title-only query keeps rows visible and local search usable', async ({ yiruPage }) => {
    ...
    const searchInput = yiruPage.getByPlaceholder('Search settings')
    await expect(searchInput).toBeVisible()
    await searchInput.fill('shortcuts')

    // The query matches the pane title, so the Shortcuts pane auto-activates.
    await expect(yiruPage.getByRole('heading', { name: 'Shortcuts', exact: true })).toBeVisible()

    // Regression: a pane-title-only query used to blank the whole list (0/112).
    await expect(yiruPage.getByText('Go to File', { exact: true })).toBeVisible()
    await expect(yiruPage.getByText('No shortcuts match those filters.')).not.toBeVisible()
    ...
    const localSearch = yiruPage.getByPlaceholder('Search command or keys')
    await localSearch.fill('go to')
    await expect(yiruPage.getByText('Go to File', { exact: true })).toBeVisible()
    await expect(yiruPage.getByText('Force Reload', { exact: true })).not.toBeVisible()

    // A row-matching global query still narrows the list as before.
    await localSearch.clear()
    await searchInput.fill('worktree')
    await expect(yiruPage.getByText('Create worktree', { exact: true })).toBeVisible()
    await expect(yiruPage.getByText('Go to File', { exact: true })).not.toBeVisible()
  })
})
