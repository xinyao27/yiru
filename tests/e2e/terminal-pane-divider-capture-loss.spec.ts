...
test('@headful keeps resizing after the divider loses pointer capture', async ({
  ...
  await setFullscreen(electronApp, yiruPage)
  await addTestRepo(yiruPage, testRepoPath)
  ...
  await waitForPaneCount(yiruPage, 2, 30_000)
  ...
  const before = await readDividerGeometry(yiruPage)
  const startX = box.x + box.width / 2
  const startY = box.y + box.height / 2
  await yiruPage.mouse.move(startX, startY)
  await yiruPage.mouse.down()
  await yiruPage.mouse.move(startX + 140, startY, { steps: 10 })
  await expect
    .poll(async () =>
      Math.abs((await readDividerGeometry(yiruPage)).first.width - before.first.width)
    )
    .toBeGreaterThan(80)
  ...
  await yiruPage.mouse.move(startX + 260, startY, { steps: 10 })
  await yiruPage.mouse.up()
  await expect.poll(async () => gridsMatch(await readDividerGeometry(yiruPage))).toBe(true)
  const after = await readDividerGeometry(yiruPage)
