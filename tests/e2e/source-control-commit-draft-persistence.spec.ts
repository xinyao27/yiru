...
test.describe('Source Control commit draft persistence', () => {
  test('preserves a typed draft when the sidebar tab remounts', async ({
    ...
    try {
      ...
      await openSourceControlForWorktree(yiruPage, testRepoPath, firstWorktree.worktreePath)

      const textarea = yiruPage.getByRole('textbox', { name: 'Commit message' })
      ...
      await expect
        .poll(
          async () => yiruPage.evaluate(() => window.__store?.getState().rightSidebarTab ?? null),
          { timeout: 5_000 }
        )
        .toBe('explorer')
      ...
      await openSourceControlForWorktree(yiruPage, testRepoPath, secondWorktree.worktreePath)
      await expect(textarea).toBeVisible({ timeout: 10_000 })
