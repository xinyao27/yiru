...
import { test as base, expect } from './helpers/yiru-app'
import { waitForSessionReady } from './helpers/store'

const fakeGhDir = mkdtempSync(path.join(os.tmpdir(), 'yiru-e2e-fake-gh-'))
...
test('GitHub Tasks drawer recovers when gh stalls on issue details', async ({
  ...
  const { repoId } = await yiruPage.evaluate((repoPath) => {
  ...
  const detailHeading = yiruPage.getByRole('heading', {
  ...
  await expect(yiruPage.getByText('Unable to load details for this GitHub item.')).toBeVisible({
    timeout: 5_000
  })
