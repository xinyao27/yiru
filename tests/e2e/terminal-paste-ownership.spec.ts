...
async function installClipboardReadTerminalBlurRepro(app: ElectronApplication): Promise<void> {
  await app.evaluate(({ BrowserWindow, ipcMain }) => {
    ...
    const global = globalThis as unknown as {
      __yiruOriginalClipboardReadTextHandler?: ClipboardReadHandler
    }
    ...
    if (!invokeHandlers || !handler || global.__yiruOriginalClipboardReadTextHandler) {
      return
    }
    global.__yiruOriginalClipboardReadTextHandler = handler
    invokeHandlers.set('clipboard:readText', async (event, ...args) => {
      ...
      return global.__yiruOriginalClipboardReadTextHandler!(event, ...args)
    })
  })
}
...
async function restoreClipboardReadTerminalBlurRepro(app: ElectronApplication): Promise<void> {
  await app.evaluate(({ ipcMain }) => {
    ...
    if (invokeHandlers && global.__yiruOriginalClipboardReadTextHandler) {
      invokeHandlers.set('clipboard:readText', global.__yiruOriginalClipboardReadTextHandler)
      delete global.__yiruOriginalClipboardReadTextHandler
    }
  })
}
...
test.describe('terminal paste ownership', () => {
  test('keyboard paste shortcuts send clipboard text to the focused terminal exactly once', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.yiru-paste-ownership-${runId}.mjs`)
    ...
    try {
      ...
      scriptStarted = true
      await waitForTerminalOutput(yiruPage, `PASTE_READY_${runId}`, 10_000)

      for (const [index, chord] of keyboardPasteChords().entries()) {
        const payload = `YIRU_E2E_PASTE_${runId}_${index}`
        ...
        await yiruPage.keyboard.press(chord)
        await waitForTerminalOutput(yiruPage, encodedPayload, 10_000, 12_000)
  ...
  test('keyboard paste survives transient terminal blur during clipboard read', async ({
    ...
    const ptyId = await waitForActivePanePtyId(yiruPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.yiru-paste-blur-${runId}.mjs`)
    writeFileSync(scriptPath, pasteEchoScript(runId))
    let scriptStarted = false
    ...
    try {
      ...
      const payload = `YIRU_E2E_TRANSIENT_BLUR_PASTE_${runId}`
      const encodedPayload = Buffer.from(payload, 'utf8').toString('base64')
      await clearTerminalPtyWriteLog(electronApp)
      await yiruPage.evaluate((text) => window.api.ui.writeClipboardText(text), payload)
      await focusActiveTerminalInput(yiruPage)
      ...
      await yiruPage.keyboard.press(keyboardPasteChords()[0])
      await waitForTerminalOutput(yiruPage, encodedPayload, 10_000, 12_000)
  ...
  test('terminal context-menu Paste sends clipboard text exactly once', async ({
    ...
    const ptyId = await waitForActivePanePtyId(yiruPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.yiru-paste-context-menu-${runId}.mjs`)
    writeFileSync(scriptPath, pasteEchoScript(runId))
    let scriptStarted = false
    ...
    try {
      ...
      const payload = `YIRU_E2E_CONTEXT_MENU_PASTE_${runId}`
      const encodedPayload = Buffer.from(payload, 'utf8').toString('base64')
      await clearTerminalPtyWriteLog(electronApp)
      await yiruPage.evaluate((text) => window.api.ui.writeClipboardText(text), payload)
      await focusActiveTerminalInput(yiruPage)
      ...
      await yiruPage.getByRole('menuitem', { name: /Paste/ }).click()
      await waitForTerminalOutput(yiruPage, encodedPayload, 10_000, 12_000)
  ...
  test('Windows multiline keyboard paste normalizes terminal newlines with one PTY owner', async ({
    ...
    const ptyId = await waitForActivePanePtyId(yiruPage)
    const runId = randomUUID()
    const sentinel = `YIRU_E2E_MULTILINE_DONE_${runId}`
    const payload = [
      `YIRU_E2E_MULTILINE_${runId}`,
    ...
    // Yiru's direct Windows bracketed-paste path must produce the same bytes.
    const terminalText = payload.replace(/\r?\n/g, '\r')
    const scriptPath = path.join(testRepoPath, `.yiru-paste-multiline-${runId}.mjs`)
    ...
    try {
      ...
      await waitForTerminalOutput(yiruPage, `PASTE_COMPLETE_${runId}:MATCH`, 10_000, 12_000)
  ...
  test('right-click paste sends clipboard text to the focused terminal exactly once', async ({
    ...
    const ptyId = await waitForActivePanePtyId(yiruPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.yiru-paste-right-click-${runId}.mjs`)
    writeFileSync(scriptPath, pasteEchoScript(runId))
    let scriptStarted = false
    ...
    try {
      ...
      const payload = `YIRU_E2E_RIGHT_CLICK_PASTE_${runId}`
      const encodedPayload = Buffer.from(payload, 'utf8').toString('base64')
      await clearTerminalPtyWriteLog(electronApp)
      await yiruPage.evaluate((text) => window.api.ui.writeClipboardText(text), payload)
      await focusActiveTerminalInput(yiruPage)

      await rightClickActiveTerminalSurface(yiruPage)
      await waitForTerminalOutput(yiruPage, encodedPayload, 10_000, 12_000)
