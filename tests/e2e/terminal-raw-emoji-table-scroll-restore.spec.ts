...
test.describe('Terminal raw emoji table scroll restore repro', () => {
  ...
  test('uses WebGL by default for ordinary terminal output when available @terminal-rendering-golden', async ({
    ...
    const marker = `YIRU_AUTO_WEBGL_SMOKE_${randomUUID()}`

    await sendToTerminal(yiruPage, ptyId, `printf ${JSON.stringify(`${marker}\\n`)}\r`)
    await waitForTerminalOutput(yiruPage, marker, 10_000)

    const expectedWebgl = await expectAutoWebgl(yiruPage)
    ...
    let diagnostics = await readTerminalRenderDiagnostics(yiruPage)
    await expect
      .poll(
        async () => {
          diagnostics = await readTerminalRenderDiagnostics(yiruPage)
  ...
  test('keeps raw emoji box table aligned after restore and scroll @terminal-rendering-golden', async ({
    ...
    await ensureTerminalVisible(yiruPage)
    await waitForActiveTerminalManager(yiruPage, 30_000)
    await setWideRenderedTableViewport(yiruPage)
    await waitForActiveTerminalColumns(yiruPage, RAW_EMOJI_BOX_TABLE_WIDTH)
    const ptyId = await waitForActivePanePtyId(yiruPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.yiru-raw-emoji-fixture-table-${runId}.mjs`)
    ...
    try {
      ...
      await waitForTerminalOutput(yiruPage, completionMarker, 20_000, 30_000)
      await expect
      ...
      await scrollActiveTerminalToText(yiruPage, 'Singer')
      await closeFeatureTips(yiruPage)
      const expectedWebgl = await expectAutoWebgl(yiruPage)
      ...
      const singerGeometry = await readVisibleSingerRowGeometry(yiruPage)
