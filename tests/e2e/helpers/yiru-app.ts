/**
 * Shared Electron fixture for Yiru E2E tests.
 *
...
type YiruTestFixtures = {
  ...
  yiruAppExtraEnv: Record<string, string>
  ...
  // developer's shell or already-running Yiru instance.
  launchEnv: NodeJS.ProcessEnv
}

type YiruWorkerFixtures = {
  /** Absolute path to the test git repo created by globalSetup. */
  testRepoPath: string
}

// Why: parse + warn at module scope so a bad YIRU_E2E_SLOWMO_MS value logs once
// per worker instead of once per test (otherwise hundreds of lines per CI run).
const YIRU_E2E_SLOWMO_MS_RAW = process.env.YIRU_E2E_SLOWMO_MS
const YIRU_E2E_SLOWMO_MS = ((): number => {
  if (YIRU_E2E_SLOWMO_MS_RAW === undefined) {
    return 0
  }
  const parsed = Number(YIRU_E2E_SLOWMO_MS_RAW)
  if (!Number.isFinite(parsed)) {
    console.warn(
      `[yiru-e2e] YIRU_E2E_SLOWMO_MS="${YIRU_E2E_SLOWMO_MS_RAW}" is not a number; ignoring (using 0).`
    )
    return 0
  }
...
function shouldLaunchHeadful(testInfo: TestInfo): boolean {
  // Why: YIRU_E2E_FORCE_HEADFUL lets a developer watch any spec in a real
  // window without retagging it `@headful` or switching projects.
  if (process.env.YIRU_E2E_FORCE_HEADFUL === '1') {
    return true
  }
  return testInfo.project.metadata.yiruHeadful === true
}

function forwardElectronProcessLogs(app: ElectronApplication, testInfo: TestInfo): void {
  if (process.env.YIRU_E2E_FORWARD_APP_LOGS !== '1') {
    return
  }
...
/**
 * Extended Playwright test with Yiru-specific fixtures.
 *
 * `yiruPage` — the main Yiru renderer window.
 *
 ...
 */
export const test = base.extend<YiruTestFixtures, YiruWorkerFixtures>({
  ...
  electronApp: async (
    ...
    const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'yiru-e2e-userdata-'))
    ...
    // Yiru's own agent runtime) set it so Electron behaves as a plain Node
    ...
    void _unused
    // Why: YIRU_E2E_SLOWMO_MS adds a pause between every Playwright action so a
    // developer running with YIRU_E2E_FORCE_HEADFUL=1 can actually watch what
    // the test does. Defaults to 0 (no slowdown) for normal runs.
    const slowMo = YIRU_E2E_SLOWMO_MS
    // Why: YIRU_E2E_RECORD_VIDEO=1 captures a webm of the renderer so a
    ...
    const recordVideoDir = process.env.YIRU_E2E_RECORD_VIDEO === '1' ? testInfo.outputDir : null
    if (recordVideoDir) {
      mkdirSync(recordVideoDir, { recursive: true })
    }
    const app = await electron.launch({
      args: [...yiruAppExtraArgs, ...getYiruElectronLaunchArgs(mainPath, headful)],
      ...(slowMo > 0 ? { slowMo } : {}),
      ...(recordVideoDir ? { recordVideo: { dir: recordVideoDir } } : {}),
      // Why: keep NODE_ENV=development so window.__store is exposed and
      // dev-only helpers activate. YIRU_E2E_USER_DATA_DIR overrides the usual
      // shared dev profile so every spec gets a clean persistence root.
      // Why: YIRU_E2E_HEADLESS suppresses mainWindow.show() so the app
      // window stays hidden during test runs, avoiding focus stealing and
      // screen clutter. Playwright interacts via CDP regardless.
      // Why: YIRU_E2E_HEADLESS suppresses mainWindow.show() for CI/headless
      // runs. YIRU_E2E_HEADFUL overrides this for tests that need a visible
      ...
      env: {
        ...
        ...((process.env.YIRU_E2E_SSH_LOCALHOST === '1' ||
          process.env.YIRU_E2E_SSH_DOCKER === '1') &&
        !cleanEnv.YIRU_RELAY_PATH
          ? { YIRU_RELAY_PATH: path.join(process.cwd(), 'out', 'relay') }
          : {}),
        ...(headful ? { YIRU_E2E_HEADFUL: '1' } : { YIRU_E2E_HEADLESS: '1' }),
        ...yiruAppExtraEnv
      }
    })
  ...
  yiruAppExtraEnv: [{}, { option: true }],
  yiruAppExtraArgs: [[], { option: true }],
  ...
  yiruPage: async ({ sharedPage }, provideFixture) => {
