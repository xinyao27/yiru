 ...
 *
 * Why: the default `yiruPage` fixture creates a fresh `userDataDir` per test
...
export function createRestartSession(testInfo: TestInfo): RestartSession {
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'yiru-e2e-restart-'))
  ...
  const launch = async (): Promise<LaunchedYiru> => {
    const app = await electron.launch({
      args: getYiruElectronLaunchArgs(mainPath, headful),
    ...
    .poll(
      () =>
        page.evaluate(async (repoId) => {
          ...
          // worktree is external to Yiru's workspace root after the visibility rollout.
