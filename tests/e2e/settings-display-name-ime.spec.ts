...
test.describe('Repository Display Name IME composition', () => {
  test('keeps Hangul syllables composed while typing slowly', async ({ yiruPage }) => {
    await waitForSessionReady(yiruPage)

    const repos = await getStoreState<Repo[]>(yiruPage, 'repos')
    expect(repos.length).toBeGreaterThan(0)
    const repo = repos[0]

    await openRepoSettings(yiruPage, repo.id)

    const repoSection = yiruPage.locator(`[data-settings-section="repo-${repo.id}"]`)
    ...
    await typeHangulGanadaSlowly(session, yiruPage, displayNameInput)
    ...
    await expect
      .poll(
        async () => {
          const current = await getStoreState<Repo[]>(yiruPage, 'repos')
