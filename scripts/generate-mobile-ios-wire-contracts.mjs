...
import {
  loadSessionTabsWireSource,
  renderSessionTabsWireContract
} from './mobile-ios-wire/session-tabs-wire.mjs'
import {
...
const sessionTabsWire = loadSessionTabsWireSource(packageRequire, z)
...
const sessionTabsContract = sessionTabsWire.contract
...
function renderWireContract(pairing, e2ee, runtime, terminal, sessionTabs, sourceSchemas, digest) {
  ...
  const terminalSource = renderTerminalWireContract(terminal, sourceSchemas)
  const sessionTabsSource = renderSessionTabsWireContract(sessionTabs)
