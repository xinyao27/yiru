...
import { isLegacyRepoForExternalWorktreeVisibility } from '@yiru/runtime-protocol/workbench/workspace/worktree-ownership'
import { removeRepoFromWorkspaceSessionsForHost } from '~main/persisted-state/workspace-session-owner-removal'

import { mergeProjectHostSetupCompatibilityState } from '../compatibility'
import { sanitizeRepoUpdatesForPersistence } from '../repo-sanitization'
import { PersistenceSlice } from '../slice'

export class RepoUpdateSlice extends PersistenceSlice {
  ...
  protected updateRepoBackedProjectHostSetup(
    ...
    const repoUpdates: Parameters<RepoUpdateSlice['updateRepo']>[1] = {}
    ...
    const updatedRepo =
      Object.keys(repoUpdates).length > 0
        ? this.callStore<Repo | null>('updateRepo', repo.id, repoUpdates)
        : repo
    if (!updatedRepo) {
      return null
    }
