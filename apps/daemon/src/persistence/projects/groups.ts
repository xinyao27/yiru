...
} from '@yiru/runtime-protocol/workbench/project-groups'
import type {
...
type RepoBackedProjectHostSetupUpdate = {
  setup: ProjectHostSetup
  repo: Repo
} | null

export class ProjectGroupSlice extends PersistenceSlice {
  updateProjectHostSetup(args: ProjectHostSetupUpdateArgs): ProjectHostSetupUpdateResult | null {
    ...
    const repo = setup.repoId
      ...
      : undefined
    if (repo) {
      const updated = this.callStore<RepoBackedProjectHostSetupUpdate>(
    ...
    const updatedSetup = this.callStore<ProjectHostSetup>(
  ...
  deleteProjectHostSetup(args: ProjectHostSetupDeleteArgs): ProjectHostSetupDeleteResult | null {
    ...
    const repo = setup.repoId
      ...
      : undefined
    if (repo) {
      this.callStore<void>('removeProject', repo.id)
      return { project, setup, repo: this.callStore<Repo>('hydrateRepo', repo) }
    }
  ...
  getRepo(id: string): Repo | undefined {
    ...
    return repo ? this.callStore<Repo>('hydrateRepo', repo) : undefined
  }
  ...
  deleteProjectGroup(groupId: string): boolean {
    ...
    for (const workspace of this.state.folderWorkspaces ?? []) {
      if (deletedGroupIds.has(workspace.projectGroupId)) {
        ...
        this.callStore<void>('removeWorkspaceLineageForFolderParent', workspace.id)
      }
    }
