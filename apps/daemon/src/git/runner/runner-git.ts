...
export function gitExecFileSync(
  ...
  try {
    const result = Bun.spawnSync([resolved.binary, ...resolved.args], {
      cwd: resolved.cwd,
      env: untranslatedGitOutputEnv(),
    ...
    const stdout = Buffer.from(result.stdout ?? []).toString(options.encoding ?? 'utf-8')
    const stderr = Buffer.from(result.stderr ?? []).toString(options.encoding ?? 'utf-8')
    if (result.exitCode !== 0) {
      throw Object.assign(new Error(`git exited with code ${result.exitCode}: ${stderr}`), {
