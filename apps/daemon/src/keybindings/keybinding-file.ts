...
function parseBindingSection(
  ...
  for (const [storedActionId, rawBinding] of Object.entries(value)) {
    if (options.skipRootKeys && ROOT_KEYS.has(storedActionId)) {
      continue
    }
    ...
    if (!actionId) {
      diagnostics.push({
        ...
        message: `Unknown keybinding action "${storedActionId}" was ignored.`
      })
      continue
    }
