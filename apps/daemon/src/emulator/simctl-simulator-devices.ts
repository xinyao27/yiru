...
function mapSimctlError(error: unknown): EmulatorError {
...
export async function listSimulatorDevices(): Promise<SimulatorDevice[]> {
  ...
  try {
    const { stdout } = await captureSubprocess('xcrun', ['simctl', 'list', 'devices', '-j'], {
      timeoutMs: 15_000
    })
    return parseSimctlDevices(stdout.toString())
  } catch (error) {
    throw mapSimctlError(error)
  }
}
...
export async function ensureSimulatorBooted(udid: string): Promise<void> {
  ...
  try {
    await captureSubprocess('xcrun', ['simctl', 'boot', udid], { timeoutMs: 45_000 })
  } catch {
...
export async function shutdownSimulatorDevice(udid: string): Promise<void> {
  ...
  try {
    await captureSubprocess('xcrun', ['simctl', 'shutdown', udid], { timeoutMs: 30_000 })
  } catch (error) {
    // Why: only the actual already-off state is idempotent; other current
    // states are real failures.
    const message =
