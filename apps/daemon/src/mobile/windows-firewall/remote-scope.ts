...
export function hasSufficientWindowsFirewallRemoteScope(
  ...
  // Why: coverage is checked per scope, never unioned across rules; accepting fragmented
  // rules would turn an advisory check into a false assurance.
...
function addressScopeIsSufficient(scope: string, localSubnet: IpRange | null): boolean {
  ...
  // Why: without a phone IP, only a rule spanning the whole selected subnet proves access.
  // A single-host VPN subnet is just the daemon host and must fail closed.
...
// Why: Windows accepts dotted-netmask CIDR; convert only contiguous masks and reject holes.
