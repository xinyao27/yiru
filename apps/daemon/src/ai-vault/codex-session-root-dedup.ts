...
function codexPathExecutionNamespace(filePath: string): ExecutionHostId {
  const wslPath = parseWslUncPath(filePath)
  return wslPath ? toWslExecutionHostId(wslPath.distro.toLowerCase()) : LOCAL_EXECUTION_HOST_ID
}
 ...
 * Yiru managed runtime home and each per-account self-contained home
