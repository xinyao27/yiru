...
export type BellDetector = {
  chunkContainsBell(data: string, scanFlags?: Partial<TerminalChunkScanFlags>): boolean
  reset(): void
}
...
export function createBellDetector(): BellDetector {
  ...
  return {
    chunkContainsBell(data: string, scanFlags: Partial<TerminalChunkScanFlags> = {}): boolean {
      if (!inOsc && !pendingEscape && !(scanFlags.hasBel ?? data.includes('\x07'))) {
        ...
        if (!(scanFlags.hasOscIntroducer ?? data.includes('\x1b]'))) {
          pendingEscape = (scanFlags.hasEsc ?? true) && data.endsWith('\x1b')
          return false
        }
      }
