...
import { scanMode2031Sequences } from '@yiru/runtime-protocol/workbench/terminal/color-scheme-protocol'
import {
  createTerminalGitHubPRLinkDetector,
  type TerminalGitHubPRLink
} from '@yiru/runtime-protocol/workbench/terminal/github-pr-link-detector'
import { createOsc133CommandFinishedScanner } from '@yiru/runtime-protocol/workbench/terminal/osc133-command-finished'
...
export type TerminalTitleTracker = {
  ...
  handleChunk: (
    data: string,
    options?: { titleScanData?: string; scanFlags?: TerminalChunkScanFlags }
  ) => void
  /**
...
export function createTerminalTitleTracker(
  ...
  function handleChunk(
    ...
    const containsOscIntroducer = options.scanFlags?.hasOscIntroducer ?? data.includes('\x1b]')
    ...
    const containsBell =
      bellDetector && !transientFactScanningSuppressed
        ? bellDetector.chunkContainsBell(
            data,
            options.scanFlags ?? { hasOscIntroducer: containsOscIntroducer }
          )
        : false
    ...
    const titles = containsOscIntroducer ? extractAllOscTitles(titleScanData) : []
    if (titles.length > 0) {
