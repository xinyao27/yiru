...
import { HERMES_PLUGIN_NAME } from './hook-plugin-content'

export type HermesConfig = Record<string, unknown>

export type HermesConfigParseResult =
  | { ok: true; config: HermesConfig }
...
export function parseHermesConfig(content: string | null): HermesConfigParseResult {
...
export function enableHermesPlugin(config: HermesConfig): HermesConfig {
  ...
  if (disabled === null) {
    ...
    plugins.disabled = disabled.filter((name) => name !== HERMES_PLUGIN_NAME)
  }
  next.plugins = plugins
  return next
}

export function disableHermesPlugin(config: HermesConfig): HermesConfig {
  const next: HermesConfig = { ...config }
...
export function updateHermesConfigContent(
  content: string | null,
...
export function getHermesPluginEnablement(config: HermesConfig): {
  enabled: boolean
  disabled: boolean
  detail: string | null
} {
