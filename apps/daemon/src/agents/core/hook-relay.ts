...
// the relay sends to Yiru. Consumed by `src/relay/agent-hook-server.ts` (which
...
//
// Per the design doc:
// - The relay normalizes; Yiru routes. The envelope's `payload` field has
...
/** Wire envelope for a single hook event flowing relay → Yiru. */
export type AgentHookRelayEnvelope = {
  ...
  tabId?: string
  worktreeId?: string
  /** Always `null` on the wire — relay does not know Yiru's local connectionId. */
  connectionId: null
  /** Preserved from the relay-side normalized hook event so Yiru can
  ...
  env?: string
  /** Forwarded verbatim from the agent CLI POST body. Lets Yiru's warn-once
   ...
   *  Yiru's `ingestRemote` validates it again at the SSH trust boundary. */
...
/** JSON-RPC request method Yiru issues after `--connect` reattach to ask the
...
export function isRemoteAgentHooksEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = env[YIRU_FEATURE_REMOTE_AGENT_HOOKS_ENV]
  if (raw === undefined) {
    return true
  }
...
import type { AgentHookSource } from '@yiru/runtime-protocol/model/agent'
