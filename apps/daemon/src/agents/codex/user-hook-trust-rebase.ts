import type { HookCommandConfig, HookDefinition } from '../hooks/managed-hook-commands'
import {
  codexAppServerCapabilityCache,
  getCodexAppServerHostKey,
  type CodexAppServerHostKey
} from './app-server-capability-cache'
import { runCodexUserHookTrustRebaseSessionSync } from './app-server-grant-bridge'
import { isCodexAppServerUnsupportedError } from './app-server-session'
import { computeTrustKey, type CodexTrustEntry } from './config-toml-trust'
import { createCodexHookTrustEntry } from './hook-identity'
import { CODEX_TRUST_GRANT_TRANSIENT_RETRY_INTERVAL_MS } from './hook-trust-grant'
import {
  captureCodexTrustConfig,
  restoreCodexTrustConfig,
  type CodexTrustConfigSnapshot
} from './trust-config-rollback'
import { resolveCodexTrustGrantHost, type CodexTrustGrantHost } from './trust-grant-host'
import type {
...
export function mutateRealHomeHooksPreservingUserTrust(args: {
  ...
  const nativeHost: CodexTrustGrantHost = { id: LOCAL_EXECUTION_HOST_ID, kind: 'native' }
  const hostKey = getCodexAppServerHostKey(nativeHost)
  ...
  const baseRequest = resolveCodexTrustGrantHost(nativeHost).buildRequest({
