...
// in-place codex upgrade during a long Yiru session self-heals after the
...
export type CodexAppServerHostKey = ExecutionHostId

export function getCodexAppServerHostKey(host: Pick<Host, 'id'>): CodexAppServerHostKey {
  return host.id
}
...
import type { Host } from '~main/hosts/contract'
