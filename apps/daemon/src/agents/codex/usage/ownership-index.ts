...
import SyncDatabase, { type SqliteStatement } from '~main/sqlite/sync-database'

const EVENT_HASH_BYTES = 16
const GENERATION_KEY = 'generation'

type OwnershipStatements = {
  deleteEventsForFile: SqliteStatement
  deleteFile: SqliteStatement
  findFile: SqliteStatement
  findOwner: SqliteStatement
  getGeneration: SqliteStatement
  insertEvent: SqliteStatement
  insertFile: SqliteStatement
  setGeneration: SqliteStatement
}
...
export class CodexUsageOwnershipIndex {
  ...
  constructor(databasePath: string) {
    ...
    this.database = new SyncDatabase(databasePath, { timeout: 5_000 })
    this.database.exec(`
  ...
  claimEvent(fileId: number, eventKey: string): boolean {
    const eventHash = new Bun.CryptoHasher('sha256')
      .update(eventKey)
      .digest()
      .subarray(0, EVENT_HASH_BYTES)
