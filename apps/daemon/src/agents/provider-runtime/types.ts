...
import type { WorkspaceSpaceDirectoryScanResult } from '@yiru/runtime-protocol/workbench/workspace/space-types'
...
export type PtySpawnOptions = {
  ...
  /** Yiru worktree identity. When present, the local provider scopes shell
...
export type PtyProcessInfo = {
  ...
  worktreeId?: string
  /** Trusted YIRU_TERMINAL_HANDLE exported into this PTY, when known. */
  terminalHandle?: string
}

export type IPtyProvider = {
  spawn(opts: PtySpawnOptions): Promise<PtySpawnResult>
  mintAvailablePtySessionId?: (worktreeId?: string) => Promise<string>
  ...
  onData(
    callback: (payload: { id: string; chunk: TerminalChunk; sequenceChars?: number }) => void
  ): () => void
