...
import { checkIgnoredPaths } from '../git/repo/check-ignored-paths'
import type { GitRuntimeOptions } from '../git/runner/runtime-options'
import { loadHooks } from '../hooks/config'

// Why: a fresh worktree has no node_modules/.cache, and copying them is slow and
// duplicates disk; `yiru.yaml` names the ones every worktree should share instead.
...
/** Every path Yiru may have symlinked into a worktree: the per-user Worktree
 *  Shared Paths setting plus the repo's `yiru.yaml` shared directories.
 *
...
/** Resolve `worktree.sharedDirectories` from the repo-root `yiru.yaml` to
