...
import { getWslHome } from '~main/hosts/capabilities'

import { parseWslPath } from '../platform/wsl'
...
export { mergeWorktree } from './metadata-merge'
export { areWorktreePathsEqual } from './path-comparison'
 ...
 * Windows workspace layout inside ~/yiru/workspaces on the WSL filesystem
 * (e.g. \\wsl.localhost\Ubuntu\home\user\yiru\workspaces\repo\feature).
 */
...
export function computeWorkspaceRoot(repoPath: string, settings: { workspaceDir: string }): string {
  ...
  if (wsl && shouldMirrorWorkspaceDirInsideWsl(repoPath, settings.workspaceDir)) {
    ...
    if (wslHome) {
      ...
      return win32.join(wslHome, 'yiru', 'workspaces')
    }
  }
