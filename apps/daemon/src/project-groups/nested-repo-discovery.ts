...
import { isGitRepo } from '../git/repo/repo'
import {
  isNestedRepoPathIgnored,
  readNestedRepoIgnoreRules,
  type NestedRepoDirectoryEntry,
  type NestedRepoIgnoreRule,
  type NestedRepoScanFilesystem
} from './nested-repo-ignore-rules'

type TraversalFolder = {
  path: string
  depth: number
  segments: string[]
  ignoreRules: NestedRepoIgnoreRule[]
}
...
function normalizeScanOptions(options: unknown): NormalizedNestedRepoScanOptions {
  const raw: NestedRepoScanOptions = options && typeof options === 'object' ? options : {}
  return {
    maxDepth:
...
export async function scanNestedRepos(args: {
  ...
  while (nextFolderIndex < foldersToTraverse.length) {
    ...
    const currentIgnoreRules = [
      ...
      ...(await readNestedRepoIgnoreRules({
    ...
    for (const entry of dirs) {
      ...
      if (isNestedRepoPathIgnored(name, childSegments, currentIgnoreRules)) {
        continue
      }
