...
import type { PlatformScriptPlatform } from './platform-script-provider-paths'
    ...
    const timeout = setTimeout(() => {
      ...
      finish(
        new RuntimeClientError(
          'action_timeout',
          `platform script provider timed out after ${REQUEST_TIMEOUT_MS}ms`
        ),
        undefined,
...
export function mapBridgeError(message: string): RuntimeClientError {
  const text = message.trim() || 'platform script provider failed'
