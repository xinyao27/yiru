import type { ComputerSnapshotResult } from '@yiru/runtime-protocol/workbench/runtime-types'

import type { BridgeResponse, BridgeSnapshot, BridgeWindow } from './platform-script-provider-types'
...
function desktopScreenshotFailureStatus(
  ...
  return {
    ...
    code: 'screenshot_failed',
    message:
      'platform script provider returned no image; grant screen capture permission or pass --no-screenshot to inspect accessibility state only.'
  }
}
...
export function normalizeBridgeApp(app: BridgeResponse['app'] | BridgeWindow['app'] | undefined) {
  if (!app) {
    throw new RuntimeClientError('accessibility_error', 'platform script provider returned no app')
  }
  return {
    name: app.name,
