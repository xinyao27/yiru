...
export type NodeRuntimeHostTerminalManagementAdapter = {
  disconnectOnly?: () => Promise<void>
  fanoutSyntheticExits?: (code: number) => void
  listSessions: () => Promise<
    Omit<TerminalManagementListResult['sessions'][number], 'protocolVersion'>[]
  >
  protocolVersion: number
  shutdown: (sessionId: string, options: { immediate?: boolean }) => Promise<void>
}

let daemonAdapter: NodeRuntimeHostTerminalManagementAdapter | null = null
let restartDaemon: (() => Promise<void>) | null = null

export function setNodeRuntimeHostTerminalManagementAdapter(
  adapter: NodeRuntimeHostTerminalManagementAdapter | null,
...
function requireDaemonAdapter(): NodeRuntimeHostTerminalManagementAdapter {
  if (!daemonAdapter) {
