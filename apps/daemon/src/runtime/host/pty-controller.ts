...
import type { IPtyProvider, PtySpawnOptions } from '~main/agents/provider-runtime/types'
...
export function attachNodeRuntimeHostPtyController(
  ...
  const removeDataListener = provider.onData((payload) => {
    runtime.onPtyData(
      ...
      payload.chunk,
      Date.now(),
      payload.sequenceChars ?? payload.chunk.text.length
    )
  })
  const removeReplayListener = provider.onReplay((payload) => {
    runtime.onPtyData(
      payload.id,
      { text: payload.data, bytes: TERMINAL_TEXT_ENCODER.encode(payload.data) },
      Date.now()
    )
  })
