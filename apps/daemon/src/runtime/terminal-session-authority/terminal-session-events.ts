...
export class TerminalSessionEvents {
  private readonly dataListeners = new Map<
    string,
    Set<(chunk: TerminalChunk, meta?: TerminalDataMeta) => void>
  >()
  ...
  private readonly port: TerminalSessionEventPort

  constructor(port: TerminalSessionEventPort) {
    this.port = port
  }

  subscribeData(
    ptyId: string,
    listener: (chunk: TerminalChunk, meta?: TerminalDataMeta) => void
  ): () => void {
  ...
  emitData(ptyId: string, chunk: TerminalChunk, meta?: TerminalDataMeta): void {
