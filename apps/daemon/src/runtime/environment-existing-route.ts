...
import { getPreferredPairingOffer } from '@yiru/runtime-protocol/workbench/runtime-environments'
import type { RemoteRuntimeSubscription } from '~main/remote-runtime/client'
import { markEnvironmentUsed, resolveEnvironment } from '~main/runtime-environment-store'
...
export async function callRuntimeEnvironmentExistingRoute(
  ...
  return enqueueRuntimeCall(environment.id, method, async () => {
    ...
    // Why: this helper only ever forwards unary remote.host.* leaves
    ...
    // `remote.host.*` is the daemon-to-daemon bare-envelope protocol
