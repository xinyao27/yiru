...
import { resolveTranscriptAgent } from '@yiru/runtime-protocol/model/agent'
import type { AgentType, AgentTranscriptMessage } from '@yiru/runtime-protocol/model/agent'
...
export const MAX_AGENT_TRANSCRIPT_RECORD_BYTES = 2 * 1024 * 1024
const TAIL_CHUNK_BYTES = 64 * 1024

export type AgentTranscriptLineDecoder = (
  line: string,
  fallbackId: string
) => AgentTranscriptMessage | null

export function agentTranscriptLineDecoderForAgent(
  agent: AgentType
): AgentTranscriptLineDecoder | null {
...
export async function readAgentTranscriptTailFile(
  filePath: string,
  limit: number,
  ...
  try {
    ...
    const newestFirst: { message: AgentTranscriptMessage; offset: number }[] = []
  ...
  function retainPart(part: Buffer): void {
    ...
    if (lineBytes > MAX_AGENT_TRANSCRIPT_RECORD_BYTES) {
      lineParts.length = 0
      lineOversized = true
  ...
  function decodeLine(
    lineOffset: number,
    messages: { message: AgentTranscriptMessage; offset: number }[]
  ): void {
...
export async function readAgentTranscriptTail(
  ...
  | { messages: AgentTranscriptMessage[]; hasMore: boolean; beforeOffset: number }
...
> {
  ...
  try {
    const result = await readAgentTranscriptTailFile(
      filePath,
      args.limit,
      decode,
      true,
