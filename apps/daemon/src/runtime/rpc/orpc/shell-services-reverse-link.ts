...
// Why: authenticated clients share one registry while retaining transport-scoped ids.
...
export function getConnectedWebShellServicesClient(): ShellServicesClient | null {
  for (const [shellConnectionId, connection] of connectionsByShellId) {
    if (shellConnectionId.startsWith('web:')) {
