...
import { startFederatedWorker } from '../federation/worker-start'
import { assertOrchestrationWorktreeCreationSupported } from '../worktree-placement'
import * as setupGate from './setup-gate'
import { failWorkerStartWithReceipt } from './start-receipt'
import {
...
export async function handleOrchestrationWorkerStart(
  params: orchestrationContract.OrchestrationWorkerStartInput,
  { runtime, orchestrationMutation }: RpcContext
): Promise<orchestrationContract.OrchestrationWorkerStartResult> {
  ...
  try {
    ...
    if (setupGate.persistGatedSetupSpawnFailure(setupStage)) {
      failedStage = 'setup_start'
      throw new Error('Setup terminal failed to start before the gated agent launch.')
    }
    setupGate.persistWorkerReadinessStage(setupStage)
    ...
    setupGate.persistWorkerSetupWaitOutcome({ ...setupStage, wait })
    if (!wait.satisfied) {
