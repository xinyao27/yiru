import type {
  OrchestrationMessageSendInput,
  OrchestrationSendResult
} from '@yiru/runtime-protocol/contract'
import { ORCHESTRATION_FEDERATION_CONTROL_MAIL_PROTOCOL_VERSION } from '@yiru/runtime-protocol/protocol-version'
import { orchestrationSkillRecoveryData } from '~main/orchestration/rpc-contract'
...
export async function handleOrchestrationSend(
  params: OrchestrationMessageSendInput,
  { runtime, orchestrationCapability }: RpcContext
): Promise<OrchestrationSendResult> {
  ...
  if (
    routing.run &&
    (!to || ((params.type === 'worker_done' || params.type === 'heartbeat') && routing.dispatchId))
  ) {
  ...
  if (!isGroupAddress(to)) {
    ...
    if (federatedTarget && federatedDispatchId) {
      ...
      if (
        federatedTarget.protocol_version < ORCHESTRATION_FEDERATION_CONTROL_MAIL_PROTOCOL_VERSION
      ) {
        throw new OrchestrationError(
          'capability_unsupported',
          `Federated Dispatch ${dispatchId} does not support coordinator control mail; start a fresh worker after updating its runtime host.`
        )
      }
    ...
    const dispatch = routing.dispatchId ? db.getDispatchContextById(routing.dispatchId) : undefined
