...
import { orchestrationMigrationData } from '~main/orchestration/rpc-contract'
...
function federatedUnknownReceipt(
  ...
  return {
    ...
    residualResources: [],
    nextCommands: [
      `yiru orchestration worker-show --dispatch ${worker.dispatch_id} --json`,
      `yiru orchestration worker-abandon --dispatch ${worker.dispatch_id} --json`
    ]
  }
}
