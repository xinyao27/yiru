...
import type { RuntimeWorktreeListResult } from '@yiru/runtime-protocol/workbench/runtime-types'
import type {
  DetectedWorktree,
...
import type { ResolvedWorktree, WorktreeLineageResolution } from '../model/worktree-resolution'
...
export abstract class RuntimeContractMergeRepoPR extends RuntimeContractResolveGitLabRepoMRDiscussion {
  ...
  abstract getRepoHooks(repoSelector: string): Promise<RuntimeRepoHooksResult>
  ...
  abstract inspectRepoSetupScriptImports(
    repoSelector: string
  ): Promise<RuntimeSetupScriptImportCandidate[]>
  ...
  abstract showManagedWorktree(worktreeSelector: string): Promise<ResolvedWorktree>
