...
import { deriveValidatedClonePath, getClonePathComparisonKey } from '~main/git/repo/repo-clone-path'
import { gitExecFileAsync } from '~main/git/runner/runner'
import { runWithGitReadCacheInvalidation } from '~main/git/status/status'
...
export const createRepoMethods = {
  async createRepo(
    parentPath: string,
    name: string,
    kind: 'git' | 'folder' = 'git'
