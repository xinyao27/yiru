...
import type { RuntimeRepoSearchRefs } from '@yiru/runtime-protocol/workbench/runtime-types'
...
import { getBaseRefDefault, searchBaseRefDetails, getRemoteCount } from '~main/git/repo/repo'
...
export const updateRepoMethods = {
  async updateRepo(
    repoSelector: string,
    updates: Partial<
      Pick<
  ...
  async resolveHostedReviewTarget(args: {
    repoSelector: string
  ...
  getLocalGitExecutionOptionArgs(repo: Repo): [] | [{ wslDistro?: string }] {
    const localGitOptions = getLocalProjectWorktreeGitOptions(this.store, repo)
    return Object.keys(localGitOptions).length > 0 ? [localGitOptions] : []
  },

  getAgentLaunchPlatformForRepo(repo: Repo): NodeJS.Platform {
    const projectRuntime = resolveLocalProjectRuntimeForRepo(this.store, repo)
