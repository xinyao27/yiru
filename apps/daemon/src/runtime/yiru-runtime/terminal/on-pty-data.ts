...
import { extractOscTitleScanTail } from '~main/osc-title-scan-tail'
import { advertisedUrlWatcher } from '~main/ports/advertised-url-watcher'
import { classifyTerminalChunk } from '~main/terminal-output/chunk-scan-flags'
...
export abstract class RuntimeTerminalOnPtyData extends RuntimeTerminalPreAllocateHandleForPty {
  onPtyData(
    ...
    const wireByteSequence =
      (this.ptyWireByteSequenceById.get(ptyId) ?? 0n) + BigInt(chunk.bytes.byteLength)
    this.ptyWireByteSequenceById.set(ptyId, wireByteSequence)
    this.providerModeTrackersByPtyId.get(ptyId)?.scan(data, scanFlags)
    for (const tracker of this.providerModeSnapshotScansByPtyId.get(ptyId) ?? []) {
      tracker.scan(data, scanFlags)
    }
    const osc7Metadata = this.recordOsc7MetadataForPty(ptyId, data, scanFlags)
    const cwd = osc7Metadata.cwd
    const cwdChanged = osc7Metadata.cwdChanged
    const agentStatusChunk = this.processAgentStatusOscForPty(ptyId, data, scanFlags)
    ...
    this.agentDetector?.onData(ptyId, data, at, scanFlags)
    ...
    try {
      titleTrackerEntry.tracker.handleChunk(agentStatusChunk.cleanData, {
        ...
        scanFlags: previousTitleScanTail
          ? { ...scanFlags, hasEsc: true, hasOscIntroducer: true }
          : scanFlags
      })
    ...
    this.terminalSessions.emitData(ptyId, chunk, {
      seq: outputSequence,
