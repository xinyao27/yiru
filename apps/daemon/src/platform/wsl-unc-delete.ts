...
async function execFileWsl(distro: string, command: string[]): Promise<void> {
  try {
    ...
    await captureSubprocess('wsl.exe', ['-d', distro, '--', ...command], {
      timeoutMs: 30_000
    })
  } catch (error) {
    throw wslDeleteError(error)
  }
}

function wslDeleteError(error: unknown): Error {
  const detail =
