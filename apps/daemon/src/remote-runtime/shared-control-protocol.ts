...
export function getCleanupRequest(
  ...
  if (subscription.method === 'remote.host.subscribeTerminal') {
    return {
      method: 'terminal.unsubscribe',
      params: { subscriptionId: `remote.host.terminal:${subscription.requestId}` }
    }
  }
  if (subscription.method === 'remote.host.subscribeSessionChanges') {
    return {
      method: 'remote.host.unsubscribeSessionChanges',
...
export function formatSharedControlCloseMessage(code: number, reason: string): string {
...
export function sendSharedControlOrpcBinary(args: {
  ...
  args.ws.send(Buffer.from(encryptBytes(plaintext, args.sharedKey)))
  return true
}
