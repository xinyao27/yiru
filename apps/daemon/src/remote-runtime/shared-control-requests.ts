...
import type { SharedControlPendingRequest } from '@yiru/runtime-protocol/workbench/remote-runtime/shared-control-types'

import { remoteRuntimeTimeoutError, remoteRuntimeUnavailableError } from './request-frames'
import { toRemoteRuntimeClientError } from './shared-control-protocol'
...
export function requestSharedControl<TResult>(args: {
  ...
  return new Promise<RuntimeRpcResponse<TResult>>((resolve, reject) => {
    ...
    void args.ensureReady().then(
      async () => {
        try {
          // Why: queued remote-daemon mutations must revalidate after connection setup, at transmission.
