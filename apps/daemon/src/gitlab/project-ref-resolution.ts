import type { ForgeRemotePreference } from '@yiru/runtime-protocol/workbench/types'

import { gitExecFileAsync, glabExecFileAsync } from '../git/runner/runner'
...
export async function getProjectRefForRemote(
  ...
  if (localGitOptions.signal) {
    // Why: cancellation belongs to one remote-daemon read; it must not abort a shared
