...
async function runWindowsPathCommand(args: string[]): Promise<string> {
  ...
  const { stdout } = await captureSubprocess('powershell', args, {
    timeoutMs: WINDOWS_PATH_COMMAND_TIMEOUT_MS
  })
