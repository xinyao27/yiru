    ...
    var body: some View {
        ...
        case .terminal(let host, let terminal):
            TerminalLiveView(
                ...
                openTerminalFile: { request in
                    Task {
                        guard
                            let snapshot = try? await model.dependencies.workspaceRepository
                                .workspaces(for: host.id),
                            let workspace = snapshot.workspaces.first(where: {
                                $0.id == terminal.worktreeID
                            })
                        else { return }
                        model.openTerminalFile(request, host: host, workspace: workspace)
                    }
                }
            )
