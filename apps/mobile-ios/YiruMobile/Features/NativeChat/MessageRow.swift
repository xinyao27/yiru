...
struct NativeChatMessageRow: View {
    ...
    var body: some View {
        ...
        VStack(
            ...
            .padding(.horizontal, message.role == .user ? Theme.Spacing.medium : 0)
            .padding(.vertical, message.role == .user ? Theme.Spacing.small : 0)
    ...
    private func prose(_ block: NativeChatBlock) -> some View {
        ...
        case .text(let text):
            if message.role == .user {
                Text(text)
                    .font(.system(size: Theme.Typography.primary * fontScale, weight: .regular))
                ...
                AppStructuredMarkdown(
                    ...
                    fontSize: Theme.Typography.primary * fontScale
                )
            }
