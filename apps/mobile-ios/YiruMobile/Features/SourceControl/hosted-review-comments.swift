...
struct HostedReviewCommentsCard: View {
    ...
    var body: some View {
        HostedReviewSection(
            ...
            if let comments {
                if !comments.isEmpty {
                    Picker("Comment audience", selection: $audience) {
                        ForEach(HostedReviewCommentAudience.allCases) { value in
                            Text(value.title(counts: audienceCounts(comments))).tag(value)
                        }
                    }
