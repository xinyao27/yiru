...
struct HostedReviewChip: View {
            ...
            Text(state.title)
                ...
                .overlay {
                    RoundedRectangle(cornerRadius: 4)
                        .stroke(state.color, lineWidth: 0.5)
                }
            HStack(spacing: 4) {
                rollupIcon(rollup)
    ...
    private func rollupIcon(_ rollup: HostedReviewChipRollup) -> some View {
        switch rollup {
        case .conflict: YiruIcon(.warning, size: 13)
        case .failing: YiruIcon(.x, size: 13)
        case .running: YiruIcon(.circle, size: 13)
        case .passed: YiruIcon(.check, size: 13)
        case .none: EmptyView()
        }
    }
