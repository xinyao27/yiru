...
struct SourceControlContentView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            Picker("Source control view", selection: $activeTab) {
                Text("Changes").tag(SourceControlHubTab.changes)
                Text(model.hostedReviewEligibility?.provider.reviewTitle ?? "Pull Request")
                    .tag(SourceControlHubTab.pullRequest)
                Text("Commits").tag(SourceControlHubTab.history)
            }
