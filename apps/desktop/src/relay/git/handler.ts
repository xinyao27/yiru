...
import {
  ...
  parseBranchDiff,
  parseWorktreeList
} from './handler-output-parser'
import { resolveRelayPushTarget } from './handler-push-target'
import { detectConflictOperation, getStatusOp } from './handler-status-ops'
import {
...
} from './handler-submodule-ops'
import { annotatePrunableWorktreesByExistence } from './handler-worktree-list'
import {
  ...
  private async pullWithArgs(params: Record<string, unknown>, pullArgs: string[]) {
    ...
    const runPull = async (effectiveArgs: string[]): Promise<void> => {
      ...
      if (upstream && !upstream.isConfiguredUpstream) {
        // Why: legacy Yiru branches may still track origin/main while pushes
