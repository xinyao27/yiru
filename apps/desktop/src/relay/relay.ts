...
// Yiru Relay — lightweight daemon deployed to remote hosts.
...
const EMPTY_DETACHED_STARTUP_GRACE_MS = parseNonNegativeIntEnv(
  'YIRU_RELAY_EMPTY_STARTUP_GRACE_MS',
  60_000
)
...
async function runYiruCliMode(sockPath: string, argv: string[]): Promise<void> {
  const myVersion = readLaunchVersion()
  const stdin = shouldReadRemoteCliStdin(argv) ? await readYiruCliStdin() : undefined
  const sock = createConnection({ path: sockPath })
  let nextSeq = 1
  ...
  const connectTimeout = setTimeout(() => {
    process.stderr.write(`[yiru-cli] Relay connection timed out after ${CONNECT_TIMEOUT_MS}ms\n`)
    sock.destroy()
    process.exit(1)
  }, CONNECT_TIMEOUT_MS)
  ...
  sock.on('error', (err) => {
    clearTimeout(connectTimeout)
    process.stderr.write(`[yiru-cli] Relay socket error: ${err.message}\n`)
    process.exit(1)
  })
}

async function readYiruCliStdin(): Promise<string | undefined> {
...
async function main(): Promise<void> {
  ...
  if (cliMode) {
    const marker = process.argv.indexOf('--yiru-cli')
    await runYiruCliMode(sockPath, marker >= 0 ? process.argv.slice(marker + 1) : [])
    return
  }

  // Why: only the long-lived detached daemon accumulates relay.log; --connect
  // bridges and --yiru-cli are short-lived and already returned above. Route all
  ...
  dispatcher.onRequest('yiru.cli', async (params, context) => {
    return await dispatcher.requestAnyClient('yiru.cli', params, {
  ...
  // host. Each parsed payload is forwarded to Yiru via an `agent.hook`
  ...
  const hookServer = new RelayAgentHookServer({
    ...
    forward: (envelope) => {
      ...
      // replay the last status for each live pane after Yiru re-wires its
  ...
  // already sees populated YIRU_AGENT_HOOK_* env. The bind is a local-loopback
  ...
  // Why: every relay-spawned PTY needs the live YIRU_AGENT_HOOK_* coords. The
  ...
  ptyHandler.addEnvAugmenter((ctx) => {
    ...
    if (pluginOverlay.hasOpenCodeSource()) {
      ...
      if (dir) {
        ...
        env.YIRU_OPENCODE_CONFIG_DIR = dir
        if (sourceDir) {
          env.YIRU_OPENCODE_SOURCE_CONFIG_DIR = sourceDir
        }
      }
    }
    if (pluginOverlay.hasPiSource()) {
      // Why: source-dir defaulting is keyed on which Pi-compatible agent is
      // being launched (Pi vs OMP). Install Yiru's guarded extension into that
      ...
      if (kind === 'pi') {
        ...
        if (dir) {
          env.YIRU_PI_SOURCE_AGENT_DIR = dir
        }
      }
      if (shouldPrepareOmpShadow) {
        ...
        const sourceDir =
          kind === 'omp'
            ...
            : ctx.env.YIRU_OMP_SOURCE_AGENT_DIR
        const dir = pluginOverlay.materializePi(overlayId, sourceDir, 'omp')
        if (dir) {
          env.YIRU_OMP_STATUS_EXTENSION = getRelayPiStatusExtensionPath(dir)
          env.YIRU_OMP_SOURCE_AGENT_DIR = dir
        }
      }
    }
    return env
  ...
  // Why: request-driven replay. Yiru issues this *after* it re-wires the
  ...
  // Why: Yiru ships the OpenCode plugin / Pi extension source bodies over
  ...
  // would force a relay redeploy on every Yiru update). Cache them so each
  ...
  // Why: bound the per-source size so a buggy/hostile Yiru can't OOM the
