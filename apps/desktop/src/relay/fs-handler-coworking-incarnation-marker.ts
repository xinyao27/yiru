...
} from './fs-handler-coworking-path-proof'
...
const MARKER_FILENAMES = new Set([
  'yiru-coworking-incarnation-v1',
  '.yiru-coworking-incarnation-v1'
])

export function registerCoworkingIncarnationMarkerHandler(dispatcher: RelayDispatcher): void {
  dispatcher.onRequest('fs.coworkingInspectDirectoryIdentity', inspectCoworkingDirectoryIdentity)
  dispatcher.onRequest(
    'fs.coworkingReadOrCreateIncarnationMarker',
    readOrCreateCoworkingIncarnationMarker
  )
}

async function inspectCoworkingDirectoryIdentity(
  params: Record<string, unknown>,
  context: RequestContext
): Promise<{ canonicalPath: string; deviceId: string; inodeId: string }> {
  if (!hasExactCoworkingWireKeys(params, ['directoryPath'])) {
    throw new Error('coworking_marker_parameter_invalid')
  }
  relayCoworkingThrowIfAborted(context.signal)
  const canonicalPath = await realpath(relayCoworkingAbsolutePath(params.directoryPath))
...
async function readOrCreateCoworkingIncarnationMarker(
  params: Record<string, unknown>,
  context: RequestContext
): Promise<{ markerId: string }> {
  if (
    !hasExactCoworkingWireKeys(params, ['directoryPath', 'filename', 'proposedMarkerId']) ||
    typeof params.filename !== 'string' ||
    !MARKER_FILENAMES.has(params.filename) ||
    !isCoworkingIncarnationMarkerId(params.proposedMarkerId)
  ) {
    throw new Error('coworking_marker_parameter_invalid')
  }
  relayCoworkingThrowIfAborted(context.signal)
  const directoryPath = await realpath(relayCoworkingAbsolutePath(params.directoryPath))
...
async function readOrCreateMarkerFile(
  ...
  try {
    handle = await openRelayCoworkingExclusiveFile(temporaryPath, 0o600)
    temporaryPresent = true
    signal?.throwIfAborted()
    await writeRelayCoworkingFile(handle, Buffer.from(`${proposedMarkerId}\n`, 'utf8'))
    await handle.sync()
    await handle.close()
    handle = null
    signal?.throwIfAborted()
...
async function readMarkerFile(
  ...
  if (!before.isFile() || before.nlink !== 1n || before.size > MAX_MARKER_BYTES) {
    throw new Error('coworking_marker_unavailable')
  }
  ...
  try {
    ...
    if (!isCoworkingIncarnationMarkerId(markerId)) {
      throw new Error('coworking_marker_unavailable')
    }
...
function requireSameIdentity(left: BigIntStats, right: BigIntStats): void {
  if (left.dev !== right.dev || left.ino !== right.ino) {
    throw new Error('coworking_marker_path_stale')
  }
}
...
async function requireBigIntDirectory(handle: FileHandle): Promise<BigIntStats> {
  ...
  if (!stats.isDirectory() || stats.dev < 0n || stats.ino <= 0n) {
    throw new Error('coworking_marker_directory_invalid')
  }
  return stats
}
