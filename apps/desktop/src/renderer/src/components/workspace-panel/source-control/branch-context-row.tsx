...
export { shouldShowSourceControlBranchContextRow } from './branch-context-stats'
...
function BaseRefButton({
  ...
  return (
    <Button
      ...
      size="xs"
      type="button"
      className="text-foreground/90 decoration-border hover:text-foreground hover:decoration-foreground focus-visible:text-foreground focus-visible:bg-accent h-auto max-w-full min-w-0 justify-start gap-0 truncate border-0 bg-transparent p-0 text-left font-mono text-[10.5px] whitespace-normal underline underline-offset-2"
      onClick={onClick}
      title={`${title} (${baseRef})`}
    >
      {baseRef}
