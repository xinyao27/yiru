    ...
    <div className="relative">
      {!collapsed && (
        <div
          ...
          tabIndex={0}
          className="focus-visible:bg-ring/30 absolute inset-x-0 -top-1 z-10 h-2 cursor-row-resize outline-none"
          onPointerDown={startResize}
          onKeyDown={handleResizeKeyDown}
        />
      )}
      <div className="h-7 pr-3 pl-1">
        <div className="flex h-full items-stretch pr-1">
          <Button
            variant="ghost"
            size="xs"
            type="button"
            className="text-foreground/70 focus-visible:bg-accent flex h-auto min-w-0 flex-1 justify-start border-0 px-0.5 text-left text-[11px] font-semibold tracking-wider whitespace-normal uppercase"
            onClick={onToggle}
          >
            <ChevronDown
              weight="regular"
            ...
            <TooltipTrigger
              render={
                <Button
                  ...
                  variant="ghost"
                  size="icon-xs"
                  className="my-auto size-5 p-0.5 [&_svg]:size-3"
                  aria-label={translate(
