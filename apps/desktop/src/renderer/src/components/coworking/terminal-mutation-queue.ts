...
export type CoworkingTerminalMutationQueue = {
  input(data: string): boolean
  resize(cols: number, rows: number): void
  dispose(): void
}

type CoworkingTerminalMutationQueueOptions = {
  inputFlushMs: number
  invoke(mutation: CoworkingTerminalMutation): Promise<void>
  shouldDiscardAfterError(error: unknown): boolean
  onCapacityExceeded(): void
}

type PendingInputMutation = Extract<CoworkingTerminalMutation, { method: 'terminal.input' }> & {
  bytes: number
}

type PendingMutation =
  | PendingInputMutation
  | Extract<CoworkingTerminalMutation, { method: 'terminal.resize' }>

const MAX_PENDING_INPUT_BYTES = 1024 * 1024
const MAX_PENDING_MUTATIONS = 256

export function createCoworkingTerminalMutationQueue(
  options: CoworkingTerminalMutationQueueOptions
): CoworkingTerminalMutationQueue {
