...
export function useTabGroupWorkspaceModel({
  ...
  const closeEditorIfUnreferenced = useCallback(
    (entityId: string, closingTabId: string) => {
      ...
      if (!otherReference) {
        ...
        if (file?.isDirty) {
          // Why: split-group close actions bypass terminal-workspace.tsx, but the unsaved
  ...
  const leaveWorktreeIfEmpty = useCallback(() => {
    ...
    // Why: split-group close actions bypass the legacy terminal-workspace.tsx handlers
