...
function isConfirmedSignedOut(provider: ProviderRateLimits): boolean {
  ...
  // Why: transient auth refresh and network failures do not prove the agent session is signed out.
  if (provider.usageMetadata?.failureKind) {
    return false
  }
  const error = provider.error
  return error ? CONFIRMED_SIGN_OUT_PATTERNS.some((pattern) => pattern.test(error)) : false
}
