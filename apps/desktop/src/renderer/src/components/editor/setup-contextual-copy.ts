...
export function setupContextualCopy({
  ...
  copyHintNode.className =
    'pointer-events-none rounded-md border border-border/90 bg-background px-2.5 py-1 text-xs font-medium text-foreground whitespace-nowrap'
  ...
  const copyHintWidget: editor.IContentWidget = {
    ...
    getId: () => `yiru.copy-context-hint.${filePath}`,
  ...
  const copySelectionWithContext = async (): Promise<boolean> => {
    ...
    // once the snippet leaves Yiru and is pasted into a terminal.
