...
export function createCompactCommentMarkdownComponents(
  ...
  return {
    ...
    hr: () => <hr className="border-border/50 my-1" />,
    // Compact blockquotes
    blockquote: ({ children }) => (
      <blockquote className="border-border/60 text-muted-foreground/80 my-0.5 border-l-2 pl-2">
        {children}
      </blockquote>
    ),
    ...
    table: ({ children }) => (
      <div className="my-1 max-w-full overflow-x-auto">
        <table className="[&_td]:border-border/40 [&_th]:border-border/40 border-collapse text-[10px] [&_td]:border [&_td]:px-1 [&_td]:py-0.5 [&_th]:border [&_th]:px-1 [&_th]:py-0.5 [&_th]:text-left [&_th]:font-semibold">
          {children}
        </table>
      </div>
    )
...
export function createDocumentCommentMarkdownComponents(
  ...
  return {
    ...
    h1: ({ children }) => (
      <h1 className="mt-4 mb-2 text-[18px] leading-tight font-semibold first:mt-0">{children}</h1>
    ),
    h2: ({ children }) => (
      <h2 className="mt-4 mb-2 text-[16px] leading-tight font-semibold first:mt-0">{children}</h2>
    ),
    h3: ({ children }) => (
      <h3 className="mt-3 mb-2 text-[15px] leading-tight font-semibold first:mt-0">{children}</h3>
    ),
    h4: ({ children }) => <h4 className="mt-3 mb-1 font-semibold first:mt-0">{children}</h4>,
    h5: ({ children }) => <h5 className="mt-3 mb-1 font-semibold first:mt-0">{children}</h5>,
    h6: ({ children }) => <h6 className="mt-3 mb-1 font-semibold first:mt-0">{children}</h6>,
    hr: () => <hr className="border-border/60 my-4" />,
    blockquote: ({ children }) => (
      <blockquote className="border-border/70 text-muted-foreground my-3 border-l-2 pl-3">
        {children}
      </blockquote>
    ),
    img: ({ alt, src }) => {
