...
import BrowserFind from './browser-find'
import {
  consumeBrowserFocusRequest,
  YIRU_BROWSER_FOCUS_REQUEST_EVENT,
  type BrowserFocusRequestDetail
} from './browser-focus'
import { BrowserImportHintButton } from './browser-import-hint-button'
...
function toDisplayUrl(url: string): string {
  return url === YIRU_BROWSER_BLANK_URL ? 'about:blank' : redactKagiSessionToken(url)
}

function getBrowserDisplayTitle(title: string | null | undefined, url: string): string {
  if (
    url === 'about:blank' ||
    url === YIRU_BROWSER_BLANK_URL ||
    title === 'about:blank' ||
    title === YIRU_BROWSER_BLANK_URL ||
    !title
  ) {
    return 'New Tab'
  }
...
function RemoteBrowserPagePane({
  ...
  useEffect(() => {
    ...
    window.addEventListener(YIRU_BROWSER_FOCUS_REQUEST_EVENT, handleBrowserFocusRequest)
    return () =>
      window.removeEventListener(YIRU_BROWSER_FOCUS_REQUEST_EVENT, handleBrowserFocusRequest)
  ...
  const showRemoteFailureOverlay =
    ...
    remoteFailureUrl !== YIRU_BROWSER_BLANK_URL
...
function BrowserPagePane({
  ...
  const activeYiruProfileId = useAppStore((s) => s.activeYiruProfileId)
  const fallbackBrowserPartition = activeYiruProfileId
    ? getYiruProfileBrowserDefaultPartition(activeYiruProfileId)
    : null
    ...
    (webview: Electron.WebviewTag): void => {
      try {
        onUpdatePageStateRef.current(browserTab.id, {
          ...
          // Only explicit navigation/load events should drive Yiru's loading UI.
          canGoBack: webview.canGoBack(),
          canGoForward: webview.canGoForward()
        })
      } catch {
        // Why: Electron only exposes these getters after the guest fully
        // attaches. Ignoring the transient failure avoids crashing Yiru while
  ...
  useEffect(() => {
    ...
    const handleDidStopLoading = (): void => {
      ...
      if (activeLoadFailure) {
        ...
        if (normalizedAttemptedUrl === normalizedCurrentUrl) {
          ...
          // navigation falls back to a blank Chromium surface even though Yiru
      ...
      if (keepAddressBarFocusRef.current && currentUrl === YIRU_BROWSER_BLANK_URL) {
        focusAddressBarNow()
      } else {
        keepAddressBarFocusRef.current = false
      }
    ...
    const handleFailLoad = (event: {
      ...
      if (event.errorCode === -3) {
        ...
        // when the replacement navigation succeeds. Ignore that noise so Yiru
  ...
  useEffect(() => {
    ...
    if (
      ...
      // Why: browserTab.url changes are Yiru-driven navigations (address bar,
  ...
  useEffect(() => {
    ...
    // Polling only while the active tab is "loading" gives Yiru a last-resort
  ...
  const isBlankTab = browserTab.url === 'about:blank' || browserTab.url === YIRU_BROWSER_BLANK_URL
    ...
    >
      ...
      <div ref={chromeHeaderRef} className="pointer-events-auto shrink-0">
        ...
        {visibleDownloads.length > 0 ? (
          <div className="border-border/60 bg-background border-b px-3 py-1.5">
              ...
              {visibleDownloads.map((download) => {
                ...
                return (
                  <div
                    ...
                    className="text-foreground flex min-h-8 items-center gap-2 text-xs"
                  >
                    {download.status === 'completed' ? (
                      <CircleCheck className="text-muted-foreground size-3.5 shrink-0" />
                    ) : download.status === 'failed' ? (
                      <OctagonX className="text-muted-foreground size-3.5 shrink-0" />
                    ) : (
      ...
      {pageViewport?.container
        ? createPortal(
                ...
                <div className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.02),transparent_58%)] px-6">
                  <div className="flex flex-col items-center px-8 py-8 text-center opacity-70">
                    ...
                    <div className="text-center">
                      <p className="text-foreground/85 text-base font-semibold">
                        {translate(
                    ...
                    {browserAnnotations.map((annotation, index) => (
                      ...
                      >
                        ...
                        <Button
                          size="icon-xs"
                          variant="ghost"
                          className="can-hover:opacity-0 transition-opacity group-focus-within:opacity-100 group-hover:opacity-100 focus-visible:opacity-100"
            ...
            right-click for users who prefer clicking. */}
              {grabToast ? (
                <div
                  className="animate-in fade-in zoom-in-95 absolute z-30 flex items-center duration-150"
                  style={{
                    left: grabToast.x,
                    top: grabToast.y,
