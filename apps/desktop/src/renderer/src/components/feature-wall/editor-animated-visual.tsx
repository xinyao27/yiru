...
// Toolbar icons mirror rich-markdown-toolbar.tsx — same families so the
...
function ToolbarSep(): JSX.Element {
  return <span className="bg-foreground/10 mx-1 h-3.5 w-px" />
}

// Slash menu row — visible always; the active row gets the highlight
// background, mirroring rich-markdown-slash-menu.tsx.
function SlashRow(props: {
  ...
  return (
    ...
    >
      <span className="text-muted-foreground inline-flex items-center justify-center [&>svg]:size-[13px]">
        {TB_ICON[props.iconKey]}
      </span>
      <span className="leading-none whitespace-nowrap">{props.label}</span>
      <span className="text-muted-foreground font-mono text-[10.5px]">{props.shortcut}</span>
    </div>
  )
}
...
export function EditorAnimatedVisual(props: { reducedMotion: boolean }): JSX.Element {
  ...
  return (
    <div className="border-border bg-card text-foreground relative overflow-visible border">
      ...
      {/* Toolbar — visual-only, mirrors rich-markdown-toolbar.tsx button order. */}
      ...
      >
        ...
        >
          <div
            data-slash-show="all"
            className="text-muted-foreground px-2 pt-1.5 pb-1 text-[9.5px] font-bold tracking-[0.06em] uppercase"
          >
          ...
          <SlashRow
            ...
            shortcut="##"
          />
          <div data-slash-show="all" className="bg-foreground/[0.08] my-1 h-px" />
          <div
            data-slash-show="all"
            className="text-muted-foreground px-2 pt-1.5 pb-1 text-[9.5px] font-bold tracking-[0.06em] uppercase"
          >
        ...
        <div
          ref={cursorRef}
          aria-hidden
          className="pointer-events-none absolute top-0 left-0 z-20 transition-[opacity,transform] duration-[600ms] ease-[cubic-bezier(.45,.05,.2,1)]"
          style={{ opacity: 0 }}
        >
...
function DocTitle(props: { children: ReactNode }): JSX.Element {
  return (
    <div className="mb-2.5 text-[22px] leading-[1.15] font-bold tracking-[-0.01em]">
      {props.children}
    </div>
  )
}
