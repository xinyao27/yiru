    ...
    <div className="border-border border-b px-3 py-3">
      ...
      <div className="mt-2 space-y-1.5">
        {files.map((filePath) => (
          <div
            key={filePath}
            className="border-border bg-accent/20 rounded-md border px-2.5 py-1.5"
          >
...
function ViewFullCheckDetailsButton({
  ...
  return (
    <Button
      ...
      variant="outline"
      size="xs"
      className="text-muted-foreground hover:text-foreground h-6 min-w-[7.25rem] shrink-0 gap-1 px-1.5 text-[11px]"
      onClick={onClick}
    >
                ...
                {details!.annotations.map((annotation, index) => (
                  <div key={`${annotation.path ?? 'annotation'}-${index}`} className="min-w-0">
                    ...
                    {annotation.rawDetails && (
                      <pre className="bg-muted/40 text-muted-foreground mt-1 rounded p-2 font-mono text-[11px] whitespace-pre-wrap">
                        {annotation.rawDetails}
                      </pre>
                    )}
                  </div>
                          ...
                          <TooltipTrigger
                            render={
                              <Button
                                ...
                                variant="ghost"
                                size="icon-xs"
                                className="text-muted-foreground hover:text-foreground focus-visible:text-foreground size-6"
                                aria-label={translate(
...
function QueueForAgentButton({
  ...
  return (
    <button
      type="button"
      className={cn(
        'inline-flex shrink-0 items-center gap-0.5 rounded px-1.5 py-0.5 text-[10px] text-muted-foreground transition-[background-color,color,opacity] hover:bg-accent hover:text-foreground focus-visible:outline-none    ',
        className
      )}
      aria-label={label}
      title={label}
                      ...
                      <TooltipTrigger
                        render={
                          ...
                          >
                            ...
                            <span className="border-border bg-background text-foreground absolute -top-1 -right-1 flex h-3.5 min-w-3.5 items-center justify-center rounded-full border px-0.5 text-[9px] leading-none tabular-nums">
