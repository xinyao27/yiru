...
export type ActivateAndRevealResult = {
   ...
   *  `activeTabIdByWorktree`, which may point at another tab if setup scripts
   *  opened their own. */
...
export function activateAndRevealWorktree(
  ...
  // and Settings transitions do not pass through this function, so no
...
export function ensureWorktreeHasInitialTerminal(
  ...
  queueSetupCommands(store, worktreeId, terminalTab.id, setup, wrappedSetupCommandStr, opts)
...
function applyDefaultTerminalTabs(
  ...
  queueSetupCommands(store, worktreeId, firstTabId, setup, wrappedSetupCommandStr, opts)
  return firstTabId
}

function queueSetupCommands(
  store: WorktreeActivationStore,
  worktreeId: string,
