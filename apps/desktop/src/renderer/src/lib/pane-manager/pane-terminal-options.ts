...
import { DEFAULT_TERMINAL_FONT_SIZE } from '../../../../shared/terminal/terminal-fonts'
import { DESKTOP_TERMINAL_SCROLLBACK_ROWS_DEFAULT } from '../../../../shared/terminal/terminal-scrollback-policy'
...
export function resolveTerminalCursorInactiveStyle(
  ...
  // Why: inactive panes keep the configured cursor shape so focus changes do
  // not introduce separate geometry.
...
export function buildDefaultTerminalOptions(): ITerminalOptions {
  ...
  return {
    ...
    scrollbar: {
      ...
      // width bug; it stays fixed by shared/terminal/terminal-unicode-provider.ts. Width
