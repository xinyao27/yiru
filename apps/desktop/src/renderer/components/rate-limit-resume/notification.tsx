...
function workspaceLabel(worktreeId: string): string | null {
  const state = useAppStore.getState()
  return findWorktreeById(state.worktreesByRepo, worktreeId)?.displayName ?? null
}
...
function RateLimitToastBody({ notice }: { notice: RateLimitNotice }): React.JSX.Element {
  ...
  const workspace = workspaceLabel(hit.worktreeId)
