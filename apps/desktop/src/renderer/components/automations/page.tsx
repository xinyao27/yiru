...
import { AutomationRunPageFrame } from './automation-run-page-frame'
import {
...
import type { FetchExternalAutomationRuns } from './external-automation-run-table'
...
import { HermesCronOutputView } from './hermes-cron-output-view'
...
function getExternalTargetKindLabel(_manager: ExternalAutomationManager): string {
  return 'Local'
}
...
export default function AutomationsPage(): React.JSX.Element {
  ...
  const [createTarget, setCreateTarget] = useState<AutomationCreateTarget>('yiru')
  ...
  const automationHookCheckPromisesRef = useRef<
    Map<string, Promise<{ hooks: YiruHooks | null; ok: boolean }>>
  >(new Map())
  const [automationYamlHooksByRepoKey, setAutomationYamlHooksByRepoKey] = useState<
    Record<string, YiruHooks | null>
  >({})
  ...
  const loadAutomationYamlHooksForRepo = useCallback(
    async (repoId: string): Promise<YiruHooks | null> => {
      ...
      const promise = checkRuntimeHooks(settingsForRepo, repoId)
        .then((result) => ({
          hooks: result.status === 'error' ? null : ((result.hooks as YiruHooks | null) ?? null),
  ...
  const hostLabelById = useMemo(() => {
    ...
    return labels
  }, [hostLabelOverrides, runtimeEnvironments])
  ...
  const saveAutomation = async (): Promise<void> => {
    ...
    try {
      ...
      let setupDecision = resolveAutomationSetupDecisionForSave({
        ...
        projectHostSetups,
        yamlHooks:
          createTarget === 'yiru' && draft.workspaceMode === 'new_per_run'
  ...
  return (
    <main className="bg-background text-foreground relative flex h-full min-h-0 flex-col">
        ...
        <DialogContent className="max-w-md" initialFocus={deleteConfirmButtonRef}>
          <DialogHeader>
            ...
            <DialogDescription className="text-xs">
              ...
              <span className="text-foreground font-medium break-all">{deleteTarget?.name}</span>{' '}
              {translate(
      ...
      <div className="border-border/50 grid min-h-0 flex-1 grid-cols-[minmax(280px,360px)_1fr] overflow-hidden border-t">
        <section
                    ...
                    render={
                      ...
                      >
                        <span className="min-w-0">
                          ...
                          <span className="text-foreground/80 mt-1 block truncate text-xs font-medium">
                            {scheduleLabel}
                          </span>
                        ...
                        <span className="text-muted-foreground flex max-w-28 flex-col items-end gap-1 text-right text-xs">
                    ...
                    render={
                      ...
                      >
                        <span className="min-w-0">
                          ...
                          <span className="text-muted-foreground mt-1 flex min-w-0 items-center gap-2 text-xs">
        ...
        <section className="flex min-h-0 flex-col overflow-hidden">
          {selectedExternal ? (
            ...
            >
              <div
                className="border-border/50 flex shrink-0 items-center justify-between border-b px-5 py-2"
              ...
              >
                <TabsList variant="line" className="h-8">
                  ...
                  <TabsTrigger value="runs" disabled={!selected}>
                    ...
                    <span className="text-muted-foreground text-xs">{selectedRuns.length}</span>
                  </TabsTrigger>
                </TabsList>
              </div>
