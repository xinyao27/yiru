...
const MAX_VISIBLE_COWORKING_SESSION_TABS = 24
const MAX_RECENT_COWORKING_SESSION_TABS = 8

type CoworkingSessionTabStripProps = {
...
export function CoworkingSessionTabStrip({
  sessions,
  activeSessionRef,
  onSelect,
  createMenu,
  ...
  onSelectPanel,
  onClosePanel
}: CoworkingSessionTabStripProps): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    setRecentSessionRefs((current) => {
      ...
      ].slice(0, MAX_RECENT_COWORKING_SESSION_TABS)
    })
  }, [activeSessionRef])
  const visibleSessions = useMemo(
    () => projectVisibleCoworkingSessionTabs(sessions, activeSessionRef, recentSessionRefs),
    [activeSessionRef, recentSessionRefs, sessions]
  )
  const activeTabId = activePanel ? getCoworkingWorkspacePanelTabId(activePanel) : activeSessionRef
      ...
      <WorkspaceTabStripViewport
        ...
        stripProps={{
          ...
          'aria-label': translate(
            'auto.components.coworking.CoworkingWorkspaceSurface.tabs.sessions',
            'Sessions'
          ),
          onKeyDown: handleCoworkingSessionTabKeyDown
        }}
      >
...
function projectVisibleCoworkingSessionTabs(
  sessions: readonly CoworkingSessionCatalogEntry[],
  activeSessionRef: string | null,
  recentSessionRefs: readonly string[]
): readonly CoworkingSessionCatalogEntry[] {
  if (sessions.length <= MAX_VISIBLE_COWORKING_SESSION_TABS) {
    return sessions
  }
  ...
  const prioritySessions = priorityRefs
    ...
    .filter((session): session is CoworkingSessionCatalogEntry => session !== undefined)
  const prioritySet = new Set(prioritySessions.map((session) => session.sessionRef))
  const visible = sessions.slice(0, MAX_VISIBLE_COWORKING_SESSION_TABS)
...
function handleCoworkingSessionTabKeyDown(event: KeyboardEvent<HTMLDivElement>): void {
  if (
    event.key !== 'ArrowLeft' &&
