...
export type { WorkspaceSpaceEntryScan } from './directory-frame'
...
export async function scanWorkspaceSpaceEntryTree<TEntry>(
  ...
  if (rootAdmission.entries.length === 0) {
    root.children = []
    return root
  }
  ...
  const rootFrame: DirectoryFrame<TEntry> = {
    result: root,
    entries: rootAdmission.entries,
    retainedBytes: rootAdmission.retainedBytes,
    retired: false,
    nextIndex: 0,
    remainingChildren: rootAdmission.entries.length,
    childResults: Array.from({ length: rootAdmission.entries.length }, () => undefined)
  }
  const availableFrames: DirectoryFrame<TEntry>[] = [rootFrame]
  const waiters = new Set<() => void>()
  let outstandingEntries = rootAdmission.entries.length
  ...
  const expandDirectory = (
    ...
    if (admission.entries.length === 0) {
      completeChild(job.frame, job.index, result)
      return
    }
    outstandingEntries += admission.entries.length
    availableFrames.push({
      ...
      retired: false,
      nextIndex: 0,
      remainingChildren: admission.entries.length,
