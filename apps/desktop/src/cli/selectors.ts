...
export async function resolveCurrentWorktreeSelector(
  ...
  if (!enclosingWorktree) {
    throw new RuntimeClientError(
      'selector_not_found',
      `No Yiru-managed worktree contains the current directory: ${currentPath}`
    )
  }
...
// like `yiru terminal send --text "hello" --enter` Just Work.
...
export type EmulatorCliTarget = {
  worktree?: string
  device?: string
  emulator?: string // Yiru id from list
}
