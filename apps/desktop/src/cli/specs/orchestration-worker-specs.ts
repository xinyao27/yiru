...
export const ORCHESTRATION_WORKER_COMMAND_SPECS: CommandSpec[] = [
  {
    ...
    notes: [
      ...
      'Creation flags (--name, --repo, --base-branch, --display-name, --comment, --setup) are rejected for current/existing worktrees. Use exact --repo on the selected host; project/host convenience routing remains on worktree create.',
      '--on selects only the worker host; the Run and this command remain on the current Coworking host.',
  ...
  },
  {
    ...
    usage: 'yiru orchestration worker-show --dispatch <dispatch_id> [--json]',
  ...
  },
  {
    ...
    usage:
      'yiru orchestration worker-read --dispatch <dispatch_id> [--source <auto|transcript|terminal>] [--cursor <cursor>] [--limit <n>] [--json]',
    ...
    notes: [
      ...
      'A returned cursor is pinned to the exact source; start a fresh read if Yiru reports source_changed.'
    ]
  },
  {
    ...
    usage:
      'yiru orchestration worker-stop --dispatch <dispatch_id> [--retry-request <id>] [--json]',
  ...
  },
  {
    ...
    usage:
      'yiru orchestration worker-abandon --dispatch <dispatch_id> [--retry-request <id>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'dispatch', 'retry-request'],
