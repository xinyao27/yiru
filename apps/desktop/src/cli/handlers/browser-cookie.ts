...
export const BROWSER_COOKIE_HANDLERS: Record<string, CommandHandler> = {
  'cookie get': async ({ flags, client, cwd, json }) => {
    ...
    const result = await client.call(client.rpc.browser.cookie.get, {
      url,
      ...target
    })
  ...
  'cookie set': async ({ flags, client, cwd, json }) => {
    ...
    const params = {
      ...
      ...(domain ? { domain } : {}),
      ...(path ? { path } : {}),
      ...(flags.has('secure') ? { secure: true } : {}),
      ...(flags.has('httpOnly') ? { httpOnly: true } : {}),
      ...(sameSite ? { sameSite } : {}),
      ...(expires !== undefined ? { expires } : {}),
      ...(await getBrowserCommandTarget(flags, cwd, client))
    }
    const result = await client.call(client.rpc.browser.cookie.set, params)
    printResult(result, json, (v) =>
  ...
  'cookie delete': async ({ flags, client, cwd, json }) => {
    ...
    const result = await client.call(client.rpc.browser.cookie.delete, params)
