...
async function resolveOrchestrationTerminalHandle(
  ...
  if (envHandle && envHandle.length > 0) {
    if (flagName === 'from' && options.validateEnvHandle) {
      // Why: long-lived shells can retain a stale YIRU_TERMINAL_HANDLE after remint; don't bake it into coordinator preambles.
...
export const ORCHESTRATION_HANDLERS: Record<string, CommandHandler> = {
  ...
  'orchestration send': async ({ flags, client, cwd, json }) => {
    ...
    // Why: lifecycle senders keep YIRU_TERMINAL_HANDLE verbatim — no liveness probe (worker_done must survive the mid-restart window) and no remint (older runtimes require from === the stale assignee_handle).
