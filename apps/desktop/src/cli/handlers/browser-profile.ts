...
export const BROWSER_PROFILE_HANDLERS: Record<string, CommandHandler> = {
  'tab profile list': async ({ client, json }) => {
    const result = await client.call(client.rpc.browser.profileList, undefined)
  ...
  'tab profile create': async ({ flags, client, json }) => {
    ...
    const result = await client.call(client.rpc.browser.profileCreate, {
      label,
      scope
    })
  ...
  'tab profile delete': async ({ flags, client, json }) => {
    ...
    const result = await client.call(client.rpc.browser.profileDelete, {
      profileId
    })
  ...
  'tab profile set': async ({ flags, client, cwd, json }) => {
    ...
    const result = await client.call(client.rpc.browser.tabSetProfile, {
      ...target,
      profileId
    })
    printResult(
  ...
  'tab profile show': async ({ flags, client, cwd, json }) => {
    const target = await getBrowserCommandTarget(flags, cwd, client)
    const result = await client.call(client.rpc.browser.tabProfileShow, {
      ...target,
  ...
  'tab profile clone': async ({ flags, client, cwd, json }) => {
    const profileId = getRequiredStringFlag(flags, 'profile')
    const target = await getBrowserCommandTarget(flags, cwd, client)
    const result = await client.call(client.rpc.browser.tabProfileClone, {
      ...target,
      profileId
    })
