...
function getEnvParentWorkspace(): string | undefined {
  const workspaceId = process.env.YIRU_WORKSPACE_ID
...
async function getCreateRepoSelector(
  ...
  throw new RuntimeClientError(
    'invalid_argument',
    'Missing repo selector. Pass --repo or run from inside a Yiru-managed worktree.'
  )
}
...
export const WORKTREE_HANDLERS: Record<string, CommandHandler> = {
  ...
  'worktree create': async ({ flags, client, cwd, json }) => {
    ...
    const callerTerminalHandle =
      typeof process.env.YIRU_TERMINAL_HANDLE === 'string' &&
      process.env.YIRU_TERMINAL_HANDLE.length > 0
    ...
    if (
      ...
      try {
        // Why: agent shells can lose YIRU_TERMINAL_HANDLE while still running
        // inside a Yiru worktree. Cwd keeps CLI-created children nestable and
