...
export async function resumeInterruptedServeUpdate(
  ...
  if (!installed) {
    await recordServeUpdateHandoffFailure(
      args.handoffPath,
      args.handoff,
      `Timed out waiting for Yiru ${args.handoff.targetVersion} to be installed.`
    )
  }
...
export async function superviseForegroundServe(
  ...
  while (true) {
    ...
    if (
      ...
      throw new RuntimeClientError('runtime_serve_failed', `Yiru serve exited via ${result.signal}`)
    }
    ...
    if (!installed) {
      await recordServeUpdateHandoffFailure(
        args.handoffPath!,
        handoff,
        `Timed out waiting for Yiru ${handoff.targetVersion} to be installed.`
      )
...
): Promise<{ code: number | null; signal: NodeJS.Signals | null; readiness: ServeReadiness }> {
...
export async function completeServeUpdateHandoff(
  ...
  await writeServeUpdateHandoffState(handoffPath, { ...state, phase: 'completed', runtimeId })
...
export async function recordServeUpdateHandoffFailure(
  ...
  await writeServeUpdateHandoffState(handoffPath, { ...state, phase: 'failed', reason })
