...
/**
 * Proves a host managed home is an Yiru-owned account directory and cannot
...
export function assertOwnedHostCodexManagedHomePath({
  ...
  const markerPath = join(canonicalCandidate, '.yiru-managed-home')
  let markerIsRegularFile: boolean
  try {
    markerIsRegularFile = lstatSync(markerPath).isFile()
  } catch (error) {
    throw new Error('Managed Codex home is missing Yiru ownership marker.', { cause: error })
  }
