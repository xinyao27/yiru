...
import { deleteLocalSpeechModel } from './model-deletion'
import {
  clearOpenAiSpeechApiKey,
  hasOpenAiSpeechApiKey,
  saveOpenAiSpeechApiKey
} from './openai-api-key-store'
import { getSpeechModelManager, getSpeechSttService } from './runtime-service'
            ...
            if (newStatus !== 'granted') {
              throw new Error(
                'Microphone access not granted. In System Settings > Privacy & Security > Microphone, ' +
                  'click "+" and add the Electron app, then restart Yiru.'
              )
            }
          }
        }
