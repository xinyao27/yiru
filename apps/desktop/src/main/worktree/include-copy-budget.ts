...
export function formatWorktreeIncludeCopyWarning(
  ...
  const nameList = (entries: readonly SkippedWorktreeCopyPath[]): string => {
    ...
    return rest > 0
      ? translateMain('worktree.includeCopy.morePaths', '{{value0}} and {{value1}} more', {
          value0: names,
          value1: rest.toLocaleString('en-US')
        })
      : names
  }
  ...
  if (overBudget.length > 0) {
    const interpolation = {
      value0: describe(overBudget),
      value1: formatByteLimit(budget.maxBytes),
      value2: budget.maxEntries.toLocaleString('en-US')
    }
    sentences.push(
      overBudget.length === 1
        ? translateMain(
  ...
  sentences.push(
    skipped.length === 1
      ? translateMain(
          'worktree.includeCopy.manualOne',
          'Copy it in manually if this workspace needs it.'
        )
      : translateMain(
          'worktree.includeCopy.manualMany',
