...
export function registerBrowserHandlers(): void {
  // Why: native file selection is the one L2 holdout. The shell selects the path
  // and consumes it in the same handler; the renderer never receives or supplies it.
  ...
  ipcMain.handle(
    ...
    async (event, args: { profileId: string }): Promise<BrowserCookieImportResult> => {
      ...
      const filePath = await pickCookieFile(BrowserWindow.fromWebContents(event.sender))
      if (!filePath) {
