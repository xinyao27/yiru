...
import { scanAiVaultSessions } from './scanner'
import { isolatedScanRoots, jsonLines } from './scanner-test-fixtures'
...
describe('scanAiVaultSessions codex dual-root dedup', () => {
  it('lists a backfilled both-roots session once, attributed to the real home', async () => {
    const root = await mkdtemp(join(tmpdir(), 'yiru-ai-vault-codex-dedup-'))
    tempRoots.push(root)
    const roots = isolatedScanRoots(root)
    // Sandbox "real ~/.codex" and Yiru managed runtime home, hardlinked the
  ...
  it('keeps different same-name rollouts from separate roots', async () => {
    const root = await mkdtemp(join(tmpdir(), 'yiru-ai-vault-codex-collision-'))
    tempRoots.push(root)
    const roots = isolatedScanRoots(root)
    const realHome = join(root, 'real-codex-home')
    const realSessionsDir = join(realHome, 'sessions')
  ...
  it('fills the listing past cross-volume-style copy aliases', async () => {
    const root = await mkdtemp(join(tmpdir(), 'yiru-ai-vault-codex-copy-cap-'))
    tempRoots.push(root)
    const roots = isolatedScanRoots(root)
    const realHome = join(root, 'real-codex-home')
    const realSessionsDir = join(realHome, 'sessions')
