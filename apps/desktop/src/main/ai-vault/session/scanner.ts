...
import { sessionSortTime } from './scanner-accumulator'
import {
  createAntigravityWorkspaceResolver,
  type AntigravityWorkspaceResolver
} from './scanner-antigravity-history'
import { antigravityHistoryPathForBrainDir } from './scanner-antigravity-paths'
import { codexHomeForSessionsDir } from './scanner-codex-paths'
import {
  createSessionParseStats,
  parseAgentSessionFileCached,
  type SessionParseStats
} from './scanner-parse-cache'
import { discoverInScopeClaudeFiles } from './scanner-scope-discovery'
import { DEFAULT_CODEX_HOME_DIR, discoverAiVaultSessionSources } from './scanner-source-discovery'
import type {
  AiVaultScanOptions,
...
import { clampPositiveInteger, errorMessage } from './scanner-values'
...
async function parseSessionCandidates(args: {
  ...
  while (index < args.candidates.length) {
    ...
    // Why: cross-volume copies lack shared inodes, so parsed identity must
    // collapse aliases before they consume the unique-session parse budget.
