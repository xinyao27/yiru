...
export const COWORKING_SESSION_INVENTORY_MAX_PATH_BYTES = 64 * 1024 * 1024
...
export async function discoverCoworkingSessionInventorySources(args: {
  ...
  for (const root of roots) {
    const discovery = await discoverFiles({
      ...
      maxEntries: COWORKING_SESSION_INVENTORY_MAX_TRAVERSAL_ENTRIES,
      maxCandidates: COWORKING_SESSION_INVENTORY_MAX_CANDIDATES,
      maxPathBytes: COWORKING_SESSION_INVENTORY_MAX_PATH_BYTES,
      budget,
      signal: args.signal
    })
