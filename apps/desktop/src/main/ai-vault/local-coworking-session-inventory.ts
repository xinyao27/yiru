...
type LocalCoworkingSessionInventorySnapshot = AiVaultSessionInventorySnapshot & {
...
export type LocalCoworkingSessionInventoryPageArgs = {
  bindingKey: string
  cursor: string | null
  executionHostId: ExecutionHostId
...
const cursorStore = new AiVaultSessionInventoryCursorStore<LocalCoworkingSessionInventorySnapshot>({
  pageSize: INVENTORY_PAGE_SIZE
})
const snapshotCache = new SessionInventorySnapshotCache<LocalCoworkingSessionInventorySnapshot>({
...
export async function listLocalCoworkingSessionInventoryPage(
  args: LocalCoworkingSessionInventoryPageArgs
...
export function releaseLocalCoworkingSessionInventoryPage(
  args: LocalCoworkingSessionInventoryPageArgs
): void {
...
async function openLocalInventorySnapshot(
  args: LocalCoworkingSessionInventoryPageArgs,
  signal: AbortSignal
): Promise<LocalCoworkingSessionInventorySnapshot> {
  signal.throwIfAborted()
  const issues: AiVaultScanIssue[] = []
  const roots = await resolveLocalInventoryRoots(args, signal)
  const discoveries = await discoverCoworkingSessionInventorySources({
    options: {
...
async function readLocalInventoryPage(
  snapshot: LocalCoworkingSessionInventorySnapshot,
  offset: number,
  pageSize: number,
  signal: AbortSignal
...
function localInventoryBindingKey(args: LocalCoworkingSessionInventoryPageArgs): string {
  return JSON.stringify([args.bindingKey, args.executionHostId])
}

function localSnapshotKey(args: LocalCoworkingSessionInventoryPageArgs): string {
