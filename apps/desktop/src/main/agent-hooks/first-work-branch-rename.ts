...
export type FirstWorkBranchRenameDeps = {
  ...
  /** True only for Yiru-created worktrees whose branch Yiru is allowed to rename. */
  canRenameYiruCreatedBranch: (worktreeId: string) => boolean
...
async function runAutoRename(
  ...
  if (getRepoExecutionHostId(repo) !== LOCAL_EXECUTION_HOST_ID) {
    return stop('repo is owned by a paired runtime')
  }
  ...
  if (!deps.canRenameYiruCreatedBranch(worktreeId)) {
    return stop(`worktree is not eligible for auto-rename`, true)
  }
  // Only rename a branch Yiru auto-named — never one the user chose.
  ...
  const resolvedParams = resolveTextGenerationParams(settings, 'local', 'branchName', repo)
