...
export function registerComputerUsePermissionHandlers(): void {
  ipcMain.handle(
    ...
    async (
      ...
      const { openComputerUsePermissions } = await import('./macos-computer-use-permissions')
  ...
  ipcMain.handle(
    ...
    async (): Promise<ComputerUsePermissionResetResult> => {
      const { resetComputerUsePermissions } = await import('./macos-computer-use-permissions')
