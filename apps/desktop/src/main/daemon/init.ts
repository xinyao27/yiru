...
// the local trace sink honors (YIRU_DIAGNOSTICS_DISABLED); absence of the arg
  ...
  return async (socketPath, tokenPath) => {
    ...
    const child = fork(
      ...
      ['--socket', socketPath, '--token', tokenPath, ...daemonLogArgs()],
      {
        ...
        // Why: run the relocated Yiru.exe copy instead of the install-dir one.
...
// later daemon crash while Yiru is closed is still recoverable on next launch.
...
export async function cleanupDaemonForProtocol(
  ...
  try {
    ...
    // Why: the daemon exposes a single-shot `shutdown` RPC (server.ts)
