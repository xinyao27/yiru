...
export async function killAllDaemonSessions(): Promise<{
  killedCount: number
  ...
  return {
    ...
    killedSessionIds: [...initialIds].filter((sessionId) => !remainingOriginalIds.has(sessionId))
  }
}

export async function killDaemonSession(sessionId: string): Promise<{ success: boolean }> {
  ...
  try {
    await owner.shutdown(sessionId, { immediate: true })
    return { success: true }
  } catch {
    return { success: false }
  }
...
export async function restartPtyDaemon(): Promise<{ success: boolean }> {
  try {
    await restartDaemon()
    return { success: true }
  } catch (err) {
