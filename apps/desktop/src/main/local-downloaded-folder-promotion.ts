...
function publishedEntryFromStats(
  ...
  return {
    ...
    ...(kind === 'file'
      ? { fileState: { size: stats.size, mtimeNs: stats.mtimeNs, mode: stats.mode } }
      : {})
  }
}
...
async function copyTrackedFileNoClobber(
  sourcePath: string,
...
async function publishTrackedFileNoClobber(
  ...
  try {
    ...
    await copyTrackedFileNoClobber(sourcePath, destinationPath, publishedEntries, signal)
    return
  }
  // Why: hard links preserve dev+ino; register ownership before any async
  // rollback window can let an unrelated entry take over the destination.
  ...
  for (const entry of entries.toReversed()) {
    try {
      ...
      if (entry.kind === 'file') {
        ...
        // Why: rmdir removes only our still-empty directory; third-party
        // children make rollback fail closed instead of being deleted.
...
async function publishDirectoryNoClobber(
  ...
  // Why: capture identity synchronously after the exclusive claim so rollback
  // never deletes a directory that another process replaced.
...
export async function promoteLocalDownloadedFolder(
  ...
  try {
    ...
    // each destination entry preserves no-clobber while promotion stays local.
