...
export function getPiAgentStatusHandlerSourceLines(kind: PiAgentKind): string[] {
  const sessionStartHandler =
    kind === 'pi'
      ? [
          ...
          '    // Why: /reload re-registers the session but is not a turn',
          '    // boundary, so it must not clear visible status or unread state.',
  ...
  // Why: OMP can switch sessions in-process, so each latest-only post needs
  // fresh identity from the event context.
  ...
  return [
    ...
    '  const ownerPid = process.env.YIRU_PI_STATUS_OWNED',
    '  const selfPid = String(process.pid)',
    '  if (ownerPid && ownerPid !== selfPid) return',
    '  process.env.YIRU_PI_STATUS_OWNED = selfPid',
    ...sessionStartHandler,
