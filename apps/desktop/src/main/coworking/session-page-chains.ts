...
import type {
  CoworkingSessionPageProjector,
  CoworkingSessionPageState
} from './session-page-projector'
import type { CoworkingSessionCatalogDescription } from './session-resolution'
import {
  requireExactWorktreeIdentity,
  requireInventoryScope,
  sessionChainBindingKey,
  toSessionWorktree
} from './session-worktree-binding'
...
export type CoworkingSessionCatalogPageResult = {
  sessions: readonly CoworkingSessionCatalogDescription[]
  nextCursor: string | null
}

type SessionPageChain = CoworkingSessionPageState & {
  id: string
  bindingKey: string
  instanceId: string
...
type SessionPageCursor = {
  chainId: string
  page: Promise<CoworkingSessionCatalogPageResult> | null
}

export class CoworkingSessionPageChains {
  ...
  constructor(private readonly projector: CoworkingSessionPageProjector) {}
  ...
  async listPage(
    ...
    if (
      ...
      throw new Error('Unknown or mismatched Coworking session catalog cursor')
    }
    chain.activeReads++
    ...
    try {
      ...
      const requireCurrent = (): void => {
        if (
          ...
          throw new Error('Coworking session catalog changed during inventory creation')
        }
      }
  ...
  private requireCurrent(chain: SessionPageChain): void {
    if (
      ...
      throw new Error('Coworking session catalog chain was invalidated during a page read')
    }
  }
