...
export type CoworkingTerminalSessionBinding = {
...
type CoworkingTerminalSessionExecutionHostIdentity = {
  instanceId: string
  coworkingIncarnationId: string
  actualHostScope: string
...
export class CoworkingTerminalSessionBindings {
  private readonly bindings = new Map<string, CoworkingTerminalSessionBinding>()
  ...
  rememberSpawned(
    ...
    terminalHandle: string,
    session: {
      provider: CoworkingSessionProvider
      title: string
    } & CoworkingLiveSessionDisplayIdentity
  ): void {
    ...
    const displayIdentity: CoworkingLiveSessionDisplayIdentity = session
    this.rememberBinding({
      sessionKey: coworkingLiveTerminalSessionKey(worktree, terminalHandle),
      terminalHandle,
  ...
  ): CoworkingTerminalSessionBinding | null {
  ...
  private rememberBinding(binding: CoworkingTerminalSessionBinding): void {
  ...
  resolve(
    worktree: Pick<
      CoworkingSessionWorktreeIdentity,
      'instanceId' | 'coworkingIncarnationId' | 'actualHostScope' | 'target'
    >,
    terminalHandle: string
  ): CoworkingTerminalSessionBinding | null {
  ...
  resolveForExecutionHost(
    worktree: CoworkingTerminalSessionExecutionHostIdentity,
    terminalHandle: string
  ): CoworkingTerminalSessionBinding | null {
    const binding = this.bindings.get(terminalHandle)
    return binding &&
      binding.worktreeInstanceId === worktree.instanceId &&
      binding.coworkingIncarnationId === worktree.coworkingIncarnationId &&
  ...
  reconcile(
    worktree: Pick<
      ...
      'worktreeId' | 'instanceId' | 'coworkingIncarnationId'
    >,
  ...
  private emitChange(instanceId: string): void {
    for (const listener of this.listeners) {
      try {
        ...
        console.error('[coworking] Terminal-session observer failed')
      }
    }
  }
}
...
function sameTerminalSessionBinding(
  left: CoworkingTerminalSessionBinding,
  right: CoworkingTerminalSessionBinding
): boolean {
  return (
