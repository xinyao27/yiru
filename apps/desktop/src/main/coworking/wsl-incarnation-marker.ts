...
const MARKER_INVALID = '__YIRU_COWORKING_MARKER_INVALID__'
const MARKER_HOST_UNAVAILABLE = '__YIRU_COWORKING_MARKER_HOST_UNAVAILABLE__'
...
export function readOrCreateCoworkingWslIncarnationMarker(
  directory: string,
  filename: string,
  ...
  if (
    ...
    !isCoworkingIncarnationMarkerId(proposedMarkerId)
  ) {
    return Promise.reject(new CoworkingWorktreeIncarnationHostError('marker-unavailable'))
  }
    ...
    execFile(
      ...
      (error, stdout) => {
        if (error || typeof stdout !== 'string' || stdout === MARKER_HOST_UNAVAILABLE) {
          reject(new CoworkingWorktreeIncarnationHostError('host-unavailable', { cause: error }))
          return
        }
        const markerId = stdout.startsWith(MARKER_RESULT_PREFIX)
          ? stdout.slice(MARKER_RESULT_PREFIX.length)
          : ''
        if (stdout === MARKER_INVALID || !isCoworkingIncarnationMarkerId(markerId)) {
          reject(new CoworkingWorktreeIncarnationHostError('marker-unavailable'))
          return
        }
        resolve(markerId)
      }
