...
export type CoworkingAccessAuthorityOptions = {
...
export class CoworkingAccessError extends Error {
  constructor(readonly code: 'resource_busy' | 'resource_not_found' | 'unauthorized') {
    super(code)
    this.name = 'CoworkingAccessError'
  }
}

export class CoworkingAccessAuthority {
  private readonly connections = new Map<string, AuthenticatedCoworkingPrincipal>()
  private readonly requests = new Map<string, CoworkingControlRequest>()
  private readonly grants = new Map<string, CoworkingControlGrant>()
  ...
  constructor(options: CoworkingAccessAuthorityOptions) {
  ...
  request(target: {
    ...
    if (!principal || !this.isPublic(target.instanceId, target.shareEpoch)) {
      throw new CoworkingAccessError('resource_not_found')
    }
    if (this.getControlGrant(target.connectionId, target.instanceId, target.shareEpoch)) {
      ...
      throw new CoworkingAccessError('resource_busy')
    }
    ...
    const request: CoworkingControlRequest = {
  ...
  decide(ownerDecision: CoworkingOwnerDecision): CoworkingControlDecision {
    ...
    const grant: CoworkingControlGrant = {
  ...
  requireControl(
    ...
    if (!grant || !this.connections.has(connectionId) || !this.isPublic(instanceId, shareEpoch)) {
      throw new CoworkingAccessError('unauthorized')
    }
    return grant
  }
  ...
  subscribeOwnerRequests(
    listener: (requests: readonly CoworkingControlRequest[]) => void
  ): () => void {
  ...
  subscribeGrants(listener: (grants: readonly CoworkingControlGrant[]) => void): () => void {
  ...
  private findRequest(
    ...
    instanceId: string,
    shareEpoch: string
  ): CoworkingControlRequest | undefined {
  ...
  private requestSnapshot(): readonly CoworkingControlRequest[] {
  ...
  private grantSnapshot(): readonly CoworkingControlGrant[] {
