...
import {
  ...
  COWORKING_FOLDER_INCARNATION_TEMP_PREFIX
...
type FolderMetadataHost = Pick<
  CoworkingWorktreeContainmentHost,
  'joinPath' | 'resolveCanonicalAlias'
>

export function coworkingFolderPathContainsGitSegment(pathValue: string): boolean {
...
export function requireVisibleCoworkingFolderPath(
...
export function isCoworkingFolderIncarnationMetadataPath(
  root: CoworkingCanonicalHostPath,
...
export function isCoworkingFolderHiddenMetadataChild(args: {
...
async function isResolvedFolderMetadataAlias(args: {
  ...
  try {
    ...
    return (
      coworkingFolderPathContainsGitSegment(canonicalTarget.absolutePath) ||
      isCoworkingFolderIncarnationMetadataPath(args.root, canonicalTarget)
    )
  } catch {
...
function isFolderIncarnationMetadataName(name: string, rootPath: string): boolean {
  ...
  const marker = windowsPath
    ? COWORKING_FOLDER_INCARNATION_MARKER_FILENAME.toLowerCase()
    : COWORKING_FOLDER_INCARNATION_MARKER_FILENAME
  const temporaryPrefix = windowsPath
    ? COWORKING_FOLDER_INCARNATION_TEMP_PREFIX.toLowerCase()
    : COWORKING_FOLDER_INCARNATION_TEMP_PREFIX
