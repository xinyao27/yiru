...
export type CoworkingWindowsFirewallEnvironment = {
...
export async function inspectWindowsCoworkingFirewall(
  port: number,
  environment: CoworkingWindowsFirewallEnvironment
): Promise<CoworkingWindowsFirewallStatus> {
...
export async function repairWindowsCoworkingFirewall(
  port: number,
  environment: CoworkingWindowsFirewallEnvironment
): Promise<CoworkingWindowsFirewallRepairResult> {
  if (!isSupported(environment)) {
...
export async function assertWindowsCoworkingFirewallReady(
  port: number,
  environment: CoworkingWindowsFirewallEnvironment
): Promise<void> {
  const status = await inspectWindowsCoworkingFirewall(port, environment)
  if (!status.supported || status.ruleAllowed) {
    return
  }
  throw new Error('coworking_windows_firewall_unavailable')
}

function isSupported(environment: CoworkingWindowsFirewallEnvironment): boolean {
...
$ready = $false
$rules = @(Get-NetFirewallRule -ErrorAction Stop | Where-Object { [string]$_.Name -eq ${quotePowerShell(COWORKING_WINDOWS_FIREWALL_RULE_NAME)} -or [string]$_.DisplayName -eq ${quotePowerShell(COWORKING_WINDOWS_FIREWALL_RULE_NAME)} })
if ($rules.Count -eq 1) {
  $rule = $rules[0]
...
function buildRepairScript(port: number, executablePath: string): string {
  return `$ErrorActionPreference = 'Stop'
Get-NetFirewallRule -ErrorAction Stop | Where-Object { [string]$_.Name -eq ${quotePowerShell(COWORKING_WINDOWS_FIREWALL_RULE_NAME)} -or [string]$_.DisplayName -eq ${quotePowerShell(COWORKING_WINDOWS_FIREWALL_RULE_NAME)} } | Remove-NetFirewallRule
New-NetFirewallRule -Name ${quotePowerShell(COWORKING_WINDOWS_FIREWALL_RULE_NAME)} -DisplayName ${quotePowerShell(COWORKING_WINDOWS_FIREWALL_RULE_NAME)} -Description 'Allows Yiru Coworking sharing over Tailscale on private networks.' -Direction Inbound -Action Allow -Enabled True -Profile Private -Protocol TCP -LocalPort ${port} -Program ${quotePowerShell(executablePath)} -EdgeTraversalPolicy Block | Out-Null`
}
...
function getRunner(environment: CoworkingWindowsFirewallEnvironment): PowerShellRunner {
