...
type CoworkingVisibilityTransitionState = {
  isDegraded(): boolean
  enterDegraded(): void
...
export class CoworkingVisibilityTransitionSerializer {
  private tail: Promise<void> = Promise.resolve()

  constructor(private readonly state: CoworkingVisibilityTransitionState) {}

  serialize<T>(operation: () => Promise<T>): Promise<T> {
    if (this.state.isDegraded()) {
      return Promise.reject(new CoworkingVisibilityError('persistence-failed'))
    }
    return this.enqueue(async () => {
      ...
      try {
        ...
        if (error instanceof CoworkingVisibilityError && error.code === 'persistence-failed') {
