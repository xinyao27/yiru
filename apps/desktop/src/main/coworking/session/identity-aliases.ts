...
export class CoworkingSessionIdentityAliases {
  ...
  remember(
    ...
    for (const [existingKey, alias] of this.aliases) {
      if (
        ...
        alias.coworkingIncarnationId === worktree.coworkingIncarnationId &&
  ...
  resolve(
    worktree: AliasScope,
    session: Pick<CoworkingHistoricalSessionCandidate, 'provider' | 'providerSessionId'>
  ): string | null {
    return (
