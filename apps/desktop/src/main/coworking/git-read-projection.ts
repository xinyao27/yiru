...
export type CoworkingRawHistoryEntry = {
  oid: string
...
export function projectCoworkingGitStatus(
  stdout: string,
  ...
  const entries = parser.entries
    ...
    .filter((entry): entry is CoworkingGitStatusEntry => entry !== null)
...
export function parseCoworkingGitHistory(stdout: string): readonly CoworkingRawHistoryEntry[] {
  const entries: CoworkingRawHistoryEntry[] = []
...
export function projectCoworkingGitHistory(
  entries: readonly CoworkingRawHistoryEntry[],
  references: ReadonlyMap<string, string>
): readonly CoworkingGitHistoryEntry[] {
...
function projectStatusEntry(entry: {
  path: string
  oldPath?: string
  status: CoworkingGitStatusEntry['status']
  area: CoworkingGitStatusEntry['area']
  conflictStatus?: string
}): CoworkingGitStatusEntry | null {
...
function safeRelativePath(value: string): string | null {
  try {
    return normalizeCoworkingRelativePath(value)
  } catch {
    return null
  }
}
