...
import { CoworkingCatalogDescriptionReader } from './catalog-description-reader'
import { coworkingCatalogFingerprint } from './catalog-fingerprint'
import {
  buildCatalogReferenceBindings,
  buildReservedCatalogSessionBinding,
  projectCatalogEntries,
  type ResolvedCoworkingCatalogWorktree
} from './catalog-projection-model'
import {
  CoworkingCatalogReferenceTable,
  type CoworkingCatalogReferenceBinding
} from './catalog-reference-table'
import {
  CoworkingCatalogSessionPages,
  type CoworkingCatalogSessionPageRequest
} from './catalog-session-pages'
...
export type BoundCoworkingWorktree = CoworkingPublicWorktreeInstance

export type BoundCoworkingSession = {
  worktree: BoundCoworkingWorktree
  sessionKey: string
}

export class CoworkingCatalogProjection {
  private readonly references = new CoworkingCatalogReferenceTable()
  private readonly descriptions: CoworkingCatalogDescriptionReader
  private readonly sessionPages: CoworkingCatalogSessionPages
  private revision = 0
  private generation = 0
  private fingerprint = ''
  private snapshotGeneration = -1
  private snapshotDescriptions: readonly ResolvedCoworkingCatalogWorktree[] = []
  ...
  constructor(
    ...
    this.descriptions = new CoworkingCatalogDescriptionReader(visibility, source)
    this.sessionPages = new CoworkingCatalogSessionPages(source, visibility, this.references)
  }

  snapshot(): Promise<{ catalog: CoworkingDesktopCatalog; generation: number }> {
    return this.serialize(async () => {
      for (let attempt = 0; attempt < MAX_CATALOG_SNAPSHOT_ATTEMPTS; attempt++) {
        ...
        const nextFingerprint = coworkingCatalogFingerprint(descriptions)
      ...
      throw new Error('coworking_catalog_changed_during_snapshot')
    })
  }
  ...
  async resolveWorktree(worktreeRef: string): Promise<BoundCoworkingWorktree | null> {
  ...
  async resolveSession(sessionRef: string): Promise<BoundCoworkingSession | null> {
  ...
  resolvePublishedSession(sessionRef: string): BoundCoworkingSession | null {
    const binding = this.resolveCurrentSessionBinding(sessionRef)
    if (!binding) {
      return null
    }
  ...
  async sessionPage(
    request: CoworkingCatalogSessionPageRequest,
    signal: AbortSignal
  ): Promise<{ page: CoworkingSessionCatalogPage; generation: number } | null> {
  ...
  invalidate(change: CoworkingVisibilityChange): void {
    this.generation++
  ...
  private async resolveCurrentWorktree(
    ...
    >
  ): Promise<BoundCoworkingWorktree | null> {
    if (this.closed) {
      return null
    }
  ...
  private resolveCurrentSessionBinding(
    sessionRef: string
  ): Extract<CoworkingCatalogReferenceBinding, { kind: 'session' }> | null {
  ...
  private requireOpen(): void {
    if (this.closed) {
      throw new Error('Coworking catalog projection is closed')
    }
  }
  ...
  private emitChange(): void {
    for (const listener of this.listeners) {
      try {
        ...
        console.error('[coworking] Share catalog listener failed')
      }
    }
  }
