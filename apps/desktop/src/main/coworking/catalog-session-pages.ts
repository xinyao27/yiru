...
export type CoworkingCatalogSessionPageRequest = {
  worktreeRef: string
  shareEpoch: string
...
type SessionPageReadContext = {
  ...
  snapshotDescriptions: readonly ResolvedCoworkingCatalogWorktree[]
  isCurrent(): boolean
...
type CachedSessionPage = {
  ...
  page: CoworkingSessionCatalogPage
}
...
export class CoworkingCatalogSessionPages {
  private readonly dynamicBindings = new Map<string, CoworkingCatalogReferenceBinding>()
  ...
  constructor(
    ...
    private readonly references: CoworkingCatalogReferenceTable
  ) {}

  bindings(): CoworkingCatalogReferenceBinding[] {
  ...
  async read(
    ...
    const binding = pageBinding as Extract<
      CoworkingCatalogReferenceBinding,
      { kind: 'session-page' }
    >
    ...
    let instance: CoworkingPublicWorktreeInstance | null = null
    const inventoryScope = this.inventoryScope
    let releaseCursor: string | null = null
    try {
      pending.controller.signal.throwIfAborted()
      instance = await tagCoworkingSessionCatalogStage(
      ...
      const result = await tagCoworkingSessionCatalogStage(
      ...
      pending.controller.signal.throwIfAborted()
      const sessions = projectCoworkingSessionCatalogValue(
        () => sanitizeCatalogSessionDescriptions(result.sessions),
        'session-wire-projection'
      )
      if (sessions.length > COWORKING_CATALOG_MAX_SESSIONS_PER_WORKTREE) {
        throw new Error('Coworking catalog session page size exceeded')
      }
      const current = await tagCoworkingSessionCatalogStage(
        this.visibility.resolvePublicInstance(binding.instanceId, binding.shareEpoch),
        'session-publication'
      )
      if (
        ...
        !isSameCoworkingPublicWorktreeInstance(current, instance) ||
        !context.isCurrent()
      ) {
      ...
      try {
        ...
        throw coworkingSessionCatalogError(error, 'session-references')
      }
      const page = projectCoworkingSessionCatalogValue(
        () =>
          projectCatalogSessionPage(
            request.worktreeRef,
            binding,
  ...
  private deleteBindings(predicate: (binding: CoworkingCatalogReferenceBinding) => boolean): void {
...
function isValidPageRequest(
  request: CoworkingCatalogSessionPageRequest,
  context: SessionPageReadContext,
  page: CoworkingCatalogReferenceBinding | null,
  worktree: CoworkingCatalogReferenceBinding | null,
...
function sameWorktreeBinding(
  page: Extract<CoworkingCatalogReferenceBinding, { kind: 'session-page' }>,
  worktree: Extract<CoworkingCatalogReferenceBinding, { kind: 'worktree' }>
): boolean {
  return (
...
function samePageBinding(
  left: Extract<CoworkingCatalogReferenceBinding, { kind: 'session-page' }>,
  right: Extract<CoworkingCatalogReferenceBinding, { kind: 'session-page' }>
): boolean {
