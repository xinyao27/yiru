...
type TerminalCreateLedgerEntry = {
  ...
  fingerprint: string
  promise: Promise<CoworkingTerminalCreateHostResult>
  settled: boolean
}
...
export class CoworkingTerminalCreateLedger {
  private readonly entries = new Map<string, TerminalCreateLedgerEntry>()

  run(
    key: TerminalCreateLedgerKey,
    create: () => Promise<CoworkingTerminalCreateHostResult>
  ): Promise<CoworkingTerminalCreateHostResult> {
    ...
    const entry: TerminalCreateLedgerEntry = {
      ...
      coworkingIncarnationId: key.coworkingIncarnationId,
  ...
  private rememberFailure(key: string, entry: TerminalCreateLedgerEntry, error: unknown): void {
    ...
    if (error instanceof CoworkingExecutionError && error.code === 'outcome_unknown') {
...
export function coworkingTerminalCreateFingerprint(
  launch: CoworkingTerminalCreateOperation['launch']
): string {
...
function sameConnectionWorktree(
  ...
  return (
    ...
    entry.coworkingIncarnationId === key.coworkingIncarnationId
  )
}
