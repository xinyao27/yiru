...
export type CoworkingSessionReadRouteBinding = Readonly<{
  bindingKey: string
}>

export type CoworkingSessionReadRouteLease = {
...
export class CoworkingSessionReadRoutes {
  ...
  begin(
    binding: CoworkingSessionReadRouteBinding,
    cursor: string | null,
    firstRequest?: CoworkingExecutionHostSessionReadRequest
  ): CoworkingSessionReadRouteLease {
    this.expireIdleRoutes()
    if (cursor === null) {
      if (!firstRequest) {
        throw new Error('Missing first Coworking session read route')
      }
    ...
    if (!chain || chain.bindingKey !== binding.bindingKey) {
      throw new Error('Unknown or mismatched Coworking session read route')
    }
    chain.activeReads++
  ...
  commit(lease: CoworkingSessionReadRouteLease, nextCursor: string | null): void {
    try {
      if (lease.chain.releaseRequested) {
        throw new Error('Coworking session read route was released during a page read')
      }
      if (nextCursor === null) {
      ...
      if (existing && existing !== lease.chain) {
        throw new Error('Coworking session read cursor collision')
      }
      if (!existing) {
  ...
  fail(lease: CoworkingSessionReadRouteLease): void {
    lease.chain.releaseRequested = true
    if (lease.settled) {
  ...
  release(
    binding: CoworkingSessionReadRouteBinding,
    cursor: string | null
  ): CoworkingExecutionHostSessionReadRequest | null {
    if (cursor === null) {
      return null
    }
    const chain = this.chainByCursor.get(cursor)
  ...
  private settle(lease: CoworkingSessionReadRouteLease): void {
    if (lease.settled) {
      return
    }
    lease.settled = true
...
function requireValidCursor(cursor: string): void {
  if (cursor.length === 0 || cursor.length > 2_048 || cursor.includes('\0')) {
    throw new Error('Invalid Coworking session read cursor')
  }
}
