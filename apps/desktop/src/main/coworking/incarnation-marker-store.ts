import { randomUUID } from 'node:crypto'

import { parseWslUncPath } from '@yiru/workbench-model/platform'

import type { IFilesystemProvider } from '../providers/types'
import { classifyCoworkingIncarnationMarkerIoError } from './incarnation-marker-error'
import { readOrCreateCoworkingLocalIncarnationMarker } from './local-incarnation-marker'
import { CoworkingWorktreeIncarnationHostError } from './worktree-incarnation'
import { readOrCreateCoworkingWslIncarnationMarker } from './wsl-incarnation-marker'

const INCARNATION_MARKER_FILENAME = 'yiru-coworking-incarnation-v1'
export const COWORKING_FOLDER_INCARNATION_MARKER_FILENAME = '.yiru-coworking-incarnation-v1'
export const COWORKING_FOLDER_INCARNATION_TEMP_PREFIX = `${COWORKING_FOLDER_INCARNATION_MARKER_FILENAME}.tmp-`

export type CoworkingIncarnationMarkerLocation =
  | { kind: 'local'; directory: string }
  | {
      kind: 'ssh'
      filesystem: IFilesystemProvider
      directory: string
    }

export class CoworkingIncarnationMarkerStore {
  private readonly localPendingOperations = new Map<string, Promise<string>>()
  private readonly remotePendingOperations = new WeakMap<
    IFilesystemProvider,
    Map<string, Promise<string>>
  >()

  readOrCreate(
    location: CoworkingIncarnationMarkerLocation,
    filename = INCARNATION_MARKER_FILENAME
  ): Promise<string> {
    const pendingOperations = this.pendingOperationsFor(location)
    const key = `${location.directory}\0${filename}`
    const existing = pendingOperations.get(key)
    if (existing) {
      return existing
    }
    const pending = readOrCreateMarkerAtLocation(location, filename).finally(() => {
      if (pendingOperations.get(key) === pending) {
        pendingOperations.delete(key)
      }
    })
    pendingOperations.set(key, pending)
    return pending
  }

  private pendingOperationsFor(
    location: CoworkingIncarnationMarkerLocation
  ): Map<string, Promise<string>> {
    if (location.kind === 'local') {
      return this.localPendingOperations
    }
    let operations = this.remotePendingOperations.get(location.filesystem)
    if (!operations) {
      operations = new Map()
      this.remotePendingOperations.set(location.filesystem, operations)
    }
    return operations
  }
}

function readOrCreateMarkerAtLocation(
  location: CoworkingIncarnationMarkerLocation,
  filename: string
): Promise<string> {
  if (location.kind === 'ssh') {
    return readOrCreateRemoteMarker(location.filesystem, location.directory, filename)
  }
  if (parseWslUncPath(location.directory)) {
    // Why: Win32 cannot publish hardlinks inside a WSL distro filesystem.
    return readOrCreateCoworkingWslIncarnationMarker(location.directory, filename, randomUUID())
  }
  return readOrCreateCoworkingLocalIncarnationMarker(location.directory, filename)
}

async function readOrCreateRemoteMarker(
  filesystem: IFilesystemProvider,
  directory: string,
  filename: string
): Promise<string> {
  try {
    const verified = filesystem.coworkingVerifiedFiles
    if (!verified) {
      throw new CoworkingWorktreeIncarnationHostError('host-unavailable')
    }
    // Why: marker creation must remain exclusive and no-follow on the remote host.
    return await verified.readOrCreateIncarnationMarker(directory, filename, randomUUID())
  } catch (error) {
    if (isRemoteMarkerIntegrityError(error)) {
      throw new CoworkingWorktreeIncarnationHostError('marker-unavailable', { cause: error })
    }
    throw classifyCoworkingIncarnationMarkerIoError(error)
  }
}

function isRemoteMarkerIntegrityError(error: unknown): boolean {
  const message = error instanceof Error ? error.message : ''
  return message.startsWith('coworking_marker_') || message === 'remote_coworking_marker_invalid'
}
