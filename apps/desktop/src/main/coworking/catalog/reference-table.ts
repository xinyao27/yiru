...
export type CoworkingCatalogReferenceBinding =
...
export class CoworkingCatalogReferenceTable {
  private readonly referenceByAliasKey = new Map<string, string>()
  private readonly bindingByReference = new Map<string, CoworkingCatalogReferenceBinding>()
  private readonly pinnedSessionReferences = new Map<
    string,
    { reference: string; instanceId: string }
  >()

  reconcile(bindings: readonly CoworkingCatalogReferenceBinding[]): void {
    const nextReferences = new Map<string, string>()
    const nextBindings = new Map<string, CoworkingCatalogReferenceBinding>()
  ...
  referenceFor(aliasKey: string): string {
    ...
    if (!reference) {
      throw new Error('Missing Coworking catalog reference')
    }
    return reference
  }

  resolve(reference: string): CoworkingCatalogReferenceBinding | null {
    return this.bindingByReference.get(reference) ?? null
  }

  reserveSession(binding: Extract<CoworkingCatalogReferenceBinding, { kind: 'session' }>): string {
    const reference =
      this.pinnedSessionReferences.get(binding.aliasKey)?.reference ??
      this.referenceByAliasKey.get(binding.aliasKey) ??
