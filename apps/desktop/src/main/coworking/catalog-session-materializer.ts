...
import {
  CoworkingCatalogSessionBudget,
  coworkingCatalogSessionBytes
} from './catalog-session-budget'
import { isCoworkingSessionCatalogPage } from './catalog-wire-validation'
...
type MaterializeCoworkingCatalogSessionsOptions = {
  baseCatalog: CoworkingDesktopCatalog
  previousCatalog: CoworkingDesktopCatalog | null
  connection: CoworkingPeerConnection
  signal: AbortSignal
  isCurrent(): boolean
  publish(catalog: CoworkingDesktopCatalog): void
}

export async function materializeCoworkingCatalogSessions(
  options: MaterializeCoworkingCatalogSessionsOptions
...
export function markCoworkingCatalogSessionLoadError(
...
async function materializeWorktree(
  options: MaterializeCoworkingCatalogSessionsOptions,
  initialCatalog: CoworkingDesktopCatalog,
  worktree: CoworkingWorktreeCatalogEntry,
  budget: CoworkingCatalogSessionBudget
): Promise<CoworkingDesktopCatalog> {
  let catalog = initialCatalog
  let cursor = worktree.sessionCatalog.nextCursor
  const loaded: CoworkingSessionCatalogEntry[] = []
  ...
  try {
    while (cursor) {
      if (cursors.has(cursor)) {
        throw new Error('coworking_catalog_session_cursor_repeated')
      }
      if (cursors.size >= MAX_SESSION_CATALOG_PAGES) {
        throw new Error('coworking_catalog_session_page_limit_exceeded')
      }
      cursors.add(cursor)
      const value = await options.connection.request<unknown>(
        ...
        { signal: options.signal, timeoutMs: COWORKING_SESSION_PAGE_REQUEST_TIMEOUT_MS }
      )
      if (!options.isCurrent()) {
        return catalog
      }
      if (
        !isCoworkingSessionCatalogPage(value, {
          catalogRevision: options.baseCatalog.catalogRevision,
          worktreeRef: worktree.worktreeRef,
          shareEpoch: worktree.shareEpoch
        })
      ) {
        throw new Error('invalid_coworking_catalog_session_page')
      }
      ...
      if (
        ...
        throw new Error('coworking_catalog_session_capacity_exceeded')
      }
      for (const session of value.sessions) {
        if (loadedRefs.has(session.sessionRef)) {
          throw new Error('duplicate_coworking_catalog_session_ref')
        }
        loadedRefs.add(session.sessionRef)
        loadedBytes += coworkingCatalogSessionBytes(session)
        if (loadedBytes > MAX_MATERIALIZED_SESSION_BYTES) {
          throw new Error('coworking_catalog_session_byte_limit_exceeded')
        }
        loaded.push(session)
      }
    ...
    throw new Error('missing_coworking_catalog_session_cursor')
  } catch {
    if (!options.isCurrent()) {
      return catalog
    }
...
function seedLoadingSessions(
  catalog: CoworkingDesktopCatalog,
  previous: CoworkingDesktopCatalog | null
): { catalog: CoworkingDesktopCatalog; budget: CoworkingCatalogSessionBudget } {
  const budget = new CoworkingCatalogSessionBudget()
  const seeded = {
    ...catalog,
    projects: catalog.projects.map((project) => ({
      ...project,
...
function findWorktree(
  ...
  worktree: Pick<CoworkingWorktreeCatalogEntry, 'worktreeRef' | 'shareEpoch'>
): CoworkingWorktreeCatalogEntry | null {
  if (!catalog) {
    return null
  }
...
function replaceWorktree(
  catalog: CoworkingDesktopCatalog,
  worktreeRef: string,
  replacement: CoworkingWorktreeCatalogEntry
): CoworkingDesktopCatalog {
  return {
    ...catalog,
    projects: catalog.projects.map((project) => ({
