...
export class CoworkingPairedRuntimeSessionChangeSubscriptions {
  ...
  constructor(
    ...
    private readonly refreshSnapshot: (
      request: CoworkingExecutionHostSessionReadRequest
    ) => Promise<CoworkingMobileSessionTabsResult | null | undefined>
  ) {}
  ...
  ensure(environmentId: string, request: CoworkingExecutionHostSessionReadRequest): void {
    if (this.listeners.size === 0) {
      return
    }
  ...
  forget(environmentId: string, request: CoworkingExecutionHostSessionReadRequest): void {
    const bindingKey = JSON.stringify([environmentId, request.worktreeId])
  ...
  private open(bindingKey: string, binding: SessionChangesBinding): void {
    ...
    const params = CoworkingPairedRuntimeSubscribeSessionChangesParamsSchema.parse({
  ...
  private handleEvent(
    ...
    const changed = CoworkingPairedRuntimeSessionChangedEventSchema.safeParse(event.response.result)
    if (!changed.success) {
      this.fail(bindingKey, binding)
      return
    }
  ...
  private async refresh(bindingKey: string, binding: SessionChangesBinding): Promise<void> {
    let snapshot: CoworkingMobileSessionTabsResult | undefined
    try {
...
export function pairedRuntimeSessionTarget(request: CoworkingExecutionHostSessionReadRequest) {
  return {
    kind: request.worktreeKind,
    worktreeId: request.worktreeId,
    instanceId: request.worktreeInstanceId,
    coworkingIncarnationId: request.coworkingIncarnationId
  }
}

function sessionTargetIdentity(request: CoworkingExecutionHostSessionReadRequest): string {
