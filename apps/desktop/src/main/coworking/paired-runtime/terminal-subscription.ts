...
export class PairedRuntimeTerminalSubscription implements CoworkingHostSubscription {
  ...
  handleEvent(
    ...
    const parsed = CoworkingPairedRuntimeTerminalEventSchema.safeParse(event.response.result)
    if (!parsed.success) {
      this.handleTransportClose()
      return
    }
