...
export type CoworkingProbeServiceOptions = {
...
export class CoworkingProbeService {
  private readonly attemptsBySource = new Map<string, number[]>()
  private activeProbes = 0
  private readonly now: () => number

  constructor(private readonly options: CoworkingProbeServiceOptions) {
    ...
    try {
      ...
      if (!protocolVersion) {
        writeJson(response, 426, {
          ...
          supportedProtocolVersions: COWORKING_SUPPORTED_PROTOCOL_VERSIONS
        })
        return
      }
...
async function readProbeBody(request: IncomingMessage): Promise<CoworkingProbeRequest> {
  ...
  if (
    !hasExactCoworkingWireKeys(record, ['protocolVersions', 'clientPublicKeyB64']) ||
...
function selectProtocolVersion(versions: readonly number[]): number | null {
  return versions.includes(COWORKING_PROTOCOL_VERSION) ? COWORKING_PROTOCOL_VERSION : null
}
