...
const WSL_CANONICAL_PATH_PREFIX = '__YIRU_COWORKING_CANONICAL_PATH__'
const WSL_DIRECTORY_IDENTITY_PREFIX = '__YIRU_COWORKING_DIRECTORY_IDENTITY__'
const WSL_MISSING_PATH = '__YIRU_COWORKING_MISSING_PATH__'

export type CoworkingWslCanonicalDirectoryResult =
  | { status: 'resolved'; path: string }
  | { status: 'missing' }
  | { status: 'invalid' }
  | { status: 'unavailable' }

export type CoworkingWslDirectoryIdentityResult =
...
export function resolveCoworkingWslCanonicalDirectory(
  uncPath: string
): Promise<CoworkingWslCanonicalDirectoryResult> {
  return runCoworkingWslProbe(
    uncPath,
...
export function inspectCoworkingWslDirectoryIdentity(
  candidatePath: string,
  wslDistro?: string
): Promise<CoworkingWslDirectoryIdentityResult> {
  return runCoworkingWslProbe(
    candidatePath,
...
function runCoworkingWslProbe(
  ...
  return new Promise((resolve) => {
    execFile(
      'wsl.exe',
      ['-d', parsed.distro, '--', 'sh', '-c', script, 'yiru-coworking-path', parsed.linuxPath],
