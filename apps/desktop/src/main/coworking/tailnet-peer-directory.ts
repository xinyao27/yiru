...
export type DiscoveredCoworkingDesktop = {
  desktopRef: string
...
type ReconciledDesktop = {
  desktop: DiscoveredCoworkingDesktop
  missedReconciliations: number
}

export type TailnetPeerDirectory = {
  snapshot(): readonly DiscoveredCoworkingDesktop[]
  subscribe(listener: (snapshot: readonly DiscoveredCoworkingDesktop[]) => void): () => void
  start(): void
  stop(): void
}
...
export class DefaultTailnetPeerDirectory implements TailnetPeerDirectory {
  ...
  private readonly listeners = new Set<(snapshot: readonly DiscoveredCoworkingDesktop[]) => void>()
  ...
  private async probePeer(peer: TailnetNode): Promise<DiscoveredCoworkingDesktop | null> {
...
function desktopsEqual(
  left: DiscoveredCoworkingDesktop,
  right: DiscoveredCoworkingDesktop
): boolean {
  return Object.keys(left).every(
    (key) =>
      left[key as keyof DiscoveredCoworkingDesktop] ===
      right[key as keyof DiscoveredCoworkingDesktop]
  )
}
