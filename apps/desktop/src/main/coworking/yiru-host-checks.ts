...
type CoworkingChecksRuntime = Pick<
  YiruRuntimeService,
  'getHostedReviewForBranch' | 'getRepoPRChecks'
>

type CoworkingChecksGitCommands = Pick<RuntimeGitCommands, 'getRuntimeGitStatus'>

const MAX_CHECKS = 256

/** Projects owner-hosted review state without exposing repository paths or credentials. */
export class YiruCoworkingHostChecks {
  private readonly cache = new CoworkingChecksReadCache()

  constructor(
    private readonly store: Store,
    private readonly runtime: CoworkingChecksRuntime,
    private readonly gitCommands: CoworkingChecksGitCommands
  ) {}

  async invoke(
    target: CoworkingPublicWorktreeInstance,
    _operation: CoworkingChecksReadOperation,
    signal: AbortSignal
  ...
  private async readUncached(
    ...
    try {
      return CoworkingChecksReadResultSchema.parse(await this.readOwnerProjection(target, signal))
    } catch {
  ...
  private async readOwnerProjection(
    ...
    let checkDetails: Awaited<ReturnType<CoworkingChecksRuntime['getRepoPRChecks']>> = []
    let detailStatus: CoworkingChecksReadResult['detailStatus'] = 'unsupported'
    ...
    return {
      review: {
        ...
        state: review.state,
        url: normalizeCoworkingChecksHttpUrl(review.url),
      ...
      checks: checkDetails.slice(0, MAX_CHECKS).map((check) => ({
        ...
        url: check.url ? normalizeCoworkingChecksHttpUrl(check.url) : null
      })),
...
function completeEmptyChecksResult(): CoworkingChecksReadResult {
  return { review: null, checks: [], truncated: false, detailStatus: 'complete' }
}

function unavailableChecksResult(): CoworkingChecksReadResult {
