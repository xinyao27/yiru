...
function getProvenancePath(runtimeHomePath: string): string {
  return join(runtimeHomePath, '.yiru-hook-trust-provenance.json')
}
...
/**
 * Records the runtime config.toml trust state Yiru leaves behind after an
...
/**
 * Promotes hook approvals the user made inside Yiru-launched Codex (written
...
function promoteCodexRuntimeHookApprovalsToSystemUnsafe(runtimeHomePath: string): void {
  ...
  // Why: without a snapshot of what Yiru last wrote (first launch after
  ...
  // in-Yiru approval. Promoting would resurrect trust the user revoked in
  ...
  for (const [key, state] of runtimeTrust) {
    ...
    if (
      ...
      // Yiru wrote this entry and nothing touched it since — not an approval.
      continue
    }
    ...
    const hook = Array.isArray(definition?.hooks)
      ...
      : undefined
    // Why: never write trust for Yiru's managed status hook into the user's
    // real config — mutating ~/.codex for Yiru's own hooks is exactly what the
