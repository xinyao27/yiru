...
export function hasCustomCodexHomeOverride(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  const yiruCodexHome = env.YIRU_CODEX_HOME?.trim()
  const normalizedCodexHome = codexHome ? normalizePathForComparison(codexHome) : undefined
  const normalizedYiruCodexHome = yiruCodexHome
    ? normalizePathForComparison(yiruCodexHome)
    : undefined
  // Why: phase 1 owns only ~/.codex and can clean that path on downgrade. A
  // custom home needs cross-home ownership tracking before Yiru may mutate it.
  return Boolean(
    normalizedCodexHome &&
    normalizedCodexHome !== normalizedYiruCodexHome &&
