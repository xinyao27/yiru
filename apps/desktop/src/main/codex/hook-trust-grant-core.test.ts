...
import { codexAppServerCapabilityCache } from './app-server-capability-cache'
import {
...
import { readCodexTrustGrantLedgerHome } from './trust-grant-ledger'
...
beforeEach(() => {
  userDataDir = mkdtempSync(join(tmpdir(), 'yiru-trust-grant-userdata-'))
...
afterEach(() => {
  ...
  delete process.env.YIRU_DISABLE_CODEX_TRUST_RPC
...
describe('grantManagedCodexHookTrust', () => {
  ...
  it('keeps grant and fallback outcomes stable when telemetry throws', () => {
    ...
    process.env.YIRU_DISABLE_CODEX_TRUST_RPC = '1'
    expect(grantManagedCodexHookTrust(buildPlan(entries))).toMatchObject({
      lane: 'fallback',
      reason: 'disabled'
    })
