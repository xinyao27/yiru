...
import { stderrIndicatesMissingAppServer } from './app-server-capability-signal'
import { waitForProcessExitUntil } from './process-exit-deadline'

// Why: `codex app-server` is Yiru's sanctioned RPC surface into Codex-owned
  ...
  try {
    const session = async (): Promise<T> => {
      await requestRpc('initialize', {
        clientInfo: { name: 'yiru_desktop', title: 'Yiru', version: '0.0.0' }
      })
      notify('initialized')
