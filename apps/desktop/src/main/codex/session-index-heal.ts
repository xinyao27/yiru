...
} from './session-index-heal-state'

export type { CodexSessionIndexHealPaths } from './session-index-heal-state'

// Why: Codex's own sqlite metadata backfill is one-shot (backfill_state is
// stamped `complete` on first app-server startup), so rollouts that Yiru's
...
// Codex's DB-driven surfaces. Yiru never writes Codex's sqlite schema itself.
...
function buildNativeHealInvocation(
  ...
  return {
    command: spawnCmd,
    args: spawnArgs,
    // Why: pin the real home explicitly — nested Yiru launches can inherit a
