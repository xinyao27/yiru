...
beforeEach(() => {
  userDataDir = mkdtempSync(join(tmpdir(), 'yiru-trust-ledger-'))
...
describe('codex trust grant ledger', () => {
  ...
  it('treats Windows path-case variants as the same home', () => {
    const home = 'C:\\Users\\Alice\\AppData\\Roaming\\yiru\\codex-runtime-home\\home'
    writeCodexTrustGrantLedgerHome(home, { binary: null, entries: {} })
    expect(
      readCodexTrustGrantLedgerHome('c:/users/alice/appdata/roaming/yiru/codex-runtime-home/home')
    ).not.toBeNull()
  })
