...
} from './hook-trust-grant'
import { removeCodexManagedHookTrustEntries } from './managed-trust-reconciliation'
import { restoreCodexTrustConfig } from './trust-config-rollback'
 ...
 * - 'removed': hooks are opted out; Yiru entries are swept from the real home.
 */
...
/** Yiru-side state dir; nothing extra is ever written into the user's ~/.codex. */
...
function assertHooksJsonGeneration(
  ...
  if (currentRaw !== expectedRaw || resolveHooksJsonWritePath(hooksJsonPath) !== hooksWritePath) {
    ...
    throw new Error('Codex hooks.json changed while Yiru prepared its trust repair')
  }
}

/**
 * Ensures the real-home hook state matches the settings: installs and trusts
 * the Yiru status hook when enabled, sweeps it when opted out. Idempotent and
...
function installRealHomeCodexHook(userDataPath: string): RealHomeCodexHookLane {
  ...
  // Why: sweep stale Yiru entries out of events the managed lane no longer
  ...
  // Why: never leave an untrusted Yiru entry in the user's real home — it
...
function reconcileManagedHookDefinition(
  ...
  if (!hasManagedDirectCommand && nestedLocations.length === 1) {
    ...
    if (definition.matcher === undefined && !hasDirectCommand) {
      ...
      // Why: users can append groups or handlers after Yiru's first install.
...
function sweepRealHomeCodexHook(): RealHomeCodexHookLane {
  ...
  if (removedAny) {
    ...
    // Why: dead [hooks.state] blocks for a removed hook are Yiru-owned records;
    ...
    // groups must never make Yiru delete a user's trust record at the same key.
    try {
      ...
      console.warn('[codex-real-home-hooks] failed to drop Yiru trust entries:', error)
    }
  }
  return 'removed'
}

/** One-time pristine copy of the user's file, kept under Yiru's userData. */
function backupRealHomeHooksJsonOnce(userDataPath: string, previousRaw: string | null): void {
  ...
  const backupPath = join(backupDir, 'hooks.json.pre-yiru')
