// Grok chat_history.jsonl line → AgentTranscriptMessage decoder.
...
import {
  ...
  parseJsonObject,
  timestampMs
} from '~main/ai-vault/session/scanner-values'
...
export function decodeGrokTranscriptLine(
  ...
  // every row with its JSONL position so Agent transcript output preserves transcript order.
...
function grokToolCallBlocks(value: unknown): AgentTranscriptBlock[] {
...
function normalizeGrokUserQueryBlock(block: AgentTranscriptBlock): AgentTranscriptBlock[] {
  if (block.type !== 'text') {
    return [block]
  }
...
function splitGrokPastedImageQuery(text: string): { path: string; query: string } | null {
  ...
  // concatenated with the prompt; recover Yiru's attachment without exposing it.
