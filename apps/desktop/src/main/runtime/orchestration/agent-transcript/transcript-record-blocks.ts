// Centralized record→block mapping for agent transcripts. Kept separate
...
import { asRecord, extractString } from '~main/ai-vault/session/scanner-values'
...
export function claudeContentBlocks(content: unknown): AgentTranscriptBlock[] {
...
function claudeContentBlock(record: Record<string, unknown>): AgentTranscriptBlock | null {
  switch (record.type) {
    case 'text': {
...
function toolResultBlock(record: Record<string, unknown>): AgentTranscriptToolResultBlock {
...
function imageRefBlock(record: Record<string, unknown>): AgentTranscriptImageRefBlock | null {
