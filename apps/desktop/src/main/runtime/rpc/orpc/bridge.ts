...
export type RuntimeOrpcContextOptions = RuntimeOrpcConnectionState &
  Pick<RpcContext, 'mobileDevelopmentPairing'> & {
...
export function createRuntimeOrpcContext(
  ...
  return {
    ...
    get browserCommands() {
      return runtime.browserCommands
    },
...
export async function invokeRuntimeOrpcOperation<TInput, TOutput>(
  ...
  try {
    ...
    const redirectDenial = await adjudicateRedirectedProjectAccess(method, input, context)
    if (redirectDenial) {
      throw new ORPCError(redirectDenial.code, {
        status: redirectDenial.code === 'forbidden' ? 403 : 401,
        message: redirectDenial.message
      })
    }
...
function runtimeOrpcFeatureInteractionInput(
  ...
  return metadata.featureInteractionSource === YIRU_RUNTIME_RPC_BROWSER_UI_SOURCE
    ? withBrowserPaneUiRuntimeRpcSource(input)
    : input
}
