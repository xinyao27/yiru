...
const lifetimesByRuntime = new WeakMap<YiruRuntimeService, CoworkingHostChannelLifetimes>()

/** Binds owner-side logical channels to the physical paired-runtime socket that introduced them. */
export class CoworkingHostChannelLifetimes {
  ...
  private cleanupId(connectionId: string): string {
    return `coworking.host.channels:${connectionId}`
  }
}

export function getCoworkingHostChannelLifetimes(
  runtime: YiruRuntimeService
): CoworkingHostChannelLifetimes {
  const existing = lifetimesByRuntime.get(runtime)
  if (existing) {
    return existing
  }
  const created = new CoworkingHostChannelLifetimes()
