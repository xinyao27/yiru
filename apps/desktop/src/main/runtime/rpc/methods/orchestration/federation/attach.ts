import type * as orchestrationContract from '@yiru/runtime-protocol/contract'
...
import { failFederatedAttachmentWithReceipt } from './start-receipt'

export async function handleOrchestrationFederationAttachStart(
  params: orchestrationContract.OrchestrationFederationAttachStartInput,
  ...
  try {
    ...
    await runtime.sendTerminalAgentPrompt(
      ...
      buildDispatchPreamble({
        ...
        coordinatorHandle: 'Run home (relayed by Yiru)',
...
// Why: `federationAttachStart` is retired here (slice 84, Part B) — its only
// caller (`startFederatedWorker`) now passes
// `ORCHESTRATION_FEDERATION_ATTACH_START_CONTRACT` and its
// `orchestrationRequestId` envelope rides the oRPC tunnel as headers
// (`environment-orpc-unary-client.ts`'s `buildRuntimeOrpcCallHeaders`), so it
// reaches this same handler through the direct-wired oRPC route
// (`router-direct/orchestration.ts`) once the peer's oRPC tunnel is
// confirmed, falling back to the bare envelope for an older peer that still
// speaks the legacy dispatcher.
