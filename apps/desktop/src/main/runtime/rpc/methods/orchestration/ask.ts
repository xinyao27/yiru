...
export async function handleOrchestrationAsk(
  params: OrchestrationAskInput,
  { runtime, signal, orchestrationCapability, recordMutationReceipt }: RpcContext
  ...
  if (!activeDispatch) {
    throw new OrchestrationError('dispatch_inactive', 'ask requires an active supervised Dispatch.')
  }
  ...
  if (params.resume) {
    ...
    if (!run || run.legacy === 1) {
      throw new OrchestrationError('run_not_found', `Run ${activeDispatch.run_id} was not found.`)
    }
