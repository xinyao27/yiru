...
import { authorizeExternalPath, isENOENT } from './auth'
import {
  captureLocalUploadRoot,
  preScanSshImportDirectory,
  uploadSshImportDirectory
} from './import-ssh-directory'
import type { ImportItemResult } from './mutations'
...
export async function importExternalPathsSsh(
  ...
  if (options?.ensureDir) {
    // Why: terminal-drop staging needs `${worktree}/.yiru/drops` to exist
    // before the first upload. .yiru/ is reserved as Yiru-owned remote state;
