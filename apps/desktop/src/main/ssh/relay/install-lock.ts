...
import { removeRemoteTreeCommand } from '../remote/commands'
import { getRemoteHostPlatform, joinRemotePath, type RemoteHostPlatform } from '../remote/platform'
import { execCommand } from './deploy-helpers'
import { RELAY_DEPLOY_TIMEOUT_MS } from './deploy-timing'
import { isRelayGcClaimed, waitForRelayGcClaimRelease } from './gc-claim'
import {
  acquireInstallLockParentCommand,
  lockAgeSecondsCommand,
  tryCreateInstallLockCommand,
  tryStealInstallLockCommand
} from './install-lock-commands'
...
export async function isRelayInstallLockStale(
  ...
  try {
    // Why: remote time avoids clock skew between Yiru clients making a live
 ...
 * one winner while keeping the marker visible to older Windows Yiru clients.
 */
