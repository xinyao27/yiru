...
import { RemoteCliArgumentError, type ParsedRemoteCli } from './remote-cli-argument-error'
import { formatRemoteCli } from './remote-cli-format'
import {
  HostCliUnavailableError,
  runHostYiruCliPassthrough,
  type RemoteYiruCliRequest,
  type RemoteYiruCliResult
} from './remote-cli-host-passthrough'
import {
  getRemoteOrchestrationPayload,
  hasRemoteLifecycleRejection,
  resolveRemoteOrchestrationSender
} from './remote-orchestration-send'

export type { RemoteYiruCliRequest, RemoteYiruCliResult } from './remote-cli-host-passthrough'
...
const HOST_INTERACTIVE_COMMANDS: Record<string, string> = {
  serve:
    'yiru serve starts a foreground headless Yiru server and cannot run through the SSH relay bridge. Run it directly on the machine that should host Yiru.',
  'claude-teams':
    'yiru claude-teams starts an interactive Claude Code session and cannot run through the SSH relay bridge. Run it in a terminal on the Yiru host machine.',
  'agent-teams-tmux':
    'yiru agent-teams-tmux is a tmux pane shim for the Yiru host machine and cannot run through the SSH relay bridge.'
}
...
export async function runRemoteYiruCli(
  ...
  return await runLegacyRemoteYiruCli(runtime, request, parsed, json, passthroughFailure)
}

async function runLegacyRemoteYiruCli(
  runtime: YiruRuntimeService,
  request: RemoteYiruCliRequest,
...
function resolveHandle(
  ...
  return optionalString(flags, flagName) ?? env.YIRU_TERMINAL_HANDLE ?? 'unknown'
}
