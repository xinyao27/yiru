...
import type { SshResolvedConfig } from './config-parser'

export { findDefaultKeyFile, resolveAgentSocket } from './auth-resolution'
...
export function wrapRemoteCommandForPosixShell(command: string): string {
  ...
  return `exec /bin/sh -c ${shellEscape(decodeAndRun)} yiru-command ${chunkArguments}`
}
