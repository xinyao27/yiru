 ...
 * internally but does not expose the native-module selection needed by Yiru.
 * Explicitly rebuilding the modules that Yiru owns keeps the postinstall
 * path deterministic across platforms.
 *
 ...
 * @electron/rebuild's JS API directly so that the patched node-pty source is
 * rebuilt against Electron.
 */
...
const onlyModules = NATIVE_MODULES
const forceRebuild =
  process.env.YIRU_FORCE_NATIVE_REBUILD === '1' ||
  cliOptions.force ||
...
function continuePostinstallWithoutElectron() {
  if (!isPostinstall() || process.env.YIRU_STRICT_ELECTRON_INSTALL === '1') {
    return false
  }
  console.error(
...
function getPatchedNodePtyRebuildReason() {
  ...
  // Why: Yiru patches node-pty's native Unix spawn path; upstream prebuilds can
    ...
    if (requirePatchedNodePtySourceBuild && !isNodePtyReleaseBuildDir(native.dir)) {
      throw new Error(
        'node-pty resolved to ' +
          native.dir +
          '; expected build/Release so Yiru\\'s node-pty patch is active'
      )
    }
    return
  }
