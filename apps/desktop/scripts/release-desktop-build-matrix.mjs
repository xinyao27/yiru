...
const WINDOWS_RELEASE_BUILD = {
  ...
  platform: 'win',
  release_command:
    'pnpm --filter yiru run ensure:electron-runtime; if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }; pnpm --filter yiru exec electron-builder --config config/electron-builder.config.cjs --win --publish never',
...
const LINUX_RELEASE_BUILDS = [
  {
    ...
    platform: 'linux-x64',
    release_command:
      'pnpm --filter yiru run ensure:electron-runtime && pnpm --filter yiru exec electron-builder --config config/electron-builder.config.cjs --linux AppImage deb rpm --x64 --publish always',
  ...
  },
  {
    ...
    release_command:
      'pnpm --filter yiru run ensure:electron-runtime && YIRU_LINUX_ARM64_RELEASE=1 pnpm --filter yiru exec electron-builder --config config/electron-builder.config.cjs --linux AppImage deb rpm --arm64 --publish always',
    eb_cache_path: '~/.cache/electron\n~/.cache/electron-builder'
  }
]
