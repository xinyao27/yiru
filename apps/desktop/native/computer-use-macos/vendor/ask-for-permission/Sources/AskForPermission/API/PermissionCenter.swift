...
public final class PermissionCenter {
    ...
    public init(
        appName: String? = nil,
        permissionKinds: Set<PermissionKind> = Set(PermissionKind.allCases)
    ) throws {
        let resolvedBundleURL = try Self.resolveHostApplicationBundle()
        let resolvedAppName = appName
            ?? (Bundle.main.object(forInfoDictionaryKey: "CFBundleDisplayName") as? String)
            ?? (Bundle.main.object(forInfoDictionaryKey: "CFBundleName") as? String)
            ?? translate("permission.app-name.fallback", fallback: "This App")

        self.bundleURL = resolvedBundleURL
        self.appName = resolvedAppName
        self.state = PermissionStatusModel(permissionKinds: permissionKinds)
    ...
    private static func resolveHostApplicationBundle() throws -> URL {
        ...
        throw PermissionRequestError(
            ...
            message: translate(
                "permission.error.app-bundle-required",
                fallback: "AskForPermission must run from a .app bundle so the host app can be dragged into System Settings."
            )
        )
    }
}
