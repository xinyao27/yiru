...
final class SystemSettingsWindowTracker {
    ...
    func waitForWindow(timeout: Duration = .seconds(6)) async throws -> CGRect {
        ...
        throw PermissionRequestError(
            ...
            message: translate(
                "permission.error.settings-window-timeout",
                fallback: "System Settings window did not appear within the expected time."
            )
        )
    }
    ...
    func currentWindowFrame() -> CGRect? {
        ...
        for info in infoList {
            ...
            guard isSystemSettingsApplication(ownerName: ownerName, ownerPID: ownerPID) else {
                continue
            }
