        ...
        VStack(alignment: .leading, spacing: 6) {
            Text(
                Bundle.main.object(forInfoDictionaryKey: "CFBundleName") as? String
                    ?? translate("permission.window.title", fallback: "Permissions")
            )
                .font(.system(size: 20, weight: .semibold))
            Text(translate(
                "permission.window.description",
                fallback: "This app needs these permissions to work on your Mac."
            ))
                .font(.system(size: 12))
                .foregroundStyle(.secondary)
            Text(translate(
                "permission.window.privacy",
                fallback: "These permissions are only used while you use this app."
            ))
                .font(.system(size: 12))
                .foregroundStyle(.secondary)
        }
