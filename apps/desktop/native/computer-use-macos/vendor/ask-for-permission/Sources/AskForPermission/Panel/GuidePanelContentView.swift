...
struct GuidePanelContentView: View {
    ...
    static let contentSize = CGSize(width: 540, height: 138)
    ...
    var body: some View {
        ZStack(alignment: .topLeading) {
            ...
            GuideBackButton(action: onBack)
                .padding(.leading, 10)
                .padding(.top, (Self.contentSize.height - 32) / 2)
        }
    ...
    private var captionText: Text {
        Text(translate("permission.guide.drag-prefix", fallback: "Drag "))
            + Text(appName).fontWeight(.semibold)
            + Text(translate(
                "permission.guide.allow-middle",
                fallback: " to the list above to allow "
            ))
...
private struct GuideBackButton: View {
    ...
    var body: some View {
        ...
        .buttonStyle(.plain)
        .accessibilityLabel(translate("permission.action.back", fallback: "Back"))
