...
public enum AskForPermission {
    ...
    private static var configuredPermissionKinds = Set(PermissionKind.allCases)
    ...
    /// affects subsequent requests. `permissionKinds` limits status polling
    /// to the permissions the host exposes.
    public static func configure(
        appName: String,
        permissionKinds: Set<PermissionKind> = Set(PermissionKind.allCases)
    ) {
        configuredAppName = appName
        configuredPermissionKinds = permissionKinds
        cachedCenter = nil
    }
    ...
    private static func resolveCenter() -> CachedCenter {
        ...
        do {
            let center = try PermissionCenter(
                ...
                permissionKinds: configuredPermissionKinds
            )
