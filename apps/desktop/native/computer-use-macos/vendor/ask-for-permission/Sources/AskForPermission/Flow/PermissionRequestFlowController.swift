...
final class PermissionRequestFlowController {
    ...
    func run(
        ...
        panel.draggableView?.onDragEnded = { [weak self] operation, screenPoint in
            guard let self else { return }
            let settingsFrame = self.convertToAppKitCoordinates(latestSettingsFrame)
            let didDropOnSettings = !operation.isEmpty && settingsFrame.contains(screenPoint)
            let outcome: DragOutcome = didDropOnSettings ? .dropped : .userCancelled
            self.dragContinuation?.resume(returning: outcome)
    ...
    private func activateSystemSettings() {
        ...
        guard let settings = bundleIdentifiers.lazy.compactMap({ bundleIdentifier in
