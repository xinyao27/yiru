...
function skillsTreeShasAtRefs(refs) {
  ...
  if (lines.length !== requests.length) {
    throw new Error(`Expected ${requests.length} skills tree identities, received ${lines.length}`)
  }

  return refs.map((ref, refIndex) => {
    for (const [pathIndex, skillsPath] of RELEASED_SKILLS_PATHS.entries()) {
      const line = lines[refIndex * RELEASED_SKILLS_PATHS.length + pathIndex]
      if (line.endsWith(' missing')) {
        continue
      }
      const match = /^([a-f0-9]+) tree$/.exec(line)
      if (!match) {
        throw new Error(`Unexpected skills tree identity at ${ref}:${skillsPath}: ${line}`)
      }
      return match[1]
    }
    return null
...
function buildReleasedHistory(currentSkillNames = new Set(), retainedSkillNames = null) {
  ...
  for (const [index, tag] of tags.entries()) {
    ...
    for (const sourceName of [...packages.keys()].sort(compareCodeUnits)) {
      ...
      const entries = packages.get(sourceName)
      const filesWithGitHashes = collectGitPackageFiles(skillsTreeSha, sourceName, entries, blobs)
