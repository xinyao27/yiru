...
const BASELINE_PATH = 'apps/desktop/config/max-lines-baseline.txt'
...
function printAddedFailure(added) {
  ...
  console.error('')
  console.error('  Yiru caps file size (300 .ts / 400 .tsx / 600 .mjs / 800 test — non-blank,')
  console.error(
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  // Why: the gate is launched from the desktop workspace but owns repository-wide
  // debt, including mobile ratchets, so Git's top-level is the stable scan root.
