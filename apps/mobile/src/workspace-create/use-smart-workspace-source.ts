...
export type SmartCrossRepoPrompt = {
  link: { slug: { owner: string; repo: string }; number: number; type: 'pr' }
...
export function useSmartWorkspaceSource(args: UseSmartWorkspaceSourceArgs) {
  ...
  }, [client, enabled, mode, query, repoId, githubAvailable, gitlabAvailable, mrStateFilter, repos])
  ...
  const rows = useMemo<SmartWorkspaceSourceRow[]>(
    ...
    [fan, gitlabAvailable, mode, paste, query]
  )
