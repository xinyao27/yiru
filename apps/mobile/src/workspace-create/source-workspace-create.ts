...
function toReviewItem(item: MobileLinkedWorkItem, targetRepoId: string): WorkspaceCreateReviewItem {
  return {
...
async function createWorkItemWorkspace(args: {
  ...
  const reviewItem = toReviewItem(item, targetRepoId)
  ...
  const params = buildReviewWorkspaceCreateParams({
    item: reviewItem,
    setupDecision,
    agent: agent.choice,
    workspaceName,
  ...
  // buildReviewWorkspaceCreateParams computes the name; reuse it as the retry base
