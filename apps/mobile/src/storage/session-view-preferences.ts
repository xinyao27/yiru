...
const DEFAULT_SESSION_VIEW_KEY = 'yiru:defaultSessionView'
const SESSION_VIEW_OVERRIDES_PREFIX = 'yiru:nativeChatTabs:'
...
function sessionViewOverridesKey(hostId: string, worktreeId: string): string {
  return `${SESSION_VIEW_OVERRIDES_PREFIX}${encodeURIComponent(hostId)}:${encodeURIComponent(worktreeId)}`
}
