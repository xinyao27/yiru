...
/** Resolves each tab's terminal/chat view from a device default and per-tab overrides. */
export function useMobileSessionViewMode(args: {
  ...
  const toggleTabChatView = useCallback(
    (tabId: string) => {
      ...
      // Flip from the tab's effective view so a tab following a chat default can
      // still be pinned back to terminal.
