...
export type MobileTerminalSideEffectFact =
...
export type MobileTerminalSideEffectBatch = {
  facts: MobileTerminalSideEffectFact[]
  replay: boolean
}

export function decodeMobileTerminalSideEffectBatch(
  payload: Uint8Array<ArrayBufferLike>
): MobileTerminalSideEffectBatch | null {
  const value = decodeTerminalMultiplexJson(payload)
  if (!value || !Array.isArray(value.facts) || typeof value.replay !== 'boolean') {
    return null
  }
  const facts: MobileTerminalSideEffectFact[] = []
...
function decodeFact(value: unknown): MobileTerminalSideEffectFact | null {
