...
enum LegacyMobileRelayStateCleanup {
    ...
    static func perform(defaults: UserDefaults = .standard) {
        ...
        do {
            ...
            // Why: preserve the old AsyncStorage files during the supported upgrade window.
            // This marker only prevents repeated harmless Keychain deletes on every launch.
            defaults.set(true, forKey: nativeMarkerKey)
        } catch {
