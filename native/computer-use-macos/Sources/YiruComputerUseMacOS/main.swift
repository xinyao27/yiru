...
private final class PermissionWindowController: NSWindowController {
    ...
    convenience init(initialPermission: PermissionKind? = nil, terminateWhenDragAssistantCloses: Bool = false) {
        ...
        window.title = "Enable Yiru Computer Use"
...
private enum PermissionKind: CaseIterable {
    ...
    var dragInstruction: String {
        switch self {
        case .accessibility:
            "Drag Yiru Computer Use into the list above to allow Accessibility."
        case .screenshots:
            "Drag Yiru Computer Use into the list above to allow Screenshots."
        }
    }
...
private final class PermissionView: NSView {
    ...
    private func build() {
        ...
        let title = label(ready ? "Computer Use is Ready" : "Enable Yiru Computer Use", size: 22, weight: .bold)
        let subtitle = label(
            ready ? "Yiru can use local apps when you ask." : "Grant permissions so Yiru can use apps when you ask.",
            size: 12,
            weight: .regular
        )
    ...
    private func permissionRow(permission: PermissionKind, action: @escaping () -> Void) -> NSView {
        ...
        objc_setAssociatedObject(button, "yiru-action", target, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
...
private final class PermissionDragAssistantController: NSWindowController {
    ...
    convenience init(
        ...
        window.title = "Drag Yiru Computer Use"
...
private final class PermissionDragAssistantView: NSView {
    ...
    private func build() {
        ...
        objc_setAssociatedObject(closeButton, "yiru-action", target, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
...
private final class DraggableAppTile: NSView, NSDraggingSource {
    ...
    private func build() {
        ...
        let title = NSTextField(labelWithString: "Yiru Computer Use")
...
private func isAuthorizedAgentPeer(_ pid: pid_t) -> Bool {
    ...
    if isTrustedYiruApplication(pid) {
        return true
    }
    guard let parentPid = parentProcessId(pid) else { return false }
    return isTrustedYiruApplication(parentPid)
}

private func isTrustedYiruApplication(_ pid: pid_t) -> Bool {
    guard let app = NSRunningApplication(processIdentifier: pid),
