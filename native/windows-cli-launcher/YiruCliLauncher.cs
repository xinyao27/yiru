...
internal static class YiruCliLauncher
{
    private static int Main(string[] args)
    {
        try
        {
            string launcherDirectory = Path.GetDirectoryName(typeof(YiruCliLauncher).Assembly.Location);
            ...
            string electronPath = Path.Combine(appDirectory, "Yiru.exe");
            ...
            {
                Console.Error.WriteLine("Unable to locate Yiru.exe next to \"{0}\"", resourcesDirectory);
                return 1;
            }

            if (!File.Exists(cliPath))
            {
                Console.Error.WriteLine("Unable to locate the Yiru CLI entrypoint at \"{0}\"", cliPath);
                return 1;
            }
            ...
            MoveEnvironmentVariable(
                ...
                "YIRU_NODE_REPL_EXTERNAL_MODULE"
            );
        ...
        {
            Console.Error.WriteLine("Unable to start the Yiru CLI: {0}", error.Message);
            return 1;
        }
    }
