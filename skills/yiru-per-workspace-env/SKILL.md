---
name: yiru-per-workspace-env
description: >-
  Set up, review, debug, or validate Yiru per-workspace environment recipes —
  ...
  per-workspace environments, fix an `environmentRecipes` entry in `yiru.yaml`, scaffold
  provider lifecycle scripts, or resolve an `yiru vm recipe doctor` failure.
---
...
Yiru is a **thin wrapper**: you guide, detect, and scaffold; you never own the user's cloud account,
...
**The one branch that shapes everything — connection mode:** **Yiru-server** (`create` runs `yiru serve`
in the env and emits a `pairingCode`; §7c/§7f) vs **SSH** (`create` runs no server and emits a
`connection.type:"ssh"` block Yiru dials into; §7g/§7h). Settle this first — it changes the `create`
output shape and half the templates.

**Quick-start (happy path):** interview the user (connection mode Yiru-server vs SSH, provider, agent CLI,
git auth — §1.2) + read the provider's CLI docs → scaffold `scripts/yiru-vm/` from §7 → run the
base-snapshot script, then the auth script (you invoke these by hand; not via `yiru.yaml`) → wire
`environmentRecipes` in `yiru.yaml` → `yiru vm recipe doctor <id> --json` (free) → then the `--provision`
...
a long time, or need the user at the keyboard. Never create a Yiru workspace or commit unless asked.

1. **Inspect the repo** for an existing `environmentRecipes` entry, `scripts/yiru-vm/`, a state file, or setup
...
2. **Interview the user up front** — gather these choices and confirm them back before scaffolding
   ...
   - **Connection mode:** how Yiru attaches to the environment — an **Yiru server** (the VM runs
     `yiru serve` and Yiru pairs over its pairing URL; worked example §7f) or **SSH** (Yiru connects to
   ...
   - **Provider:** Vercel Sandbox, Fly, Modal, an existing SSH host, … For non-obvious providers, also
     ...
     (host/port/user/key or proxy command) or only a provider-mediated interactive shell; Yiru SSH mode
     needs the former.
...
7. **Wire the recipe** so `yiru.yaml` points create/suspend/resume/destroy at the scripts (§8). The
   workspace composer reads `environmentRecipes` from the project's primary checkout of `yiru.yaml`, **not** from
   a feature branch or worktree. So a recipe added only on a branch won't appear as a "Run on" option
   until that `yiru.yaml` change is committed and merged to the project's primary branch. Tell the user
...
8. **Dry-run doctor** — `yiru vm recipe doctor <recipe-id> --repo-path <repo> --json` (free, static; §9).
   Fix every failure before going live.
9. **[CHECKPOINT] Live self-test** — get the user's OK once, then run
   `yiru vm recipe doctor <recipe-id> --provision --json` as a loop: it runs create → validates →
...
- **Connection mode** (Yiru server vs SSH) confirmed with the user — see §1 step 2; it shapes the recipe.
...
A repo-local JSON file (e.g. `scripts/yiru-vm/<provider>-state.json`) threads non-secret values between
...
Scaffold under `scripts/yiru-vm/`. These are **shapes** — fill in the provider's real commands. All
...
- **Local-side** (`create`/`suspend`/`resume`/`destroy` + the base-snapshot/auth scripts the user
  ...
  or require WSL/Git-Bash and point `yiru.yaml` at the right launcher.
...
Worked Vercel commands for this phase are in §7f. You run this script by hand (not via `yiru.yaml`),
...
# name = yiru-${YIRU_VM_RECIPE_ID}-${YIRU_VM_INSTANCE_ID} (sanitized, length-capped)
...
# 3. remote exec: start yiru serve in the background and read the recipe JSON it writes (see below)
...
**The exact `yiru serve` invocation and its output (verified — do not improvise the flags).** Inside the
VM, run:
...
**Binary name:** in a VM built from source (the Phase-2 flow), run it as `pnpm exec yiru-dev serve …`
from the repo root — `yiru-dev` is the in-repo entrypoint and is what the §7f example uses. Plain
`yiru serve …` is the same command when the built CLI is installed on the VM's PATH. The flags/output
...
```json
{ "schemaVersion": 1, "pairingCode": "<yiru pairing URL>", "projectRoot": "<the --project-root you passed>" }
```
...
#!/usr/bin/env bash
set -euo pipefail
payload="$(cat)"                       # Yiru passes lifecycle JSON on stdin
...
# destroy: provider remove "$resource_id"   (or set destroy: none in yiru.yaml)
```
...
name="yiru-${YIRU_VM_RECIPE_ID:-vercel-sandbox}-${YIRU_VM_INSTANCE_ID:-$(date +%s)}"  # sanitize+cap to 63 chars
...
vercel sandbox exec "$name" "${vercel_args[@]}" --timeout 20m \
  --env "GH_TOKEN=$gh_token" --env "YIRU_PROJECT_ROOT=$project_root" \
  --env "YIRU_REPO_URL=$repo_url" --env "YIRU_REPO_REF=$repo_ref" \
  -- bash -lc 'set -euo pipefail; cd "$YIRU_PROJECT_ROOT"; \
    ...
    git fetch origin "$YIRU_REPO_REF"; \
    git checkout -B "$YIRU_REPO_REF" FETCH_HEAD; \
    rm -f /tmp/askpass.sh; \
    c="$(git rev-parse HEAD)"; [ -f .yiru-built ] && [ "$(cat .yiru-built)" = "$c" ] || { \
      pnpm install --prefer-offline && pnpm run build:cli && \
      node scripts/run-electron-vite-build.mjs --config config/electron-vite.vm-serve.config.ts && \
      printf "%s" "$c" > .yiru-built; }' >&2

# 3. (remote) start yiru serve in the background, writing recipe JSON to a file; poll until it parses
recipe_json="$(vercel sandbox exec "$name" "${vercel_args[@]}" --timeout 60s \
  --env "YIRU_PORT=$port" --env "YIRU_PROJECT_ROOT=$project_root" --env "YIRU_PAIRING_ADDRESS=$pairing_ws" \
  -- bash -lc 'set -euo pipefail; cd "$YIRU_PROJECT_ROOT"; rm -f /tmp/yiru-recipe.json /tmp/yiru-serve.log; \
    nohup pnpm exec yiru-dev serve --port "$YIRU_PORT" --project-root "$YIRU_PROJECT_ROOT" \
      --pairing-address "$YIRU_PAIRING_ADDRESS" --recipe-json >/tmp/yiru-recipe.json 2>/tmp/yiru-serve.log </dev/null & \
    pid=$!; for _ in $(seq 1 80); do \
      node -e "JSON.parse(require(\"node:fs\").readFileSync(\"/tmp/yiru-recipe.json\",\"utf8\"))" >/dev/null 2>&1 && { cat /tmp/yiru-recipe.json; exit 0; }; \
      kill -0 "$pid" 2>/dev/null || { cat /tmp/yiru-serve.log >&2; exit 1; }; sleep 0.25; \
    done; cat /tmp/yiru-serve.log >&2; echo "serve recipe JSON timed out" >&2; exit 1')"
...
`userData.resourceId` from stdin (§7d). This is the **Yiru-server** connection mode (the recipe emits a
...
- **`create` does NOT run `yiru serve` and does NOT emit a `pairingCode`.** Yiru itself connects to the
  host over its SSH relay, brings up the git + filesystem providers, and imports the repo. The script's
  only job is to make the host ready and **print SSH connection details** Yiru will dial.
- The result uses a `connection` block with `type: "ssh"` and a `target`, **not** the flat
  `pairingCode`/`projectRoot` shape. Exact shape (Yiru rejects anything else):
...
- `relayGracePeriodSeconds` (optional): how long Yiru keeps the SSH relay alive after the workspace
...
# 1. ensure the repo is present and at the right commit on the host (NO yiru serve here)
...
# 2. print the SSH connection block (NO pairingCode, NO yiru serve). host/port/username tell Yiru's
...
`destroy: none` and omit suspend/resume. (Yiru still disconnects/reconnects its own SSH relay on
...
`connection.type:"ssh"` block above instead of starting `yiru serve`.
...
- If committing from an interactive shell, force the runtime entrypoint back to `sshd`:
  `docker commit --change='ENTRYPOINT ["/usr/local/bin/yiru-docker-ssh-entrypoint"]' …`.
...
docker run -d --name "$name" -p 127.0.0.1::22 -e "YIRU_SSH_PUBLIC_KEY=$pubkey" "$auth_image"
...
require WSL/Git-Bash (and point `yiru.yaml` at e.g. `bash ./scripts/yiru-vm/<name>.sh` via a `.cmd`
...
# Yiru-server mode: @{ schemaVersion=1; pairingCode=$pairingCode; projectRoot=$projectRoot; userData=@{...} }
...
environmentRecipes:
  - id: cloud-sandbox
    name: Cloud Sandbox
    create: ./scripts/yiru-vm/cloud-sandbox-create.sh
    suspend: ./scripts/yiru-vm/cloud-sandbox-suspend.sh
    resume: ./scripts/yiru-vm/cloud-sandbox-resume.sh
    destroy: ./scripts/yiru-vm/cloud-sandbox-destroy.sh
```
...
**Yiru-server mode** — boot the env, start `yiru serve` in it, and print serve's result:

```json
{
  "schemaVersion": 1,
  "pairingCode": "yiru-pairing-code-or-url",
  "projectRoot": "/absolute/path/to/repo/on/remote",
  "userData": { "provider": "example", "resourceId": "provider-resource-id" }
}
```

Here `pairingCode` (from `yiru serve --recipe-json`) and `projectRoot` are required; `schemaVersion` (`1`)
and `userData` are optional.

**SSH mode** — do **not** run `yiru serve`; print the `connection.type:"ssh"` block instead (full shape +
...
Start Yiru remotely with `yiru serve --port "$PORT" --project-root "$ABS_ROOT" --pairing-address
...
`yiru vm recipe doctor <recipe-id> --repo-path <repo> --json` validates **static wiring only** — it does
...
`yiru vm recipe doctor <recipe-id> --repo-path <repo> --provision --json` actually runs the recipe end
...
- Don't make Yiru own provider lifecycle beyond invoking the configured scripts.
- Don't commit or create a Yiru workspace unless asked.
