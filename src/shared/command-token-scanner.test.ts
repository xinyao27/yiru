...
describe('command token scanner', () => {
  ...
  it('preserves quoted command paths with spaces', () => {
    expect(getFirstCommandToken('"C:\\Program Files\\Yiru\\codex.cmd" --resume')).toBe(
      'C:\\Program Files\\Yiru\\codex.cmd'
    )
  })

  it('extracts path basenames without allocating path segment arrays', () => {
    expect(getCommandTokenPathBasename('C:\\Program Files\\Yiru\\codex.cmd')).toBe('codex.cmd')
