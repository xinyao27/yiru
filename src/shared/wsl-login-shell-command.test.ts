...
describe('wsl login shell command helpers', () => {
  ...
  it('runs commands through the distro user login shell', () => {
    ...
    expect(command).toContain('exec "$_yiru_wsl_shell" -ilc')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      const root = mkdtempSync(join(tmpdir(), 'yiru-wsl-login-codex-'))
      ...
      writeFileSync(
        loginShell,
        '#!/bin/sh\nexport PATH="$YIRU_TEST_CODEX_BIN:/usr/bin:/bin"\nexec /bin/sh -c "$2"\n'
      )
  ...
  it('preserves command-scoped environment variables through the outer WSL shell', () => {
    const command = buildWslLoginShellCommand('HISTFILE=/tmp/yiru-history printf "$HISTFILE"')
    const escaped = escapeWslShCommandForWindows(command)

    expect(command).toContain('\'HISTFILE=/tmp/yiru-history printf "$HISTFILE"\'')
    expect(escaped).toContain('\\$_yiru_wsl_shell')
  ...
  it('preserves user command variables across the Windows-to-WSL argv boundary', () => {
    ...
    const command = buildWslLoginShellCommand('yiru_value=ok; printf "<%s>" "$yiru_value"')
    const escaped = escapeWslShCommandForWindows(command)
  ...
  it('starts an interactive login shell without assuming bash', () => {
    ...
    expect(command).toContain('getent passwd')
    expect(command).toContain('if [ -z "$_yiru_wsl_shell" ] || [ ! -x "$_yiru_wsl_shell" ]; then')
    expect(command).toContain('_yiru_shell_ready_root=""')
    expect(command).toContain('if [ -n "${YIRU_USER_DATA_PATH:-}" ]; then')
    expect(command).toContain('_yiru_wsl_shell_name=$(basename "$_yiru_wsl_shell"')
    expect(command).toContain('bash)')
    expect(command).toContain('--rcfile "${_yiru_shell_ready_root}/bash/rcfile"')
    expect(command).toContain('zsh)')
    expect(command).toContain('export ZDOTDIR="${_yiru_shell_ready_root}/zsh"')
    expect(command).toContain('exec "$_yiru_wsl_shell" -l')
    expectValidShSyntax(command)
  })
})
