...
describe('tui agent startup plans', () => {
  ...
  it.each([
    ...
    expect(plan?.env?.YIRU_HERMES_STARTUP_QUERY).toBe(prompt)
    const script =
    ...
    expect(script).toContain(
      testCase.shell === 'posix'
        ? '--query=${__yiru_hermes_startup_query}'
        : 'Remove-Item Env:YIRU_HERMES_STARTUP_QUERY'
    )
    if (testCase.shell === 'posix') {
      expect(plan?.launchCommand).toContain('unset YIRU_HERMES_STARTUP_QUERY')
    }
  })
  ...
  it('moves Hermes command override flags after the chat subcommand', () => {
    ...
    expect(plan?.env?.YIRU_HERMES_STARTUP_QUERY).toBe('run privately')
  })
  ...
  it('keeps Yiru ownership of the Hermes startup query and TUI mode', () => {
    const plan = buildAgentStartupPlan({
      agent: 'hermes',
    ...
    expect(plan?.env?.YIRU_HERMES_STARTUP_QUERY).toBe('automation prompt')
  })
  ...
  it('uses the Windows remote default shell for SSH Hermes queries', () => {
    ...
    expect(unwrapPowerShellScript(plan?.launchCommand)).toContain(
      "& 'hermes' 'chat' \"--query=$yiruHermesNativeQuery\" '--tui'"
    )
  })
  ...
  it('does not launch Codex with the Yiru profile when agent status hooks are enabled', () => {
    const plan = buildAgentStartupPlan({
      agent: 'codex',
      prompt: 'fix it',
      cmdOverrides: {},
  ...
  it('launches Claude without Yiru settings injection', () => {
    const plan = buildAgentStartupPlan({
      agent: 'claude',
      prompt: 'fix it',
      cmdOverrides: {},
  ...
  it('uses the Linux Yiru CLI command for Claude Agent Teams launches', () => {
    const plan = buildAgentStartupPlan({
    ...
    expect(plan?.launchCommand).toBe('yiru claude-teams')
  })

  it('uses the plain yiru shim for Claude Agent Teams on Linux SSH remotes', () => {
    // Why: the SSH relay deploys the CLI shim as `yiru` (not the local-only
    // `yiru` GNOME-screen-reader workaround), so a remote launch must not
    // emit `yiru claude-teams` — that name is not on the remote PATH and
  ...
  it('keeps the Windows yiru.cmd shim for Claude Agent Teams on SSH remotes', () => {
    // Why: the Windows remote shim is also `yiru.cmd`, matching the local
    ...
    expect(plan?.launchCommand).toBe('yiru.cmd claude-teams')
  })

  it('uses the public yiru command for local Linux Claude Agent Teams', () => {
    const plan = buildAgentStartupPlan({
      agent: 'claude-agent-teams',
      prompt: '',
      cmdOverrides: {},
  ...
  it('clears draft environment variables with the target shell syntax', () => {
    ...
    ).toBe('pi; Remove-Item Env:YIRU_PI_PREFILL -ErrorAction SilentlyContinue')
    ...
    ).toBe('pi & set "YIRU_PI_PREFILL="')
  })

  it('returns an OMP draft plan with YIRU_OMP_PREFILL (OMP-scoped, not Pi-shared)', () => {
    // Why: OMP owns its own managed prefill extension and env var.
    // yiru-prefill.ts reads YIRU_OMP_PREFILL for OMP launches — see
    ...
    expect(plan?.env).toEqual({ YIRU_OMP_PREFILL: 'fix the omp regression' })
    expect(plan?.expectedProcess).toBe('omp')
    expect(plan?.launchCommand).toBe('omp; unset YIRU_OMP_PREFILL')
  })
  ...
  it('excludes transient draft prompt env from launch config', () => {
    const plan = buildAgentDraftLaunchPlan({
      ...
      draft: 'prefill text',
      cmdOverrides: {},
      agentEnv: { YIRU_AGENT_MODE: 'managed' },
      platform: 'linux'
    })

    expect(plan?.env).toEqual({ YIRU_AGENT_MODE: 'managed', YIRU_PI_PREFILL: 'prefill text' })
    expect(plan?.launchConfig).toEqual({
      agentCommand: 'pi',
      agentArgs: '',
      agentEnv: { YIRU_AGENT_MODE: 'managed' }
