...
import watercolorIcon from '../../resources/app-icons/yiru-warm.png?asset'
...
const MAC_DOCK_ICON_SCRIPT = [
  ...
  'set appPath to system attribute "YIRU_APP_BUNDLE_PATH"',
  'set iconPath to system attribute "YIRU_APP_ICON_PATH"',
  "set image to current application's NSImage's alloc()'s initWithContentsOfFile:iconPath",
  'if image is missing value then error "Yiru app icon image could not be loaded"',
  "set ok to current application's NSWorkspace's sharedWorkspace()'s setIcon:image forFile:appPath options:0",
  'if ok is false then error "Yiru app icon could not be persisted"'
]
...
const MAC_DOCK_ICON_CLEAR_SCRIPT = [
  ...
  'if ok is false then error "Yiru app icon could not be cleared"'
]
...
function runMacCustomIconCommand(
  ...
  return runBoundedMacDockIconCommand({
    ...
    options: {
      env: {
        ...process.env,
        YIRU_APP_BUNDLE_PATH: appBundlePath,
        YIRU_APP_ICON_PATH: iconPath
      }
    },
