...
// Why: the install dir is namespaced by bundle version so concurrent Yiru
...
export function buildGuestInstallScript(bundleJs: Buffer, version: string): string {
  ...
  return [
    ...
    'mkdir -p "$d"',
    `base64 -d > "$d/bundle.$$.tmp" << 'YIRU_EOF_BUNDLE'`,
    b64.trimEnd(),
    'YIRU_EOF_BUNDLE',
    `mv "$d/bundle.$$.tmp" "$d/${WSL_HOOK_RELAY_BUNDLE_NAME}"`,
    `cat > "$d/launch.$$.tmp" << 'YIRU_EOF_LAUNCH'`,
...
export function buildWslRelaySpawnEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...process.env,
    WSL_UTF8: '1',
    YIRU_AGENT_HOOK_PORT: coords.YIRU_AGENT_HOOK_PORT,
    YIRU_AGENT_HOOK_TOKEN: coords.YIRU_AGENT_HOOK_TOKEN,
    YIRU_AGENT_HOOK_ENV: coords.YIRU_AGENT_HOOK_ENV,
    YIRU_AGENT_HOOK_VERSION: coords.YIRU_AGENT_HOOK_VERSION,
  ...
  addYiruWslInteropEnv(env as Record<string, string>)
  return env
}
