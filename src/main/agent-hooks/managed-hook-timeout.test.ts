...
const JSON_INSTALLERS = [
  ...
  },
  {
    agent: 'grok',
    timeout: MANAGED_HOOK_TIMEOUT_SECONDS,
    configPath: `${REMOTE_HOME}/.grok/hooks/yiru-status.json`,
    install: (sftp: SFTPWrapper) => new GrokHookService().installRemote(sftp, REMOTE_HOME)
  },
  {
    agent: 'copilot',
    timeout: 5,
    configPath: `${REMOTE_HOME}/.copilot/hooks/yiru.json`,
...
const MANAGED_HOOKS_DIR_NEEDLE = '/.yiru/agent-hooks/'

// Walk the parsed config and assert every Yiru-managed command carrier (a node
...
describe('managed agent hook timeouts', () => {
  ...
  it('writes a config-level timeout on local-only Droid hooks', () => {
    const homeDir = mkdtempSync(join(tmpdir(), 'yiru-droid-hook-timeout-'))
  ...
  it('bounds every generated POSIX curl wrapper with --connect-timeout and --max-time', async () => {
    ...
    const kimiWrapper = kimi.fs.files.get(`${REMOTE_HOME}/.yiru/agent-hooks/kimi-hook.sh`)!
  ...
  describe('dead-endpoint transport budget', () => {
    ...
    it.skipIf(!hasPosixCurl)(
      ...
      async () => {
        ...
        const wrapperBody = fs.files.get(`${REMOTE_HOME}/.yiru/agent-hooks/codex-hook.sh`)!

        tempDir = mkdtempSync(join(tmpdir(), 'yiru-hook-timeout-'))
