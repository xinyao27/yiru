...
// print JSON to the PTY. Yiru-managed attach (CLI/pane) already registers via EmulatorBridge;
...
export class ServeSimStateWatcher {
  ...
  private readonly yiruManagedHelperKeys = new Set<string>()
  ...
  // Why: bridge calls this when Yiru CLI/pane attaches so we do not duplicate-tab on our own session.
  markYiruManaged(info: ServeSimHelperInfo): void {
    this.yiruManagedHelperKeys.add(helperInstanceKey(info))
  }

  unmarkYiruManaged(deviceUdid: string): void {
    for (const key of this.yiruManagedHelperKeys) {
      if (key.startsWith(`${deviceUdid}::`)) {
        this.yiruManagedHelperKeys.delete(key)
      }
    }
  }
  ...
  stop(): void {
    ...
    this.yiruManagedHelperKeys.clear()
  ...
  private emitIfExternal(
    ...
    if (this.yiruManagedHelperKeys.has(instanceKey)) {
      return
    }
