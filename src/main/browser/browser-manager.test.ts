...
describe('browserManager', () => {
  ...
  it('keeps plain links current and routes explicit new-tab gestures to Yiru tabs', async () => {
    const rendererSendMock = vi.fn()
    ...
    expect(clickedLinkFrameName).toMatch(/^__yiru_clicked_link_foreground_/)
    ...
    expect(rendererSendMock).toHaveBeenCalledWith('browser:open-link-in-yiru-tab', {
      browserPageId: 'browser-1',
      url: 'https://docs.example.com/guide'
    })
  ...
  it('routes child-frame gestures with one-use tokens', async () => {
    ...
    const foregroundFrameName = firstScript.match(
      /__yiru_clicked_link_iframe_foreground_[0-9a-f-]+/
    )?.[0]
