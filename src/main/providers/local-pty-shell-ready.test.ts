...
// Why: the wrapper root is resolved from YIRU_USER_DATA_PATH (main
...
function setTestUserDataPath(path: string): void {
  process.env.YIRU_USER_DATA_PATH = path
}

const ORIGINAL_YIRU_USER_DATA_PATH = process.env.YIRU_USER_DATA_PATH

afterEach(() => {
  if (ORIGINAL_YIRU_USER_DATA_PATH === undefined) {
    delete process.env.YIRU_USER_DATA_PATH
  } else {
    process.env.YIRU_USER_DATA_PATH = ORIGINAL_YIRU_USER_DATA_PATH
  }
})
...
describe('scanForShellReady', () => {
  it('flushes marker-like output when the full marker is not BEL-terminated', () => {
    ...
    expect(scanForShellReady(state, 'before \x1b]777;yiru-shell-readyx')).toEqual({
      output: 'before \x1b]777;yiru-shell-readyx',
      matched: false,
  ...
  it('reports post-marker bytes only when bytes follow the BEL terminator in the matching call', () => {
    ...
    expect(scanForShellReady(state, 'before \x1b]777;yiru-shell-ready\x07')).toEqual({
      output: 'before ',
      matched: true,
      postMarkerBytesObserved: false
    })
    ...
    state = createShellReadyScanState()
    expect(scanForShellReady(state, 'before \x1b]777;yiru-shell-ready')).toEqual({
      output: 'before ',
      matched: false,
      postMarkerBytesObserved: false
    })
    ...
    state = createShellReadyScanState()
    expect(scanForShellReady(state, '\x1b]777;yiru-shell-ready')).toEqual({
      output: '',
      matched: false,
      postMarkerBytesObserved: false
    })
...
describe('shell-ready wrapper root resolution', () => {
  ...
  // import electron, so the wrapper root must resolve from YIRU_USER_DATA_PATH
  // (set by main at startup and by the daemon fork) rather than app.getPath.
  it('resolves the wrapper root from YIRU_USER_DATA_PATH', async () => {
    const root = mkdtempSync(join(tmpdir(), 'yiru-userdata-env-'))
    try {
...
describePosix('local PTY shell-ready launch config', () => {
  ...
  it('falls back to HOME for YIRU_ORIG_ZDOTDIR when inherited ZDOTDIR points at a wrapper dir', async () => {
    ...
    // PTYs spawned by the renderer/local provider when Yiru is launched from
    // inside a Yiru terminal (e.g. `pn dev`).
  ...
  it('emits OSC 133 C/D markers in the bash wrapper so agent exit cleanup fires', async () => {
    ...
    expect(bashRc.indexOf("trap '__yiru_osc133_preexec' DEBUG")).toBeGreaterThan(
      bashRc.indexOf('if [[ "${YIRU_SHELL_READY_MARKER:-0}" == "1" ]]; then')
    )
  ...
  it('restores wrapper ZDOTDIR from the runtime sourced path, not the baked literal', async () => {
    ...
    expect(zshenv).toContain('_yiru_wrapper_zdotdir_self="${${(%):-%x}:h}"')
    ...
    expect(zshenv.indexOf('_yiru_wrapper_zdotdir_self="${${(%):-%x}:h}"')).toBeLessThan(
...
describePosix('live zsh subprocess tests', () => {
  ...
  describeIfZsh('ZDOTDIR discovery with real zsh', () => {
    ...
    beforeEach(async () => {
      testHome = mkdtempSync(join(tmpdir(), 'yiru-zsh-test-home-'))
      userDataPath = mkdtempSync(join(tmpdir(), 'yiru-zsh-test-userdata-'))
      setTestUserDataPath(userDataPath)
    })
...
`
      ...
      // Build clean env: use wrapper ZDOTDIR but let wrapper discover YIRU_ORIG_ZDOTDIR at runtime
      ...
      delete cleanEnv.YIRU_ORIG_ZDOTDIR
      ...
      delete cleanEnv.YIRU_ATTRIBUTION_SHIM_DIR
      cleanEnv.ZDOTDIR = config.env.ZDOTDIR // Point to Yiru wrapper dir
      ...
      const result = spawnSync(
        ...
        [
          '-i',
          '-c',
          'echo "YIRU_ORIG_ZDOTDIR=${YIRU_ORIG_ZDOTDIR}" && echo "PATH_HAS_CUSTOM=${PATH%%:*}"'
        ],
        {
      ...
      expect(output).toContain(`YIRU_ORIG_ZDOTDIR=${xdgZshDir}`)
    ...
    it('preserves top-level .zshenv path and function side effects', async () => {
      ...
      // Dropping those side effects regresses non-Yiru zsh startup semantics.
      const xdgZshDir = join(testHome, '.config', 'zsh')
      mkdirSync(xdgZshDir, { recursive: true })
      writeFileSync(
        join(testHome, '.zshenv'),
...
yiru_zshenv_func() { echo "from-zshenv-function"; }
export ZDOTDIR="$HOME/.config/zsh"
`
      )
      ...
      const result = spawnSync(
        ...
        [
          '-c',
          'echo "PATH_HEAD=${PATH%%:*}" && echo "MY_VAR=${MY_VAR:-unset}" && yiru_zshenv_func'
        ],
        {
          env: cleanEnv as NodeJS.ProcessEnv,
          encoding: 'utf8'
...
`
      ...
      const result = spawnSync(
        'zsh',
        ['-c', 'echo "survived" && echo "YIRU_ORIG_ZDOTDIR=${YIRU_ORIG_ZDOTDIR}"'],
        {
          env: cleanEnv as NodeJS.ProcessEnv,
          encoding: 'utf8'
        }
      ...
      expect(result.stdout).toContain(`YIRU_ORIG_ZDOTDIR=${testHome}`)
    })
...
`
      ...
      const result = spawnSync('zsh', ['-c', 'echo "YIRU_ORIG_ZDOTDIR=${YIRU_ORIG_ZDOTDIR}"'], {
        env: cleanEnv as NodeJS.ProcessEnv,
        encoding: 'utf8'
      })
  ...
  describeIfZsh('high-priority edge cases', () => {
    ...
    beforeEach(async () => {
      testHome = mkdtempSync(join(tmpdir(), 'yiru-zsh-edge-'))
      userDataPath = mkdtempSync(join(tmpdir(), 'yiru-zsh-userdata-'))
      setTestUserDataPath(userDataPath)
    })
    ...
    it('discovers ZDOTDIR when .zshenv sources another file that sets it', async () => {
      ...
      expect(result.status).toBe(0)
      expect(result.stdout).toContain(`YIRU_ORIG_ZDOTDIR=${xdgZshDir}`)
    })
    ...
    it('preserves ZDOTDIR with spaces in path', async () => {
      ...
      expect(result.status).toBe(0)
      expect(result.stdout).toContain(`YIRU_ORIG_ZDOTDIR=${spacePath}`)
    })
    ...
    it('captures last ZDOTDIR value when set multiple times', async () => {
      ...
      expect(result.status).toBe(0)
      expect(result.stdout).toContain(`YIRU_ORIG_ZDOTDIR=${lastPath}`)
    })
    ...
    it('handles conditional ZDOTDIR based on environment', async () => {
      ...
      let result = spawnSync('zsh', ['-c', 'echo "YIRU_ORIG_ZDOTDIR=${YIRU_ORIG_ZDOTDIR}"'], {
        env: cleanEnv as NodeJS.ProcessEnv,
        encoding: 'utf8'
      })

      expect(result.status).toBe(0)
      expect(result.stdout).toContain(`YIRU_ORIG_ZDOTDIR=${localPath}`)
      ...
      result = spawnSync('zsh', ['-c', 'echo "YIRU_ORIG_ZDOTDIR=${YIRU_ORIG_ZDOTDIR}"'], {
        env: cleanEnv as NodeJS.ProcessEnv,
        encoding: 'utf8'
      })

      expect(result.status).toBe(0)
      expect(result.stdout).toContain(`YIRU_ORIG_ZDOTDIR=${remotePath}`)
    })
    ...
    it('does not source /.zshenv when HOME is empty', async () => {
      ...
      expect(zshenv).toContain('if [[ -n "${_yiru_zshenv_source_dir:-}"')
    })
    ...
    it('handles ZDOTDIR with single quote in path', async () => {
      ...
      expect(result.status).toBe(0)
      expect(result.stdout).toContain(`YIRU_ORIG_ZDOTDIR=${quotePath}`)
    })
    ...
    it('does not evaluate command substitution in ZDOTDIR', async () => {
      ...
      expect(result.stdout).toContain(`YIRU_ORIG_ZDOTDIR=${safePath}`)
    })
    ...
    it('handles ZDOTDIR with multiple trailing slashes', async () => {
      ...
      expect(result.stdout).toContain(`YIRU_ORIG_ZDOTDIR=${cleanPath}`)
    })
  })
  ...
  describeIfZsh('terminal emulator edge cases', () => {
    ...
    beforeEach(async () => {
      testHome = mkdtempSync(join(tmpdir(), 'yiru-term-'))
      userDataPath = mkdtempSync(join(tmpdir(), 'yiru-term-userdata-'))
      setTestUserDataPath(userDataPath)
    })
    ...
    it('handles sudo -E where HOME and ZDOTDIR mismatch', async () => {
      ...
      try {
        ...
        expect(config.env.YIRU_ORIG_ZDOTDIR).toBe(userZdotdir)
      } finally {
        if (previousZdotdir === undefined) {
          delete process.env.ZDOTDIR
        } else {
    ...
    it('re-discovers ZDOTDIR despite stale YIRU_ORIG_ZDOTDIR from previous session', async () => {
      ...
      const previousYiruZdotdir = process.env.YIRU_ORIG_ZDOTDIR
      process.env.YIRU_ORIG_ZDOTDIR = '/opt/yiru-old/shell-ready/zsh' // stale wrapper path
      ...
      try {
        ...
        const cleanEnv: Record<string, string | undefined> = {
          ...process.env,
          HOME: testHome,
          YIRU_ORIG_ZDOTDIR: '/opt/yiru-old/shell-ready/zsh'
        }
        delete cleanEnv.ZDOTDIR
        cleanEnv.ZDOTDIR = config.env.ZDOTDIR
        ...
        expect(result.stdout).toContain(`YIRU_ORIG_ZDOTDIR=${currentZdotdir}`)
      } finally {
        if (previousYiruZdotdir === undefined) {
          delete process.env.YIRU_ORIG_ZDOTDIR
        } else {
          process.env.YIRU_ORIG_ZDOTDIR = previousYiruZdotdir
        }
      }
    })

    it('prioritizes fresh discovery over inherited YIRU_ORIG_ZDOTDIR', async () => {
      ...
      process.env.YIRU_ORIG_ZDOTDIR = oldZdotdir
      ...
      try {
        ...
        expect(result.stdout).toContain(`YIRU_ORIG_ZDOTDIR=${freshZdotdir}`)
      } finally {
        if (previousYiruZdotdir === undefined) {
          delete process.env.YIRU_ORIG_ZDOTDIR
        } else {
    ...
    it('sources launch-time ZDOTDIR .zshenv when it is explicitly inherited', async () => {
      ...
      try {
        const { getShellReadyLaunchConfig } = await importFreshLocalPtyShellReady()
        const config = getShellReadyLaunchConfig('/bin/zsh')
        expect(config.env.YIRU_ZSHENV_SOURCE_DIR).toBe(inheritedZdotdir)
        ...
        const result = spawnSync(
          ...
          [
            '-c',
            'echo "YIRU_ORIG_ZDOTDIR=${YIRU_ORIG_ZDOTDIR}" && echo "SOURCE_MARKER=${SOURCE_MARKER:-unset}"'
          ],
          {
            env: cleanEnv as NodeJS.ProcessEnv,
            encoding: 'utf8'
        ...
        expect(result.status).toBe(0)
        expect(result.stdout).toContain(`YIRU_ORIG_ZDOTDIR=${inheritedZdotdir}`)
  ...
  describeIfZsh('automation and edge cases', () => {
    ...
    beforeEach(async () => {
      testHome = mkdtempSync(join(tmpdir(), 'yiru-auto-'))
      userDataPath = mkdtempSync(join(tmpdir(), 'yiru-auto-userdata-'))
      setTestUserDataPath(userDataPath)
    })
    ...
    it('handles empty HOME gracefully', async () => {
      ...
      expect(result.status).toBe(0)
      // Empty HOME falls back to empty YIRU_ORIG_ZDOTDIR
      expect(result.stdout).toContain('YIRU_ORIG_ZDOTDIR=\n')
    })
    ...
    it('handles unset HOME gracefully', async () => {
      ...
      expect(result.stdout).toMatch(/YIRU_ORIG_ZDOTDIR=.+/)
    })
