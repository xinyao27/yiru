...
describe('getPiAgentStatusExtensionSource', () => {
  ...
  it.each(['pi', 'omp'] as const)(
    ...
    (kind) => {
      ...
      expect(child.processEnv.YIRU_PI_STATUS_OWNED).toBe(String(SELF_PID))
      expect(grandchild.processEnv.YIRU_PI_STATUS_OWNED).toBe(String(SELF_PID))
  ...
  it('reports agent_end for a top-level run (including non-interactive) and claims the pane by pid', async () => {
    ...
    expect(harness.processEnv.YIRU_PI_STATUS_OWNED).toBe(String(SELF_PID))
  })
