...
const userDataDir = mkdtempSync(join(tmpdir(), 'yiru-pi-test-userdata-'))
...
describe('PiTitlebarExtensionService', () => {
  ...
  beforeEach(() => {
    piHome = mkdtempSync(join(tmpdir(), 'yiru-pi-test-pihome-'))
  ...
  it('buildPtyEnv installs Yiru extensions into the user agent dir without redirecting the home', () => {
    ...
    const statusExtensionSource = readFileSync(
      join(piHome, 'extensions', 'yiru-agent-status.ts'),
      'utf-8'
    )
    const titlebarExtensionSource = readFileSync(
      join(piHome, 'extensions', 'yiru-titlebar-spinner.ts'),
      'utf-8'
    )
    const prefillExtensionSource = readFileSync(
      join(piHome, 'extensions', 'yiru-prefill.ts'),
      'utf-8'
    )
    expect(statusExtensionSource).toContain('@yiru-managed-pi-extension')
    ...
    expect(titlebarExtensionSource).toContain('@yiru-managed-pi-extension')
    expect(titlebarExtensionSource).toContain('process.env.YIRU_PANE_KEY')
    expect(prefillExtensionSource).toContain('@yiru-managed-pi-extension')
    expect(prefillExtensionSource).toContain('process.env.YIRU_PANE_KEY')
    expectPiHomeIntact()
  })
  ...
  it('clearPty leaves the real Pi dir and managed extensions intact', () => {
    ...
    expect(existsSync(join(piHome, 'extensions', 'yiru-agent-status.ts'))).toBe(true)
    expectPiHomeIntact()
  })
  ...
  it('uses the same source dir for multiple PTYs with the same Pi dir', () => {
    ...
    expect(secondEnv.YIRU_PI_SOURCE_AGENT_DIR).toBe(firstEnv.YIRU_PI_SOURCE_AGENT_DIR)
    expect(readFileSync(join(piHome, 'extensions', 'user-ext', 'ext.ts'), 'utf-8')).toBe(
      'user extension'
    )
    expectPiHomeIntact()
  ...
  it('leaves OMP SQLite files in the real home instead of redirecting to an overlay', () => {
    ...
    expect(env.PI_CODING_AGENT_DIR).toBeUndefined()
    expect(env.YIRU_OMP_SOURCE_AGENT_DIR).toBe(piHome)
    expect(env.YIRU_OMP_STATUS_EXTENSION).toBe(join(piHome, 'extensions', 'yiru-agent-status.ts'))
  ...
  it('migrates missing OMP state from the old source overlay without overwriting source files', () => {
    ...
    writeFileSync(join(overlayDir, '.yiru-pi-overlay-manifest.json'), '{}')
    writeFileSync(join(overlayDir, 'extensions', 'yiru-agent-status.ts'), 'stale managed extension')
    ...
    writeFileSync(
      join(overlayDir, 'extensions', 'legacy-package', 'yiru-prefill.ts'),
      'user package file'
    )
    ...
    expect(
      readFileSync(join(piHome, 'extensions', 'legacy-package', 'yiru-prefill.ts'), 'utf-8')
    ).toBe('user package file')
    expect(readFileSync(join(piHome, 'extensions', 'yiru-agent-status.ts'), 'utf-8')).toContain(
      '/hook/omp'
    )
    expect(readFileSync(join(overlayDir, '.yiru-omp-overlay-migration-complete'), 'utf-8')).toBe(
      'complete\n'
    )
  })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      try {
        ...
        expect(existsSync(join(piHome, 'agent.db-wal'))).toBe(false)
        expect(existsSync(join(overlayDir, '.yiru-omp-overlay-migration-complete'))).toBe(false)
        ...
        expect(readFileSync(join(piHome, 'agent.db-wal'), 'utf-8')).toBe('legacy sqlite wal')
        expect(
          readFileSync(join(overlayDir, '.yiru-omp-overlay-migration-complete'), 'utf-8')
        ).toBe('complete\n')
      } finally {
        chmodSync(walPath, 0o600)
      }
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      mkdirSync(join(overlayDir, 'sessions'), { recursive: true })
      writeFileSync(join(overlayDir, 'sessions', 'legacy-session.jsonl'), 'legacy transcript')
      const outsideDir = mkdtempSync(join(tmpdir(), 'yiru-omp-target-junction-'))
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      writeFileSync(join(overlayDir, 'agent.db'), 'legacy sqlite credentials')
      writeFileSync(join(overlayDir, 'sessions', 'legacy-session.jsonl'), 'legacy transcript')
      const outsideDir = mkdtempSync(join(tmpdir(), 'yiru-omp-dangling-target-'))
  ...
  it('rebuilding updates Yiru-owned extensions while preserving user files', () => {
    const svc = new PiTitlebarExtensionService()
    svc.buildPtyEnv('pty-refresh-1', piHome, 'pi')
    writeFileSync(
      join(piHome, 'extensions', 'yiru-agent-status.ts'),
      '// @yiru-managed-pi-extension\nstale'
    )
  ...
  it("does not overwrite a user's same-named Yiru extension file", () => {
    const userStatusExtension = 'user-owned status extension'
    writeFileSync(join(piHome, 'extensions', 'yiru-agent-status.ts'), userStatusExtension, 'utf-8')

    const svc = new PiTitlebarExtensionService()
    const env = svc.buildPtyEnv('pty-same-name-extension', piHome, 'pi')

    expect(env.PI_CODING_AGENT_DIR).toBeUndefined()
    expect(readFileSync(join(piHome, 'extensions', 'yiru-agent-status.ts'), 'utf-8')).toBe(
      userStatusExtension
    )
    expectPiHomeIntact()
  })

  it('uses a Yiru-owned OMP status extension when a same-named user file exists', () => {
    const userStatusExtension = 'user-owned status extension'
    const userStatusPath = join(piHome, 'extensions', 'yiru-agent-status.ts')
    ...
    expect(env.YIRU_OMP_STATUS_EXTENSION).toBe(fallbackStatusPath)
    expect(readFileSync(fallbackStatusPath, 'utf-8')).toContain('@yiru-managed-pi-extension')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      const realExtensionsDir = mkdtempSync(join(tmpdir(), 'yiru-real-pi-extensions-'))
      try {
        ...
        expect(env.PI_CODING_AGENT_DIR).toBeUndefined()
        expect(existsSync(join(realExtensionsDir, 'yiru-agent-status.ts'))).toBe(true)
        expect(existsSync(join(realExtensionsDir, 'yiru-prefill.ts'))).toBe(true)
        expect(existsSync(join(realExtensionsDir, 'yiru-titlebar-spinner.ts'))).toBe(true)
        expect(readFileSync(join(realExtensionsDir, 'yiru-agent-status.ts'), 'utf-8')).toContain(
          '/hook/pi'
        )
      } finally {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      // Why: simulate an overlay that was left behind by a prior Yiru session,
  ...
  // Why: per-agent source dir. Yiru's user picks Pi or OMP per
  ...
  describe('per-agent default source dir (no cross-agent fallback)', () => {
    ...
    it('launching pi with both ~/.pi/agent and ~/.omp/agent present installs into ~/.pi/agent', () => {
      const fakeHome = mkdtempSync(join(tmpdir(), 'yiru-pi-both-'))
      ...
      try {
        ...
        expect(env.PI_CODING_AGENT_DIR).toBeUndefined()
        expect(env.YIRU_PI_SOURCE_AGENT_DIR).toBe(join(fakeHome, '.pi', 'agent'))
        expect(
          existsSync(join(fakeHome, '.pi', 'agent', 'extensions', 'yiru-agent-status.ts'))
        ).toBe(true)
        expect(
          existsSync(join(fakeHome, '.omp', 'agent', 'extensions', 'yiru-agent-status.ts'))
        ).toBe(false)
      } finally {
    ...
    it('launching omp with both ~/.pi/agent and ~/.omp/agent present installs into ~/.omp/agent', () => {
      const fakeHome = mkdtempSync(join(tmpdir(), 'yiru-omp-both-'))
      seedAgentDir(fakeHome, '.pi', 'pi')
      seedAgentDir(fakeHome, '.omp', 'omp')
      ...
      try {
        ...
        expect(env.PI_CODING_AGENT_DIR).toBeUndefined()
        expect(env.YIRU_OMP_SOURCE_AGENT_DIR).toBe(join(fakeHome, '.omp', 'agent'))
        expect(env.YIRU_OMP_STATUS_EXTENSION).toBe(
          join(fakeHome, '.omp', 'agent', 'extensions', 'yiru-agent-status.ts')
        )
        expect(
          readFileSync(
            join(fakeHome, '.omp', 'agent', 'extensions', 'yiru-agent-status.ts'),
    ...
    it('launching omp when only ~/.pi/agent exists does NOT mirror Pi state', () => {
      ...
      // overlay from empty (Yiru extensions only) — never cross-pollinate
      // from the other agent's dir.
      const fakeHome = mkdtempSync(join(tmpdir(), 'yiru-omp-only-pi-'))
      seedAgentDir(fakeHome, '.pi', 'pi')
      ...
      try {
        ...
        expect(env.PI_CODING_AGENT_DIR).toBeUndefined()
        expect(env.YIRU_OMP_SOURCE_AGENT_DIR).toBe(ompAgentDir)
