...
describe('buildHostCliEnv', () => {
  it('forwards only Yiru terminal-context vars from the remote env', () => {
    const env = buildHostCliEnv({
      hostEnv: { PATH: '/host/bin', NODE_OPTIONS: '--inspect' },
      remoteEnv: {
        YIRU_TERMINAL_HANDLE: 'term_remote',
        YIRU_WORKTREE_ID: 'repo::/home/alice/wt',
        YIRU_PANE_KEY: 'pane-9',
        YIRU_WORKSPACE_ID: 'ws-1',
        ...
        PATH: '/remote/bin',
        YIRU_USER_DATA_PATH: '/remote/user-data'
      },
      userDataPath: '/host/user-data',
      remoteCwd: '/home/alice/wt/sub'
    })

    expect(env.YIRU_TERMINAL_HANDLE).toBe('term_remote')
    expect(env.YIRU_WORKTREE_ID).toBe('repo::/home/alice/wt')
    expect(env.YIRU_PANE_KEY).toBe('pane-9')
    expect(env.YIRU_WORKSPACE_ID).toBe('ws-1')
    expect(env.PATH).toBe('/host/bin')
    expect(env.YIRU_USER_DATA_PATH).toBe('/host/user-data')
    expect(env.YIRU_CLI_CWD).toBe('/home/alice/wt/sub')
    expect(env.ELECTRON_RUN_AS_NODE).toBe('1')
    expect(env.NODE_OPTIONS).toBeUndefined()
    expect(env.YIRU_NODE_OPTIONS).toBe('--inspect')
  })
})
...
describe('runHostYiruCliPassthrough', () => {
  it('spawns the bundled CLI entry with the remote argv and returns captured output', async () => {
    ...
    expect(options.env.YIRU_CLI_CWD).toBe('/home/alice/wt')
    expect(options.env.YIRU_TERMINAL_HANDLE).toBe('term_remote')
