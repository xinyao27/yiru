...
vi.mock('./ssh-relay-versioned-install', () => ({
  ...
  computeRemoteRelayDir: (home: string, version: string) => `${home}/.yiru-remote/relay-${version}`,
...
describe('relay GC deploy retry', () => {
  ...
  it('recomputes install state when GC wins before a healthy relay launch', async () => {
    ...
    vi.mocked(execCommand).mockImplementation(async (_conn, command) => {
      if (command.includes('__YIRU_REMOTE_PLATFORM__')) {
        return '__YIRU_REMOTE_PLATFORM__ Linux x86_64'
      }
      if (command === 'echo $HOME') {
        return '/home/user'
      }
      if (command.includes('node-pty')) {
        return 'YIRU-NATIVE-DEPS-OK'
      }
