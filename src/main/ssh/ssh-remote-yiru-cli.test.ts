...
describe('runRemoteYiruCli', () => {
  ...
  it('uses the remote YIRU_TERMINAL_HANDLE as orchestration sender identity', async () => {
    const { runtime, db } = createRuntime()

    const result = await runRemoteYiruCli(
      runtime,
      {
    ...
    const result = await runRemoteYiruCli(
      runtime,
      {
        ...
        env: {
          ...
          YIRU_PANE_KEY: 'tab_ssh:leaf_ssh'
        }
      },
      LEGACY_FALLBACK_OPTIONS
    )
    ...
    try {
      const result = await runRemoteYiruCli(
        runtime,
        {
          ...
          cwd: '/home/alice/repo',
          env: { YIRU_PANE_KEY: 'tab_foreign:leaf_foreign' }
        },
        LEGACY_FALLBACK_OPTIONS
      )
    ...
    try {
      const result = await runRemoteYiruCli(
        runtime,
        {
          ...
          env: {
            YIRU_TERMINAL_HANDLE: 'term_ssh',
            YIRU_PANE_KEY: 'tab_owner:leaf_owner'
          }
        },
        LEGACY_FALLBACK_OPTIONS
      )
  ...
  it('uses the remote YIRU_TERMINAL_HANDLE as orchestration check identity', async () => {
    const { runtime, db } = createRuntime()
    db.insertMessage({
      from: 'term_windows',
      to: 'term_ssh',
  ...
  it('routes previously-unsupported commands through the full host CLI', async () => {
    ...
    const resultPromise = runRemoteYiruCli(
      runtime,
      {
        argv: ['worktree', 'create', '--repo', 'yiru', '--branch', 'fix/x', '--json'],
        cwd: '/home/alice/repo',
        env: { YIRU_TERMINAL_HANDLE: 'term_ssh' }
      },
      {
  ...
  it('rejects host-interactive commands with a targeted error instead of bridging them', async () => {
    ...
    expect(result.exitCode).toBe(1)
    expect(result.stderr).toContain('yiru serve')
  ...
  it('explains the root cause when falling back and the command is not in the legacy switch', async () => {
    ...
    expect(result.exitCode).toBe(1)
    expect(result.stderr).toContain('Unsupported SSH Yiru CLI command: worktree list')
    expect(result.stderr).toContain('full Yiru CLI bridge unavailable')
  })
})
