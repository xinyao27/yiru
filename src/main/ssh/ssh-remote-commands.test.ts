...
describe('ssh remote command builders', () => {
  ...
  it('emits an explicit POSIX liveness result so GC can fail closed', () => {
    const command = relayLivenessProbeCommand(posix, '/home/u/.yiru-remote/relay-0.1.0')
  ...
  it('computes install-lock age on the remote host clock', () => {
    const posixCommand = lockAgeSecondsCommand(posix, '/home/me/.yiru-remote/relay/.install-lock')
    const windowsScript = decodePowerShellCommand(
      lockAgeSecondsCommand(windows, 'C:/Users/me/.yiru-remote/relay/.install-lock')
    )
  ...
  it('serializes stale recovery with unbounded numbered sibling claims', () => {
    const posixCommand = tryStealInstallLockCommand(
      posix,
      '/home/me/.yiru-remote/relay/.install-lock',
      20 * 60
    )
    const windowsScript = decodePowerShellCommand(
      tryStealInstallLockCommand(windows, 'C:/Users/me/.yiru-remote/relay/.install-lock', 20 * 60)
    )
  ...
  it.runIf(powerShellExecutable)(
    ...
    () => {
      const script = decodePowerShellCommand(
        tryStealInstallLockCommand(
          windows,
          'C:/Users/yiru-missing/.yiru-remote/relay/.install-lock',
          20 * 60
        )
      )
  ...
  it.runIf(powerShell51Executable)(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), 'yiru-install-lock-windows-race-'))
      try {
  ...
  it.runIf(powerShell51Executable)(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), 'yiru-install-lock-windows-stale-'))
      try {
        const lockPath = join(root, '.install-lock')
        mkdirSync(lockPath)
  ...
  it.runIf(powerShell51Executable)(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), 'yiru-install-lock-windows-steal-race-'))
      try {
        const lockPath = join(root, '.install-lock')
        mkdirSync(lockPath)
        const staleDate = new Date(Date.now() - 60 * 60_000)
  ...
  it.runIf(powerShellExecutable)(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), 'yiru-install-lock-windows-compat-'))
      try {
        const lockPath = join(root, '.install-lock')
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    () => {
      const root = mkdtempSync(join(tmpdir(), 'yiru-install-lock-'))
      try {
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), 'yiru-install-lock-race-'))
      try {
        const lockDir = join(root, '.install-lock')
        mkdirSync(lockDir)
        const staleDate = new Date(Date.now() - 60 * 60_000)
