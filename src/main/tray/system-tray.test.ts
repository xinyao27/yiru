...
vi.mock('../../../resources/tray/yiru-menu-barTemplate.png?asset&asarUnpack', () => ({
  default: '/assets/yiru-menu-barTemplate.png'
}))

vi.mock('../../../resources/tray/yiru-menu-barTemplate@2x.png?asset&asarUnpack', () => ({
  default: '/assets/yiru-menu-barTemplate@2x.png'
}))
...
describe('createSystemTray', () => {
  it('keeps the Windows icon, Open/Quit menu, and left-click behavior', async () => {
    ...
    expect(trayInstances[0].setToolTip).toHaveBeenCalledWith('Yiru')
    expect(builtMenuItems().map((item) => item.label)).toEqual(['Open Yiru', undefined, 'Quit'])
...
describe('setTrayAttention', () => {
  ...
  it('uses a literal theme-aware glyph for macOS attention, then restores template mode', async () => {
    ...
    expect(created.setImage).toHaveBeenCalledWith(attentionImage)
    expect(created.setToolTip).toHaveBeenCalledWith('Yiru - activity waiting')
    ...
    expect(created.setToolTip).toHaveBeenLastCalledWith('Yiru')
  })
