...
function stubLaunchScope(runtime: YiruRuntimeService, path = '/repo/app'): void {
...
describe('YiruRuntimeService terminal startup cwd', () => {
  ...
  it('materializes restored headless mobile tabs in the persisted startup cwd', async () => {
    ...
    const runtime = new YiruRuntimeService(store as never)
    stubLaunchScope(runtime)
    const spawn = vi.fn().mockResolvedValue({ id: 'pty-1' })
    runtime.setPtyController({
      spawn,
