...
// Why: dedicated file for "one surface" separation (emulator), parallel to yiru-runtime-browser.ts. Keeps YiruRuntimeService focused; emulator routing easy to scan. No max-lines disable (split further if grows; per AGENTS + plan Phase 3).
...
export class RuntimeEmulatorCommands {
  ...
  async emulatorAttach(params: {
    ...
    if (worktreeId) {
      ...
      if (reusable) {
        ...
        serveSimStateWatcher.markYiruManaged(reusable)
      ...
      if (stoppedUdid) {
        serveSimStateWatcher.unmarkYiruManaged(stoppedUdid)
      }
    }
  ...
  // devices/AVDs) for the cross-platform `yiru emulator devices` command.
  ...
  async emulatorKill(params: {
    ...
    serveSimStateWatcher.unmarkYiruManaged(killedUdid)
  ...
  async emulatorShutdown(params: {
    ...
    if (params.managedOnly && worktreeId && !params.device && !params.emulator) {
      ...
      if (shutdownUdid) {
        serveSimStateWatcher.unmarkYiruManaged(shutdownUdid)
      }
