...
describe('plugin skill candidate scan', () => {
  it('stops at the package candidate budget and marks the scan incomplete', async () => {
    const root = await mkdtemp(join(tmpdir(), 'yiru-plugin-skill-scan-'))
    temporaryDirectories.push(root)
    await Promise.all(
      ['one', 'two'].map((vendor) => mkdir(join(root, vendor, 'yiru-cli'), { recursive: true }))
    )

    const result = await scanKnownPluginSkillCandidates(root, new Set(['yiru-cli']), 1)
  ...
  it('marks depth-truncated subtrees incomplete so hidden skills poison eligibility', async () => {
    const root = await mkdtemp(join(tmpdir(), 'yiru-plugin-skill-depth-'))
    temporaryDirectories.push(root)
    const segments = Array.from({ length: 11 }, (_, index) => `level-${index}`)
    const hiddenSkill = join(root, ...segments, 'yiru-cli')
    await mkdir(hiddenSkill, { recursive: true })

    const result = await scanKnownPluginSkillCandidates(root, new Set(['yiru-cli']))
