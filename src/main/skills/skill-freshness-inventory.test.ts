...
async function fixture() {
  const root = await mkdtemp(join(tmpdir(), 'yiru-skill-inventory-'))
  ...
  await Promise.all([
    ...
    writeFile(
      ...
      `${JSON.stringify({ schemaVersion: 1, skills: { 'yiru-cli': snapshots } }, null, 2)}\n`
    ),
    writeFile(
      ...
      `${JSON.stringify(
        {
          ...
          releases: [
            ...
            { appVersion: '2.0.0', skills: { 'yiru-cli': 2 } },
            { appVersion: '3.0.0', skills: { 'yiru-cli': 3 } }
          ]
        },
        null,
        2
  ...
  const writeSkill = async (rootPath: string, markdown: string): Promise<string> => {
    const directory = join(rootPath, 'yiru-cli')
...
describe('read-only skill freshness inventory', () => {
  it('offers an exact older official name only when all global placements are safe', async () => {
    ...
    expect(inventory.eligibleUpdateNames).toEqual(['yiru-cli'])
  })
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    async () => {
      ...
      await symlink(canonical, join(claudeRoot, 'yiru-cli'))
    ...
    async () => {
      ...
      const repos = await Promise.all(
        ['one', 'two'].map(async (id) => {
          ...
          await symlink(shared, join(root, 'yiru-cli'))
  ...
  it('keeps inaccessible placements visible and lets them poison the name', async () => {
    const test = await fixture()
    await test.writeSkill(join(test.homeDir, '.agents', 'skills'), test.oldMarkdown)
    const inaccessiblePath = join(test.homeDir, '.codex', 'skills', 'yiru-cli')
  ...
  it('does not lose an inaccessible known repository placement', async () => {
    ...
    const inaccessiblePath = join(repoPath, '.agents', 'skills', 'yiru-cli')
  ...
  it('classifies exact current bytes as current when a later snapshot reuses them', async () => {
    ...
    registry.skills['yiru-cli'].push(snapshot(4, test.currentMarkdown))
