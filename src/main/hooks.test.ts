...
const TEST_REPO_YIRU_YAML_PATH = join(TEST_REPO_PATH, 'yiru.yaml')
const TEST_WORKTREE_YIRU_YAML_PATH = join(TEST_WORKTREE_PATH, 'yiru.yaml')
const TEST_ISSUE_COMMAND_PATH = join(TEST_REPO_PATH, '.yiru', 'issue-command')
const TEST_GITIGNORE_PATH = join(TEST_REPO_PATH, '.gitignore')

describe('parseYiruYaml', () => {
  it('parses YAML with setup script only', () => {
    const yaml = `scripts:\n  setup: |\n    echo "setting up"\n    npm install\n`
    const result = parseYiruYaml(yaml)
  ...
  it('returns null when there is no scripts block', () => {
    const yaml = `other:\n  key: value\n`
    expect(parseYiruYaml(yaml)).toBeNull()
  })
  ...
  it('parses default terminal tabs from yiru.yaml', () => {
    const yaml = [
      'defaultTabs:',
      '  - title: Claude',
      '    color: "#f97316"',
    ...
    expect(parseYiruYaml(yaml)).toEqual({
      scripts: {},
      defaultTabs: [
  ...
  it('parses environmentRecipes from yiru.yaml', () => {
    const yaml = [
      'environmentRecipes:',
      '  - id: cloud-sandbox',
      '    name: Cloud Sandbox',
      '    description: Starts a per-workspace VM.',
      '    create: ./scripts/yiru-vm/start-cloud-sandbox.sh',
      '    suspend: ./scripts/yiru-vm/suspend-cloud-sandbox.sh',
      '    resume: ./scripts/yiru-vm/resume-cloud-sandbox.sh',
      '    destroy: ./scripts/yiru-vm/destroy-cloud-sandbox.sh'
    ].join('\n')
  ...
  it('parses legacy environmentRecipes command and cleanup aliases', () => {
    const yaml = [
      ...
      '  - id: manual-sandbox',
      '    name: Manual Sandbox',
      '    command: ./scripts/yiru-vm/start-manual-sandbox.sh',
      '    cleanup: none'
    ].join('\n')
      ...
      environmentRecipes: [
        {
          id: 'manual-sandbox',
          name: 'Manual Sandbox',
          create: './scripts/yiru-vm/start-manual-sandbox.sh',
  ...
  it('drops invalid and duplicate environmentRecipes', () => {
    const yaml = [
      ...
      '    create: ./scripts/yiru-vm/start-duplicate.sh',
      '  - id: missing-create',
      '    name: Missing Create',
      '  - name: Missing Id',
      '    create: ./scripts/yiru-vm/start-missing-id.sh',
      '  - id: "Cloud Sandbox"',
      '    name: Unsafe Id',
      '    create: ./scripts/yiru-vm/start-unsafe-id.sh',
      '  - 42'
    ].join('\n')
...
describe('hasUnrecognizedYiruYamlKeys', () => {
  it('returns true when the file contains only keys this version does not handle', async () => {
    ...
    const { hasUnrecognizedYiruYamlKeys } = await import('./hooks')
    expect(hasUnrecognizedYiruYamlKeys('/test/repo')).toBe(true)
  })
  ...
  it('returns false when the file contains only recognised keys', async () => {
    ...
    const { hasUnrecognizedYiruYamlKeys } = await import('./hooks')
    expect(hasUnrecognizedYiruYamlKeys('/test/repo')).toBe(false)
  })
...
describe('readIssueCommand', () => {
  it('prefers the local override over the shared yiru.yaml command', async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockImplementation(
      (path) => path === TEST_ISSUE_COMMAND_PATH || path === TEST_REPO_YIRU_YAML_PATH
    )
    vi.mocked(fs.readFileSync).mockImplementation((path) => {
      if (path === TEST_ISSUE_COMMAND_PATH) {
        return 'local command\n'
      }
      if (path === TEST_REPO_YIRU_YAML_PATH) {
        return 'issueCommand: |\n  shared command\n'
      }
      return ''
    })
  ...
  it('falls back to the shared yiru.yaml command when no local override exists', async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockImplementation((path) => path === TEST_REPO_YIRU_YAML_PATH)
    vi.mocked(fs.readFileSync).mockImplementation((path) => {
      if (path === TEST_REPO_YIRU_YAML_PATH) {
        return 'issueCommand: |\n  shared command\n'
      }
...
describe('writeIssueCommand', () => {
  it('writes only the local override file and keeps .yiru ignored locally', async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockImplementation(
      (path) => path === TEST_GITIGNORE_PATH || path === join(TEST_REPO_PATH, '.yiru')
    )
    vi.mocked(fs.readFileSync).mockImplementation((path) => {
...
describe('getEffectiveHooks', () => {
  ...
  it('uses hooks from yiru.yaml when present', async () => {
    const fs = await import('node:fs')
  ...
  it("loads setup hooks from the target worktree's yiru.yaml when a worktree path is provided", async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockImplementation(
      (path) => path === TEST_REPO_YIRU_YAML_PATH || path === TEST_WORKTREE_YIRU_YAML_PATH
    )
    vi.mocked(fs.readFileSync).mockImplementation((path) => {
      if (path === TEST_REPO_YIRU_YAML_PATH) {
        return 'scripts:\n  setup: |\n    echo old-version\n'
      }
      if (path === TEST_WORKTREE_YIRU_YAML_PATH) {
        return 'scripts:\n  setup: |\n    echo new-version\n'
      }
      return ''
    })
  ...
  it('uses local settings by default even when yiru.yaml defines only one command', async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockReturnValue(true)
    vi.mocked(fs.readFileSync).mockReturnValue('scripts:\n  archive: |\n    echo "yaml archive"\n')
  ...
  it('treats legacy shared-first policy as yiru.yaml only', async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockReturnValue(true)
    vi.mocked(fs.readFileSync).mockReturnValue('scripts:\n  archive: |\n    echo "yaml archive"\n')
...
describe('runHook', () => {
  ...
  it('runs Windows-path hooks through WSL when the project runtime targets WSL', async () => {
    ...
    execFileMock.mockImplementation((_file, _args, options, callback) => {
      ...
      expect(options).toEqual(
        expect.objectContaining({
          env: expect.objectContaining({
            YIRU_ROOT_PATH: '/mnt/c/Users/jinwo/git/yiru',
            YIRU_WORKTREE_PATH: '/mnt/c/Users/jinwo/git/yiru-feature',
            CONDUCTOR_ROOT_PATH: '/mnt/c/Users/jinwo/git/yiru',
            GHOSTX_ROOT_PATH: '/mnt/c/Users/jinwo/git/yiru'
          })
        })
    ...
    try {
      ...
      const result = await runHook(
        'setup',
        'C:\\Users\\jinwo\\git\\yiru-feature',
        {
          ...makeRepo(),
          path: 'C:\\Users\\jinwo\\git\\yiru'
        },
      ...
      expect(execFileMock).toHaveBeenCalledWith(
        ...
        [
          ...
          'bash',
          '-c',
          "cd '/mnt/c/Users/jinwo/git/yiru-feature' && echo hello"
        ],
        expect.any(Object),
        expect.any(Function)
      )
  ...
  it('writes Windows-path setup runners through WSL git when the project runtime targets WSL', async () => {
    ...
    gitExecFileSyncMock.mockReturnValue('/mnt/c/Users/jinwo/git/yiru/.git/yiru/setup-runner.sh\n')
    ...
    try {
      ...
      expect(gitExecFileSyncMock).toHaveBeenCalledWith(
        ...
        {
          cwd: 'C:\\Users\\jinwo\\git\\yiru-feature',
          wslDistro: 'Ubuntu'
        }
      )
...
describe('createSetupRunnerScript', () => {
  ...
  it('omits waitForAgentStartup unless the repo explicitly waits for setup', async () => {
    gitExecFileSyncMock.mockReset()
    gitExecFileSyncMock.mockReturnValue('/test/repo/.git/yiru/setup-runner.sh\n')
    const { createSetupRunnerScript } = await import('./hooks')
  ...
  it('marks setup-runner terminals for the always-on credential guard', async () => {
    ...
    expect(setup.envVars).toMatchObject({
      ...
      YIRU_WORKTREE_PATH: '/test/worktree',
      YIRU_INTERNAL_TERMINAL_GIT_CREDENTIAL_GUARD_POLICY: 'guard'
    })
  })
})
