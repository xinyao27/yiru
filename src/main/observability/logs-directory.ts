...
// macOS convention is `~/Library/Application Support/Yiru/logs/`; Windows and
...
function getUserDataDir(): string {
  try {
    ...
    const home = homedir()
    if (platform() === 'darwin') {
      return join(home, 'Library', 'Application Support', 'Yiru')
    }
    if (platform() === 'win32') {
      return join(process.env.APPDATA ?? home, 'Yiru')
    }
    return join(home, '.config', 'Yiru')
  }
}
