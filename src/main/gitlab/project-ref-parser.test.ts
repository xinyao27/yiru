...
describe('gitlab project ref parsing', () => {
  it('parses HTTPS and SSH GitLab.com remotes', () => {
    ...
    expect(parseGitLabProjectRef('git@gitlab.com:stablyai/yiru.git')).toEqual({
      host: 'gitlab.com',
      path: 'stablyai/yiru'
    })
  })
  ...
  it('returns null for non-GitLab hosts when host not in knownHosts', () => {
    expect(parseGitLabProjectRef('git@github.com:stablyai/yiru.git')).toBeNull()
...
describe('gitlab remote project ref candidates', () => {
  it('extracts self-hosted candidates before the host is trusted', () => {
    expect(parseRemoteProjectRefCandidate('git@gitlab.internal:team/yiru.git')).toEqual({
