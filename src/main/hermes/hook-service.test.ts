...
describe('HermesHookService', () => {
  ...
  beforeEach(() => {
    ...
    homeDir = mkdtempSync(join(tmpdir(), 'yiru-hermes-hooks-'))
  ...
  it('preserves other enabled plugins and removes Yiru from disabled list', () => {
    writeFileSync(
  ...
  it('registered plugin hooks post normalized JSON to Yiru', async () => {
    ...
    const received = new Promise<Record<string, unknown>>((resolve, reject) => {
      const server = createServer((req, res) => {
        ...
        req.on('end', () => {
          try {
            ...
            expect(req.headers['x-yiru-agent-hook-token']).toBe('token-1')
            res.writeHead(204)
            res.end()
      ...
      server.listen(0, '127.0.0.1', () => {
        ...
        const script = [
          ...
          `spec = importlib.util.spec_from_file_location("yiru_status", ${JSON.stringify(initPath)})`,
