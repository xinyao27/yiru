...
function profile(
  id: string,
  name: string,
  kind: YiruProfileKind = 'local'
): YiruProfileIndex['profiles'][number] {
  return {
    id,
    name,
...
function writeProfileState(profileId: string, repos: Repo[]): void {
  ...
  const dataFile = join(testState.dir, 'profiles', profileId, 'yiru-data.json')
...
describe('profile project presence', () => {
  beforeEach(() => {
    testState.dir = mkdtempSync(join(tmpdir(), 'yiru-profile-presence-'))
    writeIndex()
  })
  ...
  it('finds matching projects in other profiles while excluding the active profile', async () => {
    writeProfileState('personal', [
      makeRepo({ id: 'personal-repo', path: 'C:\\Code\\Yiru', displayName: 'Personal Yiru' })
    ])
    writeProfileState('work', [
      makeRepo({ id: 'work-repo', path: 'C:\\Code\\Yiru', displayName: 'Work Yiru' })
    ])

    const { findYiruProfileProjectsByPath } = await loadPresenceModule()
    const result = findYiruProfileProjectsByPath(
      {
        path: 'c:/code/yiru/',
  ...
  it('keeps SSH projects separate from local projects with the same path', async () => {
    writeProfileState('personal', [
      makeRepo({ id: 'local-repo', path: '/srv/yiru', displayName: 'Local Yiru' })
    ])
    writeProfileState('work', [
      makeRepo({
        id: 'ssh-repo',
        path: '/srv/yiru',
        displayName: 'SSH Yiru',
