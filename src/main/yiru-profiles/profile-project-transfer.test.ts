...
function profile(id: string, name: string): YiruProfileIndex['profiles'][number] {
  return {
    id,
    name,
...
function profileDataPath(profileId: string): string {
  return join(testState.dir, 'profiles', profileId, 'yiru-data.json')
}
...
describe('profile project transfer', () => {
  beforeEach(() => {
    testState.dir = mkdtempSync(join(tmpdir(), 'yiru-profile-transfer-'))
    writeIndex()
  })
  ...
  it('copies a project into another profile with a new repo id and re-keyed metadata', async () => {
    const sourceWorktreeId = 'repo-1::/workspace/yiru-feature'
    writeProfileState(
      'personal',
      makeState({
        repos: [makeRepo()],
    ...
    const { transferYiruProfileProject } = await loadTransferModule()
    const result = transferYiruProfileProject(
      {
    ...
    const targetWorktreeId = `${targetRepoId}::/workspace/yiru-feature`
    expect(target.repos).toEqual([
      expect.objectContaining({ id: targetRepoId, path: '/workspace/yiru' })
    ])
  ...
  it('moves a project, preserving SSH identity and restorable workspace session state', async () => {
    const sourceWorktreeId = 'repo-ssh::/srv/yiru-feature'
            ...
            [sourceWorktreeId]: [
              {
                ...
                sessionPartition: 'persist:yiru-profile-personal-deadbeef-browser-default',
  ...
  it('rejects a duplicate physical project inside the target profile', async () => {
    writeProfileState(
      'personal',
      makeState({
        repos: [makeRepo({ path: 'C:\\Work\\Yiru\\' })]
      })
    )
    writeProfileState(
      'work',
      makeState({
        repos: [makeRepo({ id: 'repo-existing', path: 'c:/work/yiru' })]
      })
    )
