...
export class YiruCloudRequestError extends Error {
...
function assertString(value: unknown, field: string): string {
  if (typeof value !== 'string') {
    throw new Error(`invalid_yiru_cloud_${field}`)
  }
...
function normalizeCapabilities(value: unknown): YiruCloudCapabilities {
...
function normalizeOrganizations(value: unknown): YiruCloudOrgSummary[] | undefined {
...
function normalizeCloudSummary(value: unknown): YiruProfileCloudSummary {
  if (!value || typeof value !== 'object') {
    throw new Error('invalid_yiru_cloud_profile')
  }
  const record = value as Record<string, unknown>
  return {
...
function normalizeSessionResponse(value: unknown): YiruCloudSessionExchangeResponse {
  if (!value || typeof value !== 'object') {
    throw new Error('invalid_yiru_cloud_session')
  }
  const record = value as Record<string, unknown>
  return {
...
async function postJson<T>(url: string, body: unknown, accessToken?: string): Promise<T> {
  ...
  if (!response.ok) {
    throw new YiruCloudRequestError(response.status)
  }
  return (await response.json()) as T
}

export async function exchangeYiruCloudAuthCode(
  config: YiruCloudAuthConfig,
  args: ExchangeCodeArgs
): Promise<YiruCloudSessionExchangeResponse> {
...
export async function refreshYiruCloudCapabilities(
  config: YiruCloudAuthConfig,
...
export async function refreshYiruCloudSession(
  config: YiruCloudAuthConfig,
  session: YiruCloudSession
): Promise<YiruCloudSessionExchangeResponse> {
  return normalizeSessionResponse(
...
export async function createYiruCloudProfile(
  config: YiruCloudAuthConfig,
  session: YiruCloudSession,
...
export async function selectYiruCloudOrg(
  config: YiruCloudAuthConfig,
  session: YiruCloudSession,
  orgId: string
...
export async function revokeYiruCloudSession(
  config: YiruCloudAuthConfig,
  session: YiruCloudSession
): Promise<void> {
