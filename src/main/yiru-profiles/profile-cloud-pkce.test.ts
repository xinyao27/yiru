...
async function startedFlow(): Promise<{
  authUrl: URL
  flow: ReturnType<typeof beginYiruCloudPkceFlow>
  nonce: string
  redirectUri: string
  state: string
}> {
  const flow = beginYiruCloudPkceFlow(config, 'local-default')
...
describe('Yiru cloud PKCE flow', () => {
  ...
  it('rejects a provider error that matches the flow state', async () => {
    ...
    await expect(observedFlow).resolves.toMatchObject({ message: 'yiru_cloud_auth_denied' })
  })
