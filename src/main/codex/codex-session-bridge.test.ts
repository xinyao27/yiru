...
function writeLegacyCopyMarker(relativePath: string, sourcePath: string, targetPath: string): void {
  ...
  const markerPath = join(getRuntimeCodexHomePath(), '.yiru-session-copies', `${relativePath}.json`)
...
beforeEach(() => {
  ...
  fakeHomeDir = mkdtempSync(join(tmpdir(), 'yiru-codex-session-home-'))
  userDataDir = mkdtempSync(join(tmpdir(), 'yiru-codex-session-user-data-'))
