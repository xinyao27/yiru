...
const REAL_APPROVED_COMMAND = '/bin/sh "/tmp/yiru-case-b-mCmCe6/agent-hooks/codex-hook.sh"'
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'yiru-codex-trust-test-'))
...
describe('computeTrustKey', () => {
  ...
  it('uses native Windows backslashes in the trust key Codex looks up', () => {
    ...
    const winPath = 'C:\\Users\\Rod\\AppData\\Roaming\\yiru\\hooks.json'
    ...
    expect(key.startsWith('C:\\Users\\Rod\\AppData\\Roaming\\yiru\\hooks.json:')).toBe(true)
  })
...
describe('upsertHookTrustEntries', () => {
  ...
  it('collapses duplicate blocks for the same hook key while preserving unrelated hook state', () => {
    const sourcePath = 'C:\\Users\\me\\AppData\\Roaming\\yiru\\codex-runtime-home\\home\\hooks.json'
    const key = `${sourcePath}:session_start:0:0`
    const unrelatedSourcePath =
      'C:\\Users\\me\\AppData\\Roaming\\yiru\\codex-runtime-home\\home\\hooks.json'
  ...
  it('preserves `enabled = false` when the user hand-edited it before reinstall', () => {
    ...
    // mute Yiru's hook short of full uninstall.
    const key = '/x/hooks.json:pre_tool_use:0:0'
    const original = `[hooks.state."${key}"]\nenabled = false\ntrusted_hash = "sha256:OLD"\n`
    writeFileSync(configPath, original, 'utf-8')
  ...
  it('finds and replaces a legacy forward-slash block when Yiru upserts with native backslash key', () => {
    // Why: Codex 0.140 can expose Windows trust keys with either separator
    // shape depending on startup cwd, so Yiru replaces stale blocks with both.
    const backslashPath = 'C:\\Users\\Rod\\AppData\\Roaming\\yiru\\hooks.json'
  ...
  it('produces exactly one Windows separator pair after two consecutive upserts', () => {
    ...
    const entry: CodexTrustEntry = {
      sourcePath: 'C:\\Users\\Rod\\AppData\\Roaming\\yiru\\hooks.json',
      eventLabel: 'session_start',
      groupIndex: 0,
      handlerIndex: 0,
      command: 'echo session'
  ...
  it('falls back to TOML basic-string headers when a Windows path contains an apostrophe', () => {
    ...
    const entry: CodexTrustEntry = {
      sourcePath: "C:\\Users\\O'Connor\\AppData\\Roaming\\yiru\\hooks.json",
      eventLabel: 'session_start',
      groupIndex: 0,
      handlerIndex: 0,
      command: 'echo session'
    ...
    expect((written.match(/\[hooks\.state\."/g) ?? []).length).toBe(2)
    expect(written).toContain(
      `[hooks.state."C:\\\\Users\\\\O'Connor\\\\AppData\\\\Roaming\\\\yiru\\\\hooks.json:session_start:0:0"]`
    )
    expect(written).toContain(
      `[hooks.state."C:/Users/O'Connor/AppData/Roaming/yiru/hooks.json:session_start:0:0"]`
    )
  ...
  it('finds a Codex-written block with lowercased username when Yiru key has mixed-case username', () => {
    // Why: realpathSync.native casing can differ between what Codex wrote
    // (C:\Users\rod\...) and what Yiru resolves (C:\Users\Rod\...).
    ...
    const lowercasePath = 'C:\\Users\\rod\\AppData\\Roaming\\yiru\\hooks.json'
    const mixedCasePath = 'C:\\Users\\Rod\\AppData\\Roaming\\yiru\\hooks.json'
...
describe('upsertProjectTrustLevel', () => {
  ...
  it('updates an existing legacy Windows forward-slash project block', () => {
    // Why: older Yiru builds normalized Windows project paths to forward
...
describe('readHookTrustEntries', () => {
  ...
  it('supports case-insensitive lookups for Windows hook trust keys read from config', () => {
    ...
    const rawKey = 'C:\\Users\\rod\\AppData\\Roaming\\yiru\\hooks.json:session_start:0:0'
    const lookupKey = 'C:/Users/Rod/AppData/Roaming/yiru/hooks.json:session_start:0:0'
    const original = [
      `[hooks.state.'${rawKey}']`,
      'enabled = true',
