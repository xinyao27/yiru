...
function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  ...
  return [
    ...
    'printf \'%s\' "$payload" | curl -sS -X POST "http://127.0.0.1:${YIRU_AGENT_HOOK_PORT}/hook/devin" \\',
...
export class DevinHookService {
  getStatus(): AgentHookInstallStatus {
    ...
    return {
      ...
      configPath,
      managedHooksPresent,
      detail: mergeHookInstallDetail(detail, readConfigFromYiruOverlapDetail(config))
    }
  }
