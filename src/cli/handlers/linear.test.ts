...
vi.mock('../runtime-client', () => {
  class RuntimeClient {
    ...
    constructor(
      ...
      remotePairingCode = process.env.YIRU_PAIRING_CODE ?? null,
...
describe('yiru linear CLI handlers', () => {
  ...
  beforeEach(() => {
    ...
    // Why: these tests can run inside a Yiru-managed terminal, which exports
  ...
  it('passes verified current-context hints without resolving cwd for remote runtimes', async () => {
    process.env.YIRU_TERMINAL_HANDLE = 'term_123'
    process.env.YIRU_WORKTREE_ID = 'repo::/srv/app'
    process.env.YIRU_PAIRING_CODE = 'yiru://pair?payload=bad'
    queueFixtures(callMock, okFixture('req_linear', issueResult()))
