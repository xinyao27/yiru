...
export const CORE_COMMAND_SPECS: CommandSpec[] = [
  ...
  ...SERVE_COMMAND_SPECS,
  {
    ...
    usage: 'yiru status [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    examples: ['yiru status', 'yiru status --json']
  },
  {
    path: ['claude-teams'],
    argumentMode: 'passthrough',
    summary: 'Start Claude Code Agent Teams in the current Yiru terminal',
    usage: 'yiru claude-teams [claude args...]',
    allowedFlags: [...GLOBAL_FLAGS],
    notes: [
      'Passes all following arguments through to Claude Code after enabling Agent Teams native panes.',
      'Must be run from inside a Yiru terminal. Starts Claude Code Agent Teams in the current pane and opens teammates as native Yiru splits.'
    ],
    examples: ['yiru claude-teams', 'yiru claude-teams --resume <session-id>']
  },
  {
    path: ['repo', 'list'],
    summary: 'List repos registered in Yiru',
    usage: 'yiru repo list [--json]',
    allowedFlags: [...GLOBAL_FLAGS]
  },
  {
    path: ['repo', 'add'],
    summary: 'Add a project to Yiru by filesystem path',
    usage: 'yiru repo add --path <path> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'path']
  },
  {
    path: ['repo', 'show'],
    summary: 'Show one registered repo',
    usage: 'yiru repo show --repo <selector> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'repo']
  },
  {
    path: ['repo', 'set-base-ref'],
    summary: "Set the repo's default base ref for future worktrees",
    usage: 'yiru repo set-base-ref --repo <selector> --ref <ref> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'repo', 'ref']
  },
  {
    path: ['repo', 'search-refs'],
    summary: 'Search branch/tag refs within a repo',
    usage: 'yiru repo search-refs --repo <selector> --query <text> [--limit <n>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'repo', 'query', 'limit']
  },
  {
    path: ['worktree', 'list'],
    summary: 'List Yiru-managed worktrees',
    usage: 'yiru worktree list [--repo <selector>] [--limit <n>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'repo', 'limit']
  },
  {
    path: ['worktree', 'show'],
    summary: 'Show one worktree',
    usage: 'yiru worktree show --worktree <selector> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'worktree']
  },
  {
    path: ['worktree', 'current'],
    summary: 'Show the Yiru-managed worktree for the current directory',
    usage: 'yiru worktree current [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    notes: [
      'Resolves the current shell directory to a path: selector so agents can target the enclosing Yiru worktree without spelling out $PWD.'
    ],
    examples: ['yiru worktree current', 'yiru worktree current --json']
  },
  {
    ...
    notes: [
      'This creates a new checkout. For a fresh agent in an existing worktree, use `yiru terminal create --worktree active --command "codex"` instead.',
      'By default, Yiru records the new worktree as a child of the caller context when it can infer one from the Yiru terminal or current directory.',
      'If --repo is omitted, Yiru infers the repo from the current Yiru-managed worktree.',
      'Use --project with --host to create on a ready project host setup without spelling the backing repo id.',
      'For related work, use the inferred parent or pass --parent-worktree active, folder:<id>, or worktree:<worktreeId> to make the relationship explicit. Worktree ids are the full <repo-id>::<path> values returned by `yiru worktree list --json`.',
      'Use --no-parent when the new worktree should be independent of the current context.',
      '--no-parent only affects Yiru lineage; omit --base-branch to use the repo default base, or pass the default base ref explicitly for independent top-level work.',
      'By default this creates the worktree and its first terminal without switching the active Yiru view.',
    ...
    examples: [
      ...
      'yiru worktree create --repo id:<repoId> --name independent-task --no-parent --json'
    ]
  },
  {
    path: ['worktree', 'set'],
  ...
  },
  {
    ...
    destructive: true,
    summary: 'Remove a worktree from Yiru and git',
    usage: 'yiru worktree rm --worktree <selector> [--force] [--run-hooks] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'worktree', 'force', 'run-hooks'],
    notes: ['Repo-defined yiru.yaml archive hooks are skipped unless --run-hooks is passed.']
  },
  {
    path: ['worktree', 'ps'],
    summary: 'Show a compact orchestration summary across worktrees',
    usage: 'yiru worktree ps [--limit <n>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'limit']
  },
  {
    path: ['terminal', 'list'],
    summary: 'List live Yiru-managed terminals',
    usage: 'yiru terminal list [--worktree <selector>] [--limit <n>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'worktree', 'limit']
  },
  {
    path: ['terminal', 'show'],
    summary: 'Show terminal metadata and preview',
    usage: 'yiru terminal show [--terminal <handle>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'terminal']
  },
  {
    path: ['terminal', 'read'],
    summary: 'Read bounded terminal output',
    usage: 'yiru terminal read [--terminal <handle>] [--cursor <n>] [--limit <n>] [--json]',
    ...
    examples: [
      ...
      'yiru terminal read --terminal term_abc123 --cursor 42 --limit 1000 --json'
    ]
  },
  {
    path: ['terminal', 'send'],
  ...
  },
  {
    ...
    usage: 'yiru terminal stop --worktree <selector> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'worktree']
  },
  {
    ...
    examples: [
      ...
      'yiru terminal create --worktree path:/projects/myapp --title "RUNNER" --command "opencode"',
      'yiru terminal create --worktree path:/projects/myapp --command "opencode" --focus'
    ]
  },
  {
    ...
    usage: 'yiru terminal switch [--terminal <handle>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'terminal'],
    examples: ['yiru terminal switch --terminal term_abc123']
  },
  {
    path: ['terminal', 'close'],
  ...
  },
  {
    ...
    usage: 'yiru terminal rename [--terminal <handle>] [--title <text>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'terminal', 'title'],
    notes: ['Omit --title or pass an empty string to reset to the auto-generated title.'],
    examples: [
      'yiru terminal rename --terminal term_abc123 --title "RUNNER"',
      'yiru terminal rename --terminal term_abc123 --json'
    ]
  },
  {
    path: ['terminal', 'split'],
    ...
    usage:
      'yiru terminal split [--terminal <handle>] [--direction horizontal|vertical] [--command <text>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'terminal', 'direction', 'command'],
    examples: [
      'yiru terminal split --terminal term_abc123 --direction horizontal --json',
      'yiru terminal split --terminal term_abc123 --command "codex"'
    ]
  }
]
