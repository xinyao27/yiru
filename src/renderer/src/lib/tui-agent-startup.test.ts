...
describe('buildAgentDraftLaunchPlan', () => {
  ...
  it('uses YIRU_PI_PREFILL env var for pi (no CLI flag exists)', () => {
    // Why: pi has no `--prefill` flag, and bracketed-paste-after-ready races
    // against pi's lengthy startup output. The Yiru overlay installs an
    // `yiru-prefill` extension that reads YIRU_PI_PREFILL on session_start
    ...
    expect(
      ...
      agent: 'pi',
      launchCommand: 'pi; unset YIRU_PI_PREFILL',
      expectedProcess: 'pi',
      env: { YIRU_PI_PREFILL: 'https://github.com/acme/repo/issues/42' },
