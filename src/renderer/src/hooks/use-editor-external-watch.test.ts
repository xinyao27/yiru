...
import {
  ...
  getWatchedTargetKey
} from './use-editor-external-watch'
...
describe('createExternalWatchEventHandler tombstone coalescing', () => {
  ...
  it('does not drop external edits that arrive inside the self-write TTL', async () => {
    ...
    recordSelfWrite('/repo/notes.md', 'yiru save')
  ...
  it('still suppresses the watcher echo from Yiru self-writes', async () => {
    vi.mocked(useAppStore.getState).mockReturnValue({
      openFiles: [fileNotes],
      setExternalMutation
    } as never)
    vi.mocked(getOpenFilesForExternalFileChange).mockReturnValue([fileNotes] as never)
    const readFile = vi.fn().mockResolvedValue({ content: 'yiru save', isBinary: false })
    vi.stubGlobal('window', { api: { fs: { readFile } } })
    const { handleFsChanged, dispose } = createExternalWatchEventHandler(findTarget)
  ...
  it('does not mark a dirty tab for the echo of Yiru’s own save', async () => {
    const dirtyFile = {
      ...fileNotes,
      isDirty: true
    }
