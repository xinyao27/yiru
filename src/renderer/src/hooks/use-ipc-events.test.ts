...
describe('useIpcEvents zoom routing', () => {
  ...
  it('applies app zoom for an active browser tab', async () => {
    ...
    vi.doMock('@/components/settings/settings-constants', () => ({
    ...
    const { useIpcEvents } = await import('./use-ipc-events')
...
describe('useIpcEvents browser tab close routing', () => {
  ...
  async function useIpcEventsForCloseRouting({
    ...
    const { useIpcEvents: registerIpcEvents } = await import('./use-ipc-events')
    registerIpcEvents()
  }
