...
import { SkillFreshnessUpdateDialog } from './skill-freshness-update-dialog'
import {
...
vi.mock('@/hooks/use-skill-freshness', () => ({
  useSkillFreshness: () => ({
...
vi.mock('@/hooks/use-installed-agent-skills', () => ({
  notifyInstalledAgentSkillsChanged: mocks.notifyChanged
}))

vi.mock('@/components/onboarding/onboarding-inline-command-terminal', () => ({
...
function eligibleInventory(): SkillFreshnessInventory {
  return {
    schemaVersion: 1,
    installations: [placement('yiru-cli')],
    eligibleUpdateNames: ['yiru-cli'],
    scannedAt: 1
  }
}
...
describe('SkillFreshnessUpdateDialog', () => {
  ...
  it('shows the eligible summary and the exact pre-filled draft command', async () => {
    ...
    expect(mocks.terminalProps.at(-1)).toMatchObject({
      command: 'npx skills update yiru-cli --global',
  ...
  it('resolves a request made before inventory loads once a safe command arrives', async () => {
    ...
    expect(mocks.terminalProps.at(-1)?.command).toBe('npx skills update yiru-cli --global')
  })
  ...
  it('shows the up-to-date state once every installation is current', async () => {
    ...
    mocks.inventory = {
      schemaVersion: 1,
      installations: [
        placement('yiru-cli', { status: 'current', observedPackageDigest: 'current' })
      ],
      eligibleUpdateNames: [],
      scannedAt: 2
    }
    ...
    expect(mocks.terminalCommits).toContainEqual(['npx skills update yiru-cli --global', true])
    expect(container?.textContent).toContain('All installed Yiru skills are up to date.')
  ...
  it('auto-expands Details when a rescan changes the result to blocked', async () => {
    ...
    mocks.inventory = {
      schemaVersion: 1,
      installations: [placement('yiru-cli', { topology: 'read-only' })],
      eligibleUpdateNames: [],
      scannedAt: 2
    }
    await rerender()
  ...
  it('keeps the same terminal inert while inventory is revalidated', async () => {
    ...
    expect(container?.textContent).toContain('Checking installed Yiru skills')
  ...
  it('shows a failed scan as an error instead of indefinite progress', async () => {
    ...
    expect(container?.textContent).not.toContain('Checking installed Yiru skills')
    expect(container?.querySelector('[data-testid="update-terminal"]')).toBeNull()
  })
    ...
    mocks.inventory = {
      ...
      installations: [
        ...
        placement('yiru-cli', {
          ...
          topology: 'repo-scope',
          status: 'unrecognized',
          unresolvedPath: '/repo/.agents/skills/yiru-cli'
        })
      ],
      eligibleUpdateNames: [],
      scannedAt: 1
  ...
  it('explains a self-blocked read-only placement without blaming a sibling', async () => {
    ...
    expect(container?.textContent).toContain(
      'read-only location, so Yiru left it out of the update'
    )
