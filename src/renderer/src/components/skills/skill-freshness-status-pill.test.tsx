...
import { SkillFreshnessStatusPill } from './skill-freshness-status-pill'
...
describe('SkillFreshnessStatusPill', () => {
  ...
  it('shows Update available for an eligible outdated skill', async () => {
    mocks.inventory = inventory([{ name: 'yiru-cli', status: 'outdated' }], ['yiru-cli'])

    expect((await renderPill('yiru-cli')).textContent).toBe('Update available')
  })

  it('shows Up to date when every placement is current', async () => {
    mocks.inventory = inventory([{ name: 'yiru-cli', status: 'current' }], [])

    expect((await renderPill('yiru-cli')).textContent).toBe('Up to date')
  })

  it('falls back to Installed for a blocked outdated placement', async () => {
    mocks.inventory = inventory(
      [
        { name: 'yiru-cli', status: 'outdated' },
        { name: 'yiru-cli', status: 'unrecognized' }
      ],
      []
    )

    expect((await renderPill('yiru-cli')).textContent).toBe('Installed')
  })
