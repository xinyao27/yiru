...
vi.mock('./use-local-image-src', () => ({ useLocalImageSrc: (src?: string) => src }))
vi.mock('./mermaid-block', () => ({ default: () => null }))
vi.mock('./code-block-copy-button', () => ({
  default: ({ children }: { children: React.ReactNode }) => children
}))
vi.mock('../diff-comments/diff-comment-card', () => ({ DiffCommentCard: () => null }))
vi.mock('./notes-send-menu', () => ({ NotesSendMenu: () => null }))
vi.mock('./markdown-table-of-contents-panel', () => ({ MarkdownTableOfContentsPanel: () => null }))
