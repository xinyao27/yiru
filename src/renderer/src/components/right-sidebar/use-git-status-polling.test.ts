...
async function usePollingOnce(
  ...
  const { useGitStatusPolling: runPolling } = await import('./use-git-status-polling')
...
describe('useGitStatusPolling', () => {
  ...
  it('coalesces active worktree file-watch bursts into one git status refresh', async () => {
    ...
    const emitWorktreeFileChange = (payload: FsChangedPayload): void => {
      for (const listener of windowListeners.get('yiru:worktree-file-change') ?? []) {
    ...
    expect(windowListeners.get('yiru:worktree-file-change')?.length).toBe(1)
