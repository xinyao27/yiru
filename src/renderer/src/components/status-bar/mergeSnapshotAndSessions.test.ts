...
describe('mergeSnapshotAndSessions', () => {
  ...
  it('includes browser-only workspaces in their repo', () => {
    ...
    const out = mergeSnapshotAndSessions(
      ...
      [],
      baseCtx({
        repoDisplayNameById: new Map([['yiru', 'YIRU']]),
  ...
  it('passes through snapshot worktrees with numeric metrics and hasLocalSamples', () => {
    const wt: WorktreeMemory = {
      worktreeId: 'yiru::/Users/me/Triton',
      worktreeName: 'Triton',
      repoId: 'yiru',
      repoName: 'YIRU',
      cpu: 1.5,
  ...
  it('@@ parse: an SSH-style session id resolves to its worktree group', () => {
    const ds: DaemonSession[] = [
      { id: 'yiru::/remote/Stingray@@abcd1234', cwd: '', title: 'yiru/Stingray' }
    ]
    const ctx = baseCtx({
      repoConnectionIdById: new Map([['yiru', 'ssh-conn-1']])
    })
    ...
    expect(out[0].worktrees[0]).toMatchObject({
      worktreeId: 'yiru::/remote/Stingray',
    ...
    expect(out[0].worktrees[0].sessions[0]).toMatchObject({
      sessionId: 'yiru::/remote/Stingray@@abcd1234',
      hasLocalSamples: false,
      cpu: null,
      memory: null,
      bound: false
  ...
  it('warm-reattach local PTY: chip stays off when repo has no connectionId', () => {
    ...
    const ds: DaemonSession[] = [
      { id: 'yiru::/local/Triton@@deadbeef', cwd: '/local/Triton', title: 'yiru/Triton' }
    ]
    const ctx = baseCtx({
      repoConnectionIdById: new Map([['yiru', null]])
    })
    const out = mergeSnapshotAndSessions(null, ds, ctx)
    expect(out[0]).toMatchObject({
      repoId: 'yiru',
  ...
  it('tab walk wins over @@ parse when they disagree', () => {
    const tabId = 'tab-xyz'
    const ds: DaemonSession[] = [{ id: 'yiru::/wrong/path@@feedface', cwd: '', title: 'yiru' }]
    const ctx = baseCtx({
      tabsByWorktree: {
        'yiru::/correct/path': [makeTab(tabId, 'My Tab')]
      },
      ptyIdsByTabId: { [tabId]: ['yiru::/wrong/path@@feedface'] }
    })
    const out = mergeSnapshotAndSessions(null, ds, ctx)
    expect(out[0].worktrees[0].worktreeId).toBe('yiru::/correct/path')
  ...
  it('treats startup deferred reattach tab ptyId wake hints as bound sessions', () => {
    ...
    const sessionId = 'yiru::/Users/me/Triton@@deferred'
    const ds: DaemonSession[] = [{ id: sessionId, cwd: '/Users/me/Triton', title: 'yiru/Triton' }]
    const restoredTab = { ...makeTab(tabId, 'Restored'), ptyId: sessionId }
    const ctx = baseCtx({
      tabsByWorktree: {
        'yiru::/Users/me/Triton': [restoredTab]
      },
      ptyIdsByTabId: { [tabId]: [] }
    })
  ...
  it('local-bound interaction state: numeric metrics + bound=true + tabId set', () => {
    ...
    const ctx = baseCtx({
      tabsByWorktree: { 'yiru::/Users/me/Triton': [makeTab(tabId)] },
  ...
  it('remote-orphan interaction state: null metrics + bound=false', () => {
    const ds: DaemonSession[] = [{ id: 'yiru::/remote/Wt@@deadbeef', cwd: '', title: 'yiru/Wt' }]
    const out = mergeSnapshotAndSessions(null, ds, baseCtx())
    const session = out[0].worktrees[0].sessions[0]
    expect(session).toMatchObject({
      hasLocalSamples: false,
  ...
  it('uses repoDisplayNameById to humanize new project groups when available', () => {
    const ds: DaemonSession[] = [{ id: 'stably-ai/yiru::/remote/Wt@@1', cwd: '', title: '' }]
    const ctx = baseCtx({
      repoDisplayNameById: new Map([['stably-ai/yiru', 'YIRU']])
    })
    const out = mergeSnapshotAndSessions(null, ds, ctx)
    expect(out[0].repoName).toBe('YIRU')
  })
