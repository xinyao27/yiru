...
function UsageBar({ usedPercent }: { usedPercent: number }): React.JSX.Element {
  return (
...
export function ProviderUsageSegment({
  ...
  if (limits.status === 'fetching' && !limits.session && !limits.weekly && !limits.fableWeekly) {
    return (
      <span className="inline-flex items-center gap-1 text-muted-foreground">
        <ProviderIcon provider={provider} />
        <span className="animate-pulse">···</span>
  ...
  if (limits.status === 'error' && !limits.session && !limits.weekly && !limits.fableWeekly) {
    return (
      <span className="inline-flex items-center gap-1 text-muted-foreground">
        <ProviderIcon provider={provider} />
  ...
  return (
    <span className="inline-flex items-center gap-1.5">
      <ProviderIcon provider={provider} />
      {limits.session && !compact ? <UsageBar usedPercent={limits.session.usedPercent} /> : null}
