...
vi.mock('@/components/dashboard/use-now', () => ({
  useNow: vi.fn(() => 2000)
}))

vi.mock('@/components/dashboard/dashboard-agent-row', () => ({
  default: ({
    agent,
    onActivate
  }: {
...
describe('WorktreeCardAgents activation', () => {
  ...
  it('reveals the worktree and focuses an automation worker row hydrated during reveal', async () => {
    ...
    const { default: WorktreeCardAgents } = await import('./worktree-card-agents')
  ...
  it('reveals the worktree and focuses a compact automation worker row hydrated during reveal', async () => {
    ...
    const row = Array.from(host.querySelectorAll('[title]')).find((element) =>
      element.getAttribute('title')?.includes('Compact worker')
    )
