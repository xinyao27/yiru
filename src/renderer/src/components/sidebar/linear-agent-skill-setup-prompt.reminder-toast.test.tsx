...
} from './linear-agent-skill-setup-prompt'
import {
...
const HOST_DISMISS_STORAGE_KEY = 'yiru.linearTicketsSkill.setupDismissed.host'
...
vi.mock('@/lib/agent-skill-cli-prerequisite', () => ({
  ...
  ensureYiruCliAvailableForAgentSkillTerminal: mocks.ensureCli,
  isYiruCliAvailableOnPath: (status: CliInstallStatus | null | undefined) =>
    status?.state === 'installed' && status.pathConfigured
}))

vi.mock('../settings/cli-skill-runtime-setup', () => ({
...
vi.mock('../settings/agent-skill-setup-panel', () => ({
...
describe('LinearAgentSkillSetupPrompt reminder toast', () => {
  ...
  it('shows a warning toast on a later modal-only activation after Not now', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        id: 'linear-agent-skill-setup-yiru.linearTicketsSkill.setupDismissed.host',
        description:
  ...
  it('does not repeat the Yiru CLI in CLI-only reminder toast copy', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      'Yiru CLI is missing',
      expect.objectContaining({
        description: 'Install the Yiru CLI to enable your agents to read and edit Linear tasks.'
      })
    )
  })
  ...
  it('keeps remote setup nuance in reminder toast copy', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        description:
          'Install the Yiru CLI and the Linear skill to enable your agents to read and edit Linear tasks. Remote agent environments may need their own setup.'
      })
    )
  })
  ...
  it('keeps WSL target nuance in reminder toast copy', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        description:
          'Install the Yiru CLI and the Linear skill to enable your agents to read and edit Linear tasks. This setup runs in the selected WSL agent runtime.'
      })
    )
  })
  ...
  it('opens the setup dialog from the reminder toast action', async () => {
    ...
    expect(toast.dismiss).toHaveBeenCalledWith(
      'linear-agent-skill-setup-yiru.linearTicketsSkill.setupDismissed.host'
    )
  })
