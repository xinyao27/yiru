...
vi.mock('@/hooks/use-virtualized-scroll-anchor', () => ({
...
vi.mock('./worktree-activity-status-indicator', () => ({
...
describe('WorktreeList lineage child card renderer', () => {
  beforeAll(async () => {
    WorktreeList = (await import('./worktree-list')).default as WorktreeListComponent
  }, 20_000)
  ...
  it('exposes expanded state on project group controls with children', async () => {
    setProjectGroupWithoutWorktreeRowsState()
    const markup = await renderWorktreeListMarkup()
  ...
  it('exposes expanded state on status group controls with worktrees', async () => {
  ...
  it('exposes expanded state on grouped repo controls with worktrees', async () => {
