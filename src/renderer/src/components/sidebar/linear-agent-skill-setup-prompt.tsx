...
} from '../settings/cli-skill-runtime-setup'
import {
...
import { LinearAgentSkillSetupDialog } from './linear-agent-skill-setup-dialog'
...
export function LinearAgentSkillSetupPrompt({
  ...
  const command = useMemo(
    () => buildSkillCommandForRuntime(YIRU_LINEAR_SKILL_INSTALL_COMMAND, agentRuntime),
    [agentRuntime]
  )
  ...
  const cliAvailable = isYiruCliAvailableOnPath(cliStatus)
  ...
  return (
    <div
      className={cn(
        'mt-1.5 rounded-md border border-sidebar-border bg-sidebar-accent/35 px-2.5 py-2 text-[11px] text-muted-foreground',
        className
      )}
