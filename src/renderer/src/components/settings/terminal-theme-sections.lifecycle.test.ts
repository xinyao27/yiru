...
vi.mock('./terminal-settings-preview', () => ({
...
function findElementByProp(
  ...
  if (Array.isArray(node)) {
    for (const child of node) {
      const found = findElementByProp(child, propName, expectedValue)
      if (found) {
        return found
      }
    }
  ...
  return findElementByProp(element.props?.children, propName, expectedValue)
}
...
describe('TerminalThemeCatalogSection', () => {
  ...
  it('collapses light customization with a transition while matching dark mode', () => {
    ...
    const preview = findElementByTypeName(element, 'TerminalSettingsPreview')
    const matchDarkModeSwitch = findElementByTypeName(element, 'SettingsSwitchRow')
    const transitionRegion = findElementByProp(element, 'inert', true)
