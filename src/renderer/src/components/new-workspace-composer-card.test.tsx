...
import NewWorkspaceComposerCard from './new-workspace-composer-card'
...
vi.mock('@/components/agent/agent-combobox', () => ({
  default: () => <button type="button">Agent picker</button>
}))

vi.mock('@/components/sparse/sparse-checkout-preset-select', () => ({
  default: () => <div data-testid="sparse-select" />
}))

vi.mock('@/components/new-workspace/smart-workspace-name-field', () => ({
  default: ({
    branchesEnabled,
...
vi.mock('@/components/new-workspace/project-combobox', () => ({
  default: ({
    options,
    value,
    onValueChange
      ...
      ephemeralVmRecipes: [
        {
          id: 'vercel',
          name: 'Vercel Sandbox',
          create: './scripts/yiru-vm/vercel.start.sh',
          destroy: './scripts/yiru-vm/vercel.cleanup.sh',
