...
import {
  getYiruProfileProjectLiveWorkSummary,
  getYiruProfileSwitchLiveWorkSummary,
  type YiruProfileSwitchLiveWorkState
} from './yiru-profile-switch-liveness'

const NOW = 1_000_000

function makeState(
  overrides: Partial<YiruProfileSwitchLiveWorkState> = {}
): YiruProfileSwitchLiveWorkState {
  return {
...
describe('getYiruProfileSwitchLiveWorkSummary', () => {
  it('reports quiet profiles as safe to switch without confirmation', () => {
    expect(getYiruProfileSwitchLiveWorkSummary(makeState(), NOW)).toEqual({
  ...
  it('counts live PTYs as live terminal work', () => {
    const summary = getYiruProfileSwitchLiveWorkSummary(
      makeState({
        ptyIdsByTabId: {
          'tab-1': ['pty-1', 'pty-2'],
          'tab-2': []
  ...
  it('filters live work to the selected project', () => {
    const summary = getYiruProfileProjectLiveWorkSummary(
      makeState({
        agentStatusByPaneKey: {
        ...
        browserTabsByWorktree: {
          'repo-1::/workspace/yiru': [makeBrowserWorkspace('browser-1')],
        ...
        tabsByWorktree: {
          'repo-1::/workspace/yiru': [makeTab({ id: 'tab-live', title: 'Codex working' })],
