...
describe('connectPanePty', () => {
  ...
  it('orders a startup draft behind xterm focus input when Codex renders its composer', async () => {
    ...
    const deps = createDeps({
      startup: {
        ...
        draftPrompt: 'https://github.com/xinyao27/yiru/issues/42'
      }
    })
    ...
    expect(transport.sendInputAccepted).toHaveBeenCalledWith(
      '\x1b[200~https://github.com/xinyao27/yiru/issues/42\x1b[201~'
    )
  ...
  it('confirms an Yiru-launched Droid fresh spawn in a no-OSC shell (Git Bash)', async () => {
  ...
  // xterm onData or pointerdown on the container (see terminal-pane.tsx). This test
