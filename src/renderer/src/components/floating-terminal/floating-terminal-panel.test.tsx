...
vi.mock('@/components/tab-bar/tab-bar', () => ({
  default: function TabBar() {
    return null
  }
}))

vi.mock('@/components/terminal-pane/terminal-pane', () => ({
  default: function TerminalPane() {
    return null
  }
}))

vi.mock('@/components/browser-pane/browser-pane', () => ({
  default: function BrowserPane() {
    return null
  }
}))

vi.mock('@/components/emulator-pane/emulator-pane', () => ({
...
vi.mock('@/components/terminal/use-terminal-save-dialog', () => ({
...
function makeFile(overrides: Partial<OpenFile> = {}): OpenFile {
  ...
  return {
    id,
    filePath: overrides.filePath ?? `/tmp/yiru/${id}.md`,
...
async function renderPanel(
  ...
  hookRuntime.index = 0
  const { FloatingTerminalPanel } = await import('./floating-terminal-panel')
...
describe('FloatingTerminalPanel close behavior', () => {
  beforeEach(() => {
    ...
    mocks.getFloatingMarkdownDirectory.mockResolvedValue('/tmp/yiru/floating-notes')
    mocks.getFloatingTerminalCwd.mockResolvedValue('/tmp/yiru')
  ...
  it('creates floating markdown files in local filesystem mode', async () => {
    ...
    vi.mocked(createUntitledMarkdownFileWithTemplateSelection).mockResolvedValue({
      filePath: '/tmp/yiru/floating-notes/untitled.md',
    ...
    expect(createUntitledMarkdownFileWithTemplateSelection).toHaveBeenCalledWith(
      '/tmp/yiru/floating-notes',
      FLOATING_TERMINAL_WORKTREE_ID,
      undefined,
      { activeRuntimeEnvironmentId: null }
    )
    expect(mocks.openFile).toHaveBeenCalledWith(
      expect.objectContaining({ filePath: '/tmp/yiru/floating-notes/untitled.md' }),
