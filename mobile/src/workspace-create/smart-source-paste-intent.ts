...
import { parseGitLabMergeRequestLink } from '../../../src/shared/new-workspace/gitlab-links'
...
export type GitHubPasteIntent =
  | { kind: 'github-link'; link: GitHubPullRequestLink }
...
// Pure: classify pasted text into a review lookup intent. A slug-bearing
// GitHub URL becomes 'github-link'; a bare "#123"/number becomes 'github-number';
// a GitLab MR URL becomes 'gitlab-link'.
export function resolvePasteIntent(query: string): PasteIntent {
  ...
  const glLink = parseGitLabMergeRequestLink(trimmed)
  if (glLink) {
