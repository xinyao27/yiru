...
function subscribeLikeYiru(dir) {
  ...
  return client
    .subscribeViaWatcherProcess(
      ...
      (err, events) => {
        if (err) {
          stats.watchErrors++
          // Yiru's error path: unsubscribe from inside the error callback.
          if (subRef.current) {
            stats.unsubscribes++
...
async function deleteRootLane(baseDir, durationMs, lane) {
  ...
  while (Date.now() < end) {
    ...
    let subRef
    try {
      subRef = await subscribeLikeYiru(dir)
    } catch {
      rmrf(dir)
      round++
      continue
...
async function main() {
  ...
  const baseDir = path.join(os.tmpdir(), 'yiru-7547-harness-fixed', `run-${process.pid}`)
