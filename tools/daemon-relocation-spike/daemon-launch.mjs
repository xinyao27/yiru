// Launches the copied Yiru.exe as the daemon host (ELECTRON_RUN_AS_NODE=1) and
// waits for its {type:'ready'} IPC signal. Mirrors the real fork() options in
// src/main/daemon/daemon-init.ts (detached, ipc channel, YIRU_USER_DATA_PATH).
...
export function launchDaemonHost(options) {
  ...
  // if run against a build that carries the YIRU_NODE_PTY_NATIVE_DIR patch.
  if (nodePtyNativeDir) {
    env.YIRU_NODE_PTY_NATIVE_DIR = nodePtyNativeDir
  }
