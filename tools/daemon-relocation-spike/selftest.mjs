...
function makeSyntheticInventory() {
  ...
  return {
    appDir: 'C:\\App',
    unpackedRoot,
    hostExe: { name: 'Yiru.exe', ...f('C:\\App\\Yiru.exe') },
    runtimeData: [
...
function run() {
  ...
  for (const tier of TIERS) {
    ...
    check(`${tier}: has exe`, dests.includes('Yiru.exe'))
  ...
  // ── Module-path filter ──────────────────────────────────────────────
  const appDir = 'C:\\Users\\me\\AppData\\Local\\Programs\\yiru'
  const modules = [
    'C:\\Users\\me\\AppData\\Local\\Programs\\yiru\\Yiru.exe',
    'C:\\Windows\\System32\\kernel32.dll',
    'C:\\Users\\me\\AppData\\Local\\yiru-daemon-host\\Yiru.exe'
  ]
  const resident = findAppDirResidentModules(modules, appDir)
  check('detects app-dir module', resident.length === 1)
  check('detects the right module', resident[0].toLowerCase().includes('programs\\yiru\\yiru.exe'))
  // Sibling-prefix must NOT match (C:\...\yiru vs C:\...\yiru-daemon-host).
  check(
    'sibling prefix not matched',
    findAppDirResidentModules(['C:\\a\\yiru-daemon-host\\x.dll'], 'C:\\a\\yiru').length === 0
  )
  // Forward/back-slash + case normalization.
  check(
    'slash + case normalized',
    findAppDirResidentModules(['c:/a/YIRU/x.dll'], 'C:\\A\\yiru').length === 1
  )
  check(
    'relocated host has zero app-dir modules',
    findAppDirResidentModules(
      ['C:\\work\\daemon-host\\Yiru.exe', 'C:\\Windows\\System32\\ntdll.dll'],
      appDir
    ).length === 0
  )
