...
**Windows only.** Given two Yiru Windows installers (version N and N+1), this
...
- **The daemon is identified by command-line marker, never by exe name.** With
  `ELECTRON_RUN_AS_NODE` the daemon image is `Yiru.exe`; a relocated Phase 1
...
| Flag                                          | Meaning                                                     |
| --------------------------------------------- | ----------------------------------------------------------- |
| `--from <path>` / `--to <path>`               | Local `yiru-windows-setup.exe` for base (N) / update (N+1)  |
...
- **Pre-existing app process → hard refusal.** If a Yiru _app_ process (not a
...
- **Pre-existing install → refusal unless `--allow-existing-install`.** If an
  Yiru install already exists under `%LOCALAPPDATA%\Programs`, the run refuses,
...
(`%LOCALAPPDATA%\Programs\Yiru`). A developer's box already has a real Yiru there,
...
1. **Snapshots** the shared state (`registry-shortcut-backup.mjs`): `reg export`s
   each existing key to `.reg` files, copies the Yiru `*.lnk` shortcuts, and records
...
and the Yiru shortcuts — the state that steers a future update and the user-visible
...
pnpm win-update-e2e \
  ...
  --expect cold-restore --install-dir C:\YiruE2E
```
...
1. **Preflight** — assert win32; warn if elevated; **refuse** to run if a
   pre-existing Yiru _app_ process (not a daemon) is running that the harness
...
2. **Install N** silently (`<setup.exe> /S`) and locate `Yiru.exe`.
...
| `registry-shortcut-backup.mjs` | Isolated mode: snapshot/restore the shared HKCU keys + Yiru shortcuts     |
...
- The harness assumes the packaged main honors `YIRU_E2E_USER_DATA_DIR` to
